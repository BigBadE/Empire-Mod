...
public class Android_Tiers_Patches {
  ...
  //
  public static bool Prefix(PawnGroupMakerParms parms, bool warnOnZeroResults,
  ...
  public static void Patch(Harmony harmony) {
    ...
    Type typ = FactionColonies.returnUnknownTypeFromName(
        "AndroidTiers.PawnGroupMakerUtility_Patch");

    // Get type inside of type
    Type[] types =
        typ.GetNestedTypes(BindingFlags.Public | BindingFlags.Static);
    foreach (Type t in types) {
      Log.Message(t.ToString());
      if (t.ToString() ==
          "AndroidTiers.PawnGroupMakerUtility_Patch+GeneratePawns_Patch") {
        typ = t;
    ...
    MethodInfo originalpre =
        typ.GetMethod("Listener", BindingFlags.Static | BindingFlags.NonPublic);
