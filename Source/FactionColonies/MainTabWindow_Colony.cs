...
public class MainTabWindow_Colony : MainTabWindow {
  ...
  public override Vector2 InitialSize {
    get { return new Vector2(350f, 400f); }
  }

  public void MainTabWindow() { this.closeOnClickedOutside = false; }
  ...
  public List<string> stats_tab_0 =
      new List<string>{"happiness", "loyalty", "unrest", "prosperity"};

  public List<string> buttons_tab_0 = new List<string>{
      "FCOverview".Translate(), "Military".Translate(), "Actions".Translate()};

  public override void PostOpen() { base.PostOpen(); }
  ...
  public override void PreOpen() {
    ...
    if (faction != null) {
      ...
      Log.Message(
          "WorldComp FactionFC is null - Something is wrong! Empire Mod");
    }
  }
  ...
  private void DrawColonySettlementCreationButton(Rect inRect) {
    ...
    if (gfaction != null) {
      ...
      if (Widgets.ButtonText(button, "CreateNewColony".Translate())) {
        ...
        Messages.Message("SelectTile".Translate(),
    ...
    } else // create new faction
    {
      if (Widgets.ButtonText(button, "Create New Faction")) {
        ...
        if (Find.CurrentMap.Parent != null &&
            Find.CurrentMap.Parent.Tile != null &&
            Find.WorldObjects.SettlementAt(Find.CurrentMap.Parent.Tile) !=
                null) {
          Messages.Message(
              Find.WorldObjects.SettlementAt(Find.CurrentMap.Parent.Tile).Name +
                  " " + "SetAsFactionCapital".Translate() + "!",
  ...
  private void DrawTabFaction(Rect inRect) {
    ...
    if (Widgets.ButtonTextSubtle(
            new Rect(0, 0, tabSize, 30), "Faction".Translate(), 0f, 8f,
            SoundDefOf.Mouseover_Category, new Vector2(-1f, -1f))) {
      tab = 0;
      faction.updateTotalProfit();
    }
  }
  private void DrawTabColony(Rect inRect) {
    ...
    if (Widgets.ButtonTextSubtle(
            new Rect(tabSize, 0, tabSize, 30), "Colonies".Translate(), 0f, 8f,
            SoundDefOf.Mouseover_Category, new Vector2(-1f, -1f))) {
      tab = 1;
      scroll = 0;
      maxScroll = (settlementList.Count() * yspacing) - 264;
  ...
  private void DrawTabReports(Rect inRect) {
    ...
    if (Widgets.ButtonTextSubtle(
            new Rect(tabSize * 2, 0, tabSize, 30), "Bills".Translate(), 0f, 8f,
            SoundDefOf.Mouseover_Category, new Vector2(-1f, -1f))) {
  ...
  private void DrawTabEvent(Rect inRect) {
    ...
    if (Widgets.ButtonTextSubtle(
            new Rect(tabSize * 3, 0, tabSize, 30), "Events".Translate(), 0f, 8f,
            SoundDefOf.Mouseover_Category, new Vector2(-1f, -1f))) {
      // tab = 0;
      // Open Event window
  ...
  private void DrawSettlementButtons(Rect inRect) {
    ...
    List<String> headerList = new List<String>(){"Settlement".Translate(),
                                                 "Level".Translate(),
                                                 "FreeWorkers".Translate(),
                                                 "Unrest".Translate(),
                                                 "Loyalty".Translate(),
                                                 "Profit".Translate(),
                                                 "Location",
    ...
    for (int i = 0; i < headerList.Count() - 2;
         i++) //-2 to exclude location and ID
    {
      int xspacingUpdated;
      ...
      case 0:
        ...
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementName);
        };
        break;
      case 1:
        xspacingUpdated = xspacing - 10;
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementLevel);
        };
        break;
      case 2:
        xspacingUpdated = xspacing;
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementFreeWorkers);
        };
        break;
      case 3:
        xspacingUpdated = xspacing - 4;
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementUnrest);
        };
        break;
      case 4:
        xspacingUpdated = xspacing;
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementLoyalty);
        };
        break;
      case 5:
        xspacingUpdated = xspacing + 14;
        method = delegate {
          settlementList.Sort(FactionColonies.CompareSettlementProfit);
        };
        break;
      default:
      ...
      if (i == 0) {
        Widgets.Label(new Rect(2 + adjust2, 60, xspacingUpdated, 40),
                      headerList[i]);
        if (Widgets.ButtonInvisible(
                new Rect(2 + adjust2, 60, xspacingUpdated, 40))) {
          method.Invoke();
        }
      } else {
        Widgets.Label(new Rect(2 + adjust2, 60, xspacingUpdated, 40),
    ...
    for (int i = 0; i < settlementList.Count();
         i++) // browse through list.  settlementList[i] = a settlement
    {
      SettlementFC settlement = settlementList[i];
      if (i * yspacing + scroll >= 0 && i * yspacing + scroll <= 264) {
        if (i % 2 == 0) {
          Widgets.DrawHighlight(
              new Rect(0, yoffset + i * yspacing + scroll, 312, 30));
        }
        int adjust = 0;
        for (int k = 0; k < 6; k++) // Browse through settlement information -2
                                    // to exclude location and ID
        {
          int xspacingUpdated;
          GUIContent varString;
          switch (k) {
          ...
          case 1:
            ...
            break;
          case 2:
            varString = new GUIContent(
                (settlement.workersUltraMax - settlement.getTotalWorkers())
                    .ToString());
            xspacingUpdated = xspacing - 14;
            break;
          case 3:
            varString = new GUIContent(settlement.unrest.ToString(),
                                       texLoad.iconUnrest);
            xspacingUpdated = xspacing + 4;
            break;
          case 4:
            varString = new GUIContent(settlement.loyalty.ToString(),
                                       texLoad.iconLoyalty);
            xspacingUpdated = xspacing;
            break;
          case 5:
            varString =
                new GUIContent(settlement.totalProfit.ToString(),
                               settlement.returnHighestResource().getIcon());
          ...
          if (k == 0) {
            if (Widgets.ButtonText(
                    new Rect(2 + adjust, yoffset + i * yspacing + scroll,
                             xspacingUpdated, 30),
                    "")) { // When button of settlement name is pressed
              Find.WindowStack.Add(new settlementWindowFC(settlement));
            }
            Widgets.Label(new Rect(2 + adjust, yoffset + i * yspacing + scroll,
                                   xspacingUpdated, 30),
                          settlement.name);
          } else {
            ...
            // ist[0] = name;   //settlement name
            // list[1] = settlementLevel.ToString(); //settlement level
            // list[2] = settlementMilitaryLevel.ToString(); //settlement
            // military level list[3] = unrest.ToString(); //settlement unrest
            // list[4] = loyalty.ToString(); //settlement loyalty
            // list[5] = getTotalProfit().ToString(); //settlement profit
            // list[6] = mapLocation.ToString(); //settlement location
            // list[7] = ID
            // Widgets.Label(new Rect(headerSpacing + 2 + k * xspacing, yoffset
            // + i * yspacing + scroll, xspacing, 30), varString);
          }
  ...
  private void DrawFactionName(Rect inRect) {
    ...
    if (Widgets.ButtonImage(
      ...
      if (fact != null)
        ...
        Messages.Message("No faction created to customize",
  ...
  private void DrawFactionStats(Rect inRect, int statSize) {
    ...
    for (int i = 0; i < stats.Count(); i++) {
      if (stats[i] == "happiness") {
        Widgets.DrawBox(
            new Rect(0, 105 + ((statSize + 15) * i), 125, statSize + 10));
        Widgets.DrawHighlight(
            new Rect(0, 105 + ((statSize + 15) * i), 125, statSize + 10));
        if (Widgets.ButtonImage(new Rect(5 - 2, 110 + ((statSize + 15) * i) - 2,
                                         statSize + 4, statSize + 4),
                                texLoad.iconHappiness)) {
          Find.WindowStack.Add(
              new descWindowFC("FactionHappinessDesc".Translate(),
                               "FactionHappiness".Translate()));
        };
        Widgets.Label(new Rect(50, 110 + ((statSize + 15) * i), 80, statSize),
                      Convert.ToInt32(faction.averageHappiness) + "%");
      }
      if (stats[i] == "loyalty") {
        Widgets.DrawBox(
            new Rect(0, 105 + ((statSize + 15) * i), 125, statSize + 10));
        ...
        if (Widgets.ButtonImage(new Rect(5 - 2, 110 + ((statSize + 15) * i) - 2,
          ...
          Find.WindowStack.Add(new descWindowFC(
              "FactionLoyaltyDesc".Translate(), "FactionLoyalty".Translate()));
        };
        Widgets.Label(new Rect(50, 110 + ((statSize + 15) * i), 80, statSize),
                      Convert.ToInt32(faction.averageLoyalty) + "%");
      }
      if (stats[i] == "unrest") {
        Widgets.DrawBox(
            new Rect(0, 105 + ((statSize + 15) * i), 125, statSize + 10));
        ...
        if (Widgets.ButtonImage(new Rect(5 - 2, 110 + ((statSize + 15) * i) - 2,
                                         statSize + 4, statSize + 4),
                                texLoad.iconUnrest)) {
          Find.WindowStack.Add(new descWindowFC("FactionUnrestDesc".Translate(),
                                                "FactionUnrest".Translate()));
        };
        Widgets.Label(new Rect(50, 110 + ((statSize + 15) * i), 80, statSize),
                      Convert.ToInt32(faction.averageUnrest) + "%");
      }
      if (stats[i] == "prosperity") {
        ...
        if (Widgets.ButtonImage(new Rect(5 - 2, 110 + ((statSize + 15) * i) - 2,
          ...
          Find.WindowStack.Add(new descWindowFC(
              "FactionProsperityDesc", "FactionProsperity".Translate()));
        };
        Widgets.Label(new Rect(50, 110 + ((statSize + 15) * i), 80, statSize),
                      Convert.ToInt32(faction.averageProsperity) + "%");
      }
    }
  }

  private void DrawFactionButtons(
      Rect inRect,
      int buttonSize) // Used to draw a list of buttons from the 'buttons' list
  {
    Text.Anchor = TextAnchor.MiddleCenter;
    Text.Font = GameFont.Small;
    for (int i = 0; i < buttons.Count(); i++) {
      if (Widgets.ButtonText(
              new Rect(140, 110 + ((buttonSize + 5) * i), 170, buttonSize),
              buttons[i])) {
        if (buttons[i] ==
            "FCOverview".Translate()) { // if click trade policy button
        ...
        if (buttons[i] == "Actions".Translate()) {
          ...
          list.Add(new FloatMenuOption("TaxDeliveryMap".Translate(), delegate {
            ...
            list2.Add(new FloatMenuOption("SetMap".Translate(), delegate {
                ...
                if (map.IsPlayerHome == true) {
                  ...
                  settlementList.Add(new FloatMenuOption(map.Parent.LabelCap, delegate {
                    ...
                    Find.LetterStack.ReceiveLetter(
                        "Map Set!",
                        "The tax delivery map has been set to the player colony of " +
                            map.Parent.LabelCap +
                            ".\n All taxes and other goods will be delivered there",
              ...
              if (settlementList.Count == 0) {
                settlementList.Add(
                    new FloatMenuOption("No valid settlements to use.", null));
              }
          ...
          list.Add(new FloatMenuOption("SetCapital".Translate(),
                                       delegate { faction.setCapital(); }));

          list.Add(
              new FloatMenuOption("ActivateResearch".Translate(),
                                  delegate { faction.updateDailyResearch(); }));
          ...
          if (faction.hasPolicy(FCPolicyDefOf.feudal))
            list.Add(
                new FloatMenuOption("FCRequestMercenary".Translate(), delegate {
                  if (faction.traitFeudalBoolCanUseMercenary) {
                    ...
                    PawnKindDef kind =
                        groupMaker.GeneratePawns(group).RandomElement().kindDef;
                    ...
                    pawnkind.apparelAllowHeadgearChance =
                        kind.apparelAllowHeadgearChance;
                    ...
                    Pawn pawn =
                        PawnGenerator.GeneratePawn(new PawnGenerationRequest(
                            pawnkind, Find.FactionManager.OfPlayer,
                            PawnGenerationContext.NonPlayer, -1, false, false,
                            false, false, true, false, 20f, false, true, true,
                            true, false, false, false, false, 0f, null, 1f,
                    ...
                    Find.LetterStack.ReceiveLetter(
                        ...
                        TranslatorFormattedStringExtensions.Translate(
                            "FCMercenaryJoinedText", pawn.NameFullColored),
                        LetterDefOf.PositiveEvent, new LookTargets(pawn));

                  } else {
                    Messages.Message(
                        TranslatorFormattedStringExtensions.Translate(
                            "FCActionMercenaryOnCooldown",
  ...
  private void DrawFactionResourceIcons(
      Rect inRect, int x, int y,
      int resourceSize) // Used to draw a list of resources from the faction
  {
    Text.Font = GameFont.Small;
    Text.Anchor = TextAnchor.MiddleCenter;
    int k;
    ...
    for (int i = 0; i < faction.returnNumberResource(); i++) {
      ...
      if (Widgets.ButtonImage(new Rect(5 + x + (j * (resourceSize + 5)),
                                       y - 5 + ySpacing * k, resourceSize,
                                       resourceSize),
                              faction.returnResourceByInt(i).getIcon())) {
        Find.WindowStack.Add(new descWindowFC(
            "TotalFactionProduction".Translate() + ": " +
                faction.returnResourceByInt(i).name,
            char.ToUpper(faction.returnResourceByInt(i).name[0]) +
                faction.returnResourceByInt(i).name.Substring(1)));
      }
      Widgets.Label(new Rect(5 + x + (j * (resourceSize + 5)),
                             y + resourceSize - 10 + ySpacing * k, resourceSize,
                             resourceSize),
                    faction.returnResourceByInt(i).amount.ToString());
    }
  }
  ...
  private void DrawFactionEconomicStats(Rect inRect) {
    ...
    Widgets.Label(new Rect(195, 300, 115, 20),
                  Convert.ToInt32(faction.profit).ToString() + " " +
                      "Silver".Translate().ToLower());

    Widgets.Label(new Rect(195, 315, 115, 30), "TimeTillTax".Translate() + ":");
    Widgets.Label(new Rect(195, 340, 115, 20),
                  GenDate.ToStringTicksToDays(faction.taxTimeDue -
                                              Find.TickManager.TicksGame));
  }
