...
public class FactionColonies : ModSettings {
  ...
  public static void updateChanges() {
    ...
    if (factionFC.updateVersion < 0.314) {
      ...
      LoadedModManager.GetMod<FactionColoniesMod>()
          ...
          .settlementMaxLevel = 10;

      foreach (SettlementFC settlement in factionFC.settlements) {
        settlement.power = new ResourceFC("power", "Power", 0, settlement, 7);
        settlement.medicine =
            new ResourceFC("medicine", "Medicine", 0, settlement, 8);
        settlement.research =
            new ResourceFC("research", "Research", 0, settlement, 6);
    ...
    if (factionFC.updateVersion < 0.329) {
      foreach (SettlementFC settlement in factionFC.settlements) {
        ...
        foreach (FCPrisoner prisoner in settlement.prisonerList) {
          ...
          HealthUtility.HealNonPermanentInjuriesAndRestoreLegs(
              prisoner.prisoner);
        }
      }
    }
    ...
    if (factionFC.updateVersion < 0.346) {
      factionFC.militaryCustomizationUtil.fireSupportDefs =
    ...
    if (factionFC.updateVersion < 0.349) {
      foreach (SettlementFC settlement in factionFC.settlements) {
        if (settlement.planetName == null) {
          // Log.Message("Reset planet military location for " +
          // settlement.name);
          settlement.militaryLocationPlanet = Find.World.info.name;
          settlement.planetName = Find.World.info.name;
        }
    ...
    if (factionFC.updateVersion < 0.350) {
      ...
      foreach (MercenarySquadFC squad in factionFC.militaryCustomizationUtil
                   .mercenarySquads) {
    ...
    // if ((factionFC.name != "PlayerFaction".Translate() ||
    // factionFC.settlements.Count() > 0) && getPlayerColonyFaction() == null ||
    // broken == true)
    //{
    //	Log.Message("Old save detected - Adjusting factions to fix possible
    //issues. Note: You will see some until the next load."); 	Faction newFaction
    //= createPlayerColonyFaction(); 	foreach (WorldObject obj in
    ...
    //			{
    //				if (obj.Faction == null || obj.Faction.def ==
    //null)
    //				{
    //					//Log.Message(obj.Label);
    //					obj.SetFaction(newFaction);
    //					Log.Message("Reseting Faction of Settlement " + obj.LabelCap +
    //". If this is in error, please report it on the Empire Discord");
    //
    //				}
    //
    //
    ...
    //{
    //	if (faction.Name == factionFC.name && faction.leader != null ||
    //faction.def.defName == "PColonySpacer" || faction.def.defName ==
    //"PColonyTribal" || faction.def.defName == "PColonyIndustrial")
    //	{
    ...
    //			faction.def =
    //DefDatabase<FactionDef>.GetNamed("PColony"); 			factionFC.updateFaction();
    //		}
    //	}
    //}
    ...
    if (factionFC.factionIconPath == null ||
        factionFC.factionIconPath == "FactionIcons/Base") {
    ...
    if (Find.WorldObjects.SettlementAt(factionFC.capitalLocation) == null &&
        factionFC.SoSShipCapital == false) {
      Messages.Message(
      ...
      // if
      // (Find.WorldObjects.SettlementAt(Find.World.GetComponent<FactionFC>().capitalLocation).Map
      // != null)
      //{
    ...
    if (factionFC.taxMap == null) {
      Messages.Message(
          "Your tax map has not been set. Please set it using the faction menu.",
          MessageTypeDefOf.CautionInput);
    }

    if (factionFC.policies.Count() < 2) {
      Find.LetterStack.ReceiveLetter("FCTraits".Translate(),
                                     "FCSelectYourTraits".Translate(),
    ...
    if (factionFC.updateVersion < 0.350) {
      ...
      // str += "\n- Fixed specific animals from the Witcher Mod, Alpha Animals,
      // Genetic Rim animals from being animal tithes";

      // str += "\n- A bit more randomness added to enemy factions attacking. It
      // is now possible for their attacking force to be 2 below or 2 above their
      // standard attack force size. (This is before combat modifiers)"; str +=
      // "\n- Bug Fixes"; str += "\n";
      str +=
      ...
      Find.LetterStack.ReceiveLetter("Empire Mod Update!", str,
                                     LetterDefOf.NewQuest);
    }
  }

  public static void testLogFunction() { Log.Message("Test Successful"); }
  ...
  public static bool canCraftItem(ThingDef thing,
    ...
    if (thing.recipeMaker != null) {
      if (thing.recipeMaker.researchPrerequisites != null) {
        foreach (ResearchProjectDef research in thing.recipeMaker
                     .researchPrerequisites) {
          if (!(Find.ResearchManager.GetProgress(research) >=
                research.baseCost)) {
      ...
      if (thing.recipeMaker.researchPrerequisite != null) {
        if (!(Find.ResearchManager.GetProgress(
    ...
    if (thing.thingSetMakerTags != null &&
        thing.thingSetMakerTags.Contains("SingleUseWeapon") &&
        !includeSingleUse) {
      canCraft = false;
    }
  ...
  public static Faction getPlayerColonyFaction() {
    ...
    return Find.FactionManager.FirstFactionOfDef(
        DefDatabase<FactionDef>.GetNamed("PColony"));
  }
  ...
  public static Settlement createPlayerColonySettlement(int tile,
    ...
    if (worldcomp.settlements.Count() < 1) {
      Find.World.GetComponent<FactionFC>().timeStart =
    ...
    if (createWorldObject) {
      ...
      settlement.Name =
          SettlementNameGenerator.GenerateSettlementName(settlement, null);
    ...
    if (worldcomp.hasPolicy(FCPolicyDefOf.militaristic))
      settlementfc.constructBuilding(
          DefDatabase<BuildingFCDef>.GetNamed("barracks"), 0);
    ...
    if (worldcomp.hasPolicy(FCPolicyDefOf.expansionist) &&
        settlementfc.settlementLevel == 1)
    ...
    Find.LetterStack.ReceiveLetter("FCSettlementFormed".Translate(),
                                   "TheSettlement".Translate() + " " +
                                       settlementfc.name +
                                       "HasBeenFormed".Translate() + "!",
  ...
  [DebugAction("Empire", "Increment Time 5 Days",
  ...
  [DebugAction("Empire", "Increment Time 1 Year",
               allowedGameStates = AllowedGameStates.Playing)]
  ...
  [DebugAction("Empire", "Reset All Military Squad Assignments",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void resetAllMilitarySquads() {
    ...
    for (int i = util.AllMercenaries.Count() - 1; i >= 0; i--) {
      if (util.AllMercenaries[i].squad.hasLord == true) {
        util.AllMercenaries[i].squad.map.lordManager.RemoveLord(
            util.AllMercenaries[i].squad.lord);
      }
  ...
  [DebugAction("Empire", "Make Random Event",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void makeRandomEvent() {
    List<DebugMenuOption> list = new List<DebugMenuOption>();
    foreach (
        FCEventDef evtDef in DefDatabase<FCEventDef>.AllDefsListForReading) {
      if (evtDef.isRandomEvent == true)
        list.Add(new DebugMenuOption(
            evtDef.label, DebugMenuOptionMode.Action,
            delegate() {
              FCEvent evt;
  ...
  [DebugAction("Empire", "Proc MilitaryTimeDue",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void procMilitaryTimeDue() {
    Log.Message("Debug - Proc MilitaryTimeDue");
    Find.World.GetComponent<FactionFC>().militaryTimeDue =
        Find.TickManager.TicksGame + 1;
  }

  [DebugAction("Empire", "Fix Missing Settlements",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void checkForMissingSettlements() {
    ...
    foreach (SettlementFC settlement in factionfc.settlementsOnPlanet) {
      if (Find.WorldObjects.AnySettlementAt(settlement.mapLocation) == false) {
        Settlement worldObj = (Settlement) WorldObjectMaker.MakeWorldObject(
            WorldObjectDefOf.Settlement);
  ...
  [DebugAction("Empire", "Reset Faction Leaders",
               allowedGameStates = AllowedGameStates.Playing)]
  ...
  [DebugAction("Empire", "Attack Player Settlement",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void attackPlayerSettlement() {
    ...
    foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
                 .settlements) {
      list.Add(new DebugMenuOption(
          settlement.name, DebugMenuOptionMode.Action, delegate() {
            Log.Message("Debug - Attack Player Settlement - " +
                        settlement.name);
            Faction enemyFaction = Find.FactionManager.RandomEnemyFaction();
            MilitaryUtilFC.attackPlayerSettlement(
                militaryForce.createMilitaryForceFromFaction(enemyFaction,
                                                             true),
                settlement, enemyFaction);
          }));
    }
    Find.WindowStack.Add(new Dialog_DebugOptionListLister(list));
  }

  [DebugAction("Empire", "Change Settlement Defending Force",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void ChangeAttackPlayerSettlementMilitaryForce() {
    ...
    foreach (FCEvent evt in worldcomp.events) {
      ...
      if (evt.def == FCEventDefOf.settlementBeingAttacked) {
        ...
        list.Add(new DebugMenuOption(
            worldcomp.returnSettlementByLocation(evt.location, evt.planetName)
                .name,
            DebugMenuOptionMode.Action, delegate() {
              // when event is selected, select defending force to replace it
              // with

              List<DebugMenuOption> list2 = new List<DebugMenuOption>();
              foreach (SettlementFC settlement in worldcomp.settlements) {
                if (settlement.isMilitaryValid() == true &&
                    settlement.name != evt.settlementFCDefending.name) {

                  list2.Add(new DebugMenuOption(
                      settlement.name + " - " +
                          settlement.settlementMilitaryLevel +
                          " - Busy: " + settlement.isMilitaryBusySilent(),
                      DebugMenuOptionMode.Action, delegate() {
                        if (settlement.isMilitaryBusy() == false) {
                          Log.Message(
                              "Debug - Change Player Settlement - " +
                              evt.militaryForceDefending.homeSettlement.name +
                              " to " + settlement.name);
                          MilitaryUtilFC.changeDefendingMilitaryForce(
                              evt, settlement);
                        }
                      }));
                }
  ...
  [DebugAction("Empire", "Upgrade Player Settlement",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void UpgradePlayerSettlement() {
    ...
    foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
      ...
      list.Add(new DebugMenuOption(
          settlement.name, DebugMenuOptionMode.Action, delegate() {
            Log.Message("Debug - Upgrade Player Settlement - " +
                        settlement.name);
  ...
  [DebugAction("Empire", "Upgrade Player Settlement x5",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void UpgradePlayerSettlementx5() {
    ...
    foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
      ...
      list.Add(new DebugMenuOption(
          settlement.name, DebugMenuOptionMode.Action, delegate() {
            Log.Message("Debug - Upgrade Player Settlement x5- " +
                        settlement.name);
            settlement.upgradeSettlement();
            settlement.upgradeSettlement();
            settlement.upgradeSettlement();
  ...
  [DebugAction("Empire", "Test Function",
               allowedGameStates = AllowedGameStates.Playing)]
  ...
  [DebugAction("Empire", "De-Level Player Settlement",
               allowedGameStates = AllowedGameStates.Playing)]
  private static void DelevelPlayerSettlement() {
    ...
    foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
      ...
      list.Add(new DebugMenuOption(
          settlement.name, DebugMenuOptionMode.Action, delegate() {
            Log.Message("Debug - Delevel Player Settlement - " +
                        settlement.name);
  ...
  [DebugAction("Empire", "Reset Military Squads Cooldowns",
               allowedGameStates = AllowedGameStates.Playing)]
  ...
  public static void CallinAlliedForces(SettlementFC settlement, bool DropPod) {
    ...
    settlement.militarySquad.updateSquadStats(
        settlement.settlementMilitaryLevel);
    ...
    tool = new DebugTool("Select Deployment Position", delegate() {
      ...
      if (DropPod) {
        ...
        PawnsArrivalModeWorkerUtility.DropInDropPodsNearSpawnCenter(
            parms, settlement.militarySquad.AllEquippedMercenaryPawns);
      } else {
        PawnsArrivalModeWorker_EdgeWalkIn worker =
            new PawnsArrivalModeWorker_EdgeWalkIn();
        worker.TryResolveRaidSpawnCenter(parms);
        worker.Arrive(settlement.militarySquad.AllEquippedMercenaryPawns,
                      parms);
        // Log.Message(settlement.militarySquad.DeployedMercenaries.Count().ToString());

        foreach (Mercenary merc in settlement.militarySquad
                     .DeployedMercenaries) {

          merc.pawn.mindState.forcedGotoPosition = DropPosition;
          JobGiver_ForcedGoto jobGiver_Standby = new JobGiver_ForcedGoto();
          ThinkResult resultStandby = jobGiver_Standby.TryIssueJobPackage(
              merc.pawn, new JobIssueParams());
          bool isValidStandby = resultStandby.IsValid;
          if (isValidStandby) {
            // Log.Message("valid");
            merc.pawn.jobs.StartJob(resultStandby.Job,
                                    JobCondition.InterruptForced);
          }
        }
        foreach (Mercenary merc in settlement.militarySquad
                     .DeployedMercenaryAnimals) {
          merc.pawn.mindState.forcedGotoPosition = DropPosition;
          JobGiver_ForcedGoto jobGiver_Standby = new JobGiver_ForcedGoto();
          ThinkResult resultStandby = jobGiver_Standby.TryIssueJobPackage(
              merc.pawn, new JobIssueParams());
      ...
      Find.LetterStack.ReceiveLetter(
          "Military Deployed",
          "The Military forces of " + settlement.name +
              " have been deployed to " + Find.CurrentMap.Parent.LabelCap,
          LetterDefOf.NeutralEvent,
          new LookTargets(settlement.militarySquad.AllEquippedMercenaryPawns));
      // MilitaryAI.SquadAI(ref settlement.militarySquad);

      DebugTools.curTool = null;
      settlement.sendMilitary(Find.CurrentMap.Index, Find.World.info.name,
                              "Deploy", 1, null);
    });
    DebugTools.curTool = tool;
  ...
  public static void CallinAlliedForces(SettlementFC settlement, bool DropPod,
    ...
    MercenarySquadFC squad =
        factionfc.militaryCustomizationUtil.createMercenarySquad(settlement,
                                                                 true);
    ...
    tool = new DebugTool("Select Deployment Position", delegate() {
      ...
      if (DropPod) {
        parms.spawnCenter = DropPosition;
        PawnsArrivalModeWorkerUtility.DropInDropPodsNearSpawnCenter(
            parms, squad.AllEquippedMercenaryPawns);
      } else {
        PawnsArrivalModeWorker_EdgeWalkIn worker =
            new PawnsArrivalModeWorker_EdgeWalkIn();
        worker.TryResolveRaidSpawnCenter(parms);
      ...
      Find.LetterStack.ReceiveLetter(
          "Military Deployed",
          "The Military forces of " + squad.settlement.name +
              " have been deployed to " + Find.CurrentMap.Parent.LabelCap,
          LetterDefOf.NeutralEvent,
          new LookTargets(squad.AllEquippedMercenaryPawns));
      factionfc.traitMilitaristicTickLastUsedExtraSquad =
          Find.TickManager.TicksGame;
  ...
  public static void FireSupport(SettlementFC settlement,
                                 MilitaryFireSupport support) {
    DebugTool tool = null;
    IntVec3 DropPosition;
    tool = new DebugTool(
        "Select Artillery Position",
        delegate() {
          ...
          if (PaymentUtil.getSilver() > cost) {
            ...
            MilitaryFireSupport fireSupport = new MilitaryFireSupport(
                "fireSupport", map, spawnCenter, projectiles.Count() * 15, 600,
                support.accuracy, projectiles);
            Find.World.GetComponent<FactionFC>()
                .militaryCustomizationUtil.fireSupport.Add(fireSupport);

            Messages.Message(
                support.name +
                    " will be fired upon shortly on the marked position!",
                MessageTypeDefOf.ThreatSmall);
            settlement.artilleryTimer = Find.TickManager.TicksGame + 60000;

          } else {
            Messages.Message(
                "You do not have enough silver to pay for the strike!",
  ...
  //[DebugAction("Empire", "Call In Artillery", allowedGameStates =
  ...
  [DebugAction("Empire", "Call In Allied Forces",
               allowedGameStates = AllowedGameStates.PlayingOnMap)]
  private static void CallInAlliedForcesDebug() {
    CallInAlliedForcesSelect();
  }

  [DebugAction("Empire", "Level Up Faction",
               allowedGameStates = AllowedGameStates.PlayingOnMap)]
  ...
  public static void removePlayerSettlement(SettlementFC settlement) {
    ...
    Messages.Message(TranslatorFormattedStringExtensions.Translate(
                         "SettlementRemoved", settlement.name),
                     MessageTypeDefOf.NegativeEvent);
    ...
    if (Find.World.info.name == settlement.planetName) {
      ...
      faction.deleteSettlementQueue.Add(new SettlementSoS2Info(
          settlement.planetName, settlement.mapLocation));
    }
  ...
  Reset:
    foreach (FCEvent evt in faction.events) {
      ...
      if (evt.def == FCEventDefOf.captureEnemySettlement ||
          evt.def == FCEventDefOf.raidEnemySettlement) {
      ...
      if (evt.def == FCEventDefOf.constructBuilding ||
          evt.def == FCEventDefOf.enactSettlementPolicy ||
          evt.def == FCEventDefOf.upgradeSettlement ||
      ...
      if (evt.def.isRandomEvent == true &&
          evt.settlementTraitLocations.Count() > 0) {
  ...
  public static int CompareFloatMenuOption(FloatMenuOption x,
                                           FloatMenuOption y) {
  ...
  public static int CompareSettlementMilitaryLevel(SettlementFC x,
                                                   SettlementFC y) {
    return y.settlementMilitaryLevel.CompareTo(x.settlementMilitaryLevel);
  }
  public static int CompareSettlementFreeWorkers(SettlementFC x,
                                                 SettlementFC y) {
    return ((y.workersUltraMax - y.getTotalWorkers())
                .CompareTo((x.workersUltraMax - x.getTotalWorkers())));
  }
  ...
  public static int CompareSettlementProsperity(SettlementFC x,
                                                SettlementFC y) {
  ...
  public static int ReturnTicksToArrive(int currentTile, int destinationTile) {
    ...
    if (currentTile == -1 || destinationTile == -1) {
      if (!medievalOnly && def != null &&
          Find.ResearchManager.GetProgress(def) == def.baseCost) {
    ...
    using(WorldPath tempPath = Find.WorldPathFinder.FindPath(
              currentTile, destinationTile, null, null)) {
      if (tempPath == WorldPath.NotFound) {
        ticksToArrive = 600000;
      } else {
        ticksToArrive = CaravanArrivalTimeEstimator.EstimatedTicksToArrive(
            currentTile, destinationTile, tempPath, 0f,
            CaravanTicksPerMoveUtility.GetTicksPerMove(null, null),
  ...
  public static Faction copyPlayerColonyFaction() {
    ...
    if (FactionColonies.checkForMod("kentington.saveourship2")) {
      ...
      Type typ = FactionColonies.returnUnknownTypeFromName(
          "SaveOurShip2.WorldSwitchUtility");
      Type typ2 = FactionColonies.returnUnknownTypeFromName(
          "SaveOurShip2.WorldFactionList");

      var mainclass = Traverse.CreateWithType(typ.ToString());
      var dict = mainclass.Property("PastWorldTracker")
                     .Field("WorldFactions")
                     .GetValue();

      var planetfactiondict = Traverse.Create(dict);
      var unknownclass =
          planetfactiondict.Property("Item", new object[]{Find.World.info.name})
              .GetValue();
      ...
      foreach (Faction other in Find.FactionManager
                   .AllFactionsVisibleInViewOrder) {
        faction.TryMakeInitialRelationsWith(other);
      }
      Find.FactionManager.Add(faction);
    }
  ...
  private static List<float> getAttackPoints() {
    ...
    for (int i = -Convert.ToInt32(plusOrMinusRandomAttackValue * 10);
         i < plusOrMinusRandomAttackValue * 10; i++) {
      list.Add((i / 10));
    }
    return list;
  }
  ...
  public static float randomAttackModifier() {
    float y = (from x in getAttackPoints() select x)
                  .RandomElementByWeight(
                      (float x) => new SimpleCurve{
                          {new CurvePoint(0f, 1f), true},
                          {new CurvePoint(plusOrMinusRandomAttackValue, .1f),
                           true}}.Evaluate(Math.Abs(x) - 2));
    return y;
  }
  ...
  public override void ExposeData() {
    ...
    Scribe_Values.Look(ref disableHostileMilitaryActions,
                       "disableHostileMilitaryActions");
    Scribe_Values.Look(ref disableRandomEvents, "disableRandomEvents");
    Scribe_Values.Look(ref minDaysTillMilitaryAction,
                       "minDaysTillMilitaryAction");
    Scribe_Values.Look(ref maxDaysTillMilitaryAction,
                       "maxDaysTillMilitaryAction");
...
public class FactionColoniesMod : Mod {
  ...
  public override void DoSettingsWindowContents(Rect inRect) {
    ...
    minMaxDaysTillMilitaryAction =
        new IntRange(minDaysTillMilitaryAction, maxDaysTillMilitaryAction);

    settings.minMaxDaysTillMilitaryAction =
        new IntRange(minDaysTillMilitaryAction, maxDaysTillMilitaryAction);

    Listing_Standard listingStandard = new Listing_Standard();
    listingStandard.Begin(inRect);
    listingStandard.Label("Silver amount gained per resource");
    listingStandard.IntEntry(ref settings.silverPerResource,
                             ref silverPerResource);
    ...
    listingStandard.IntEntry(ref settings.productionTitheMod,
                             ref productionTitheMod);
    ...
    listingStandard.IntEntry(ref settings.settlementMaxLevel,
                             ref settlementMaxLevel);
    listingStandard.CheckboxLabeled("MedievalTechOnly".Translate(),
                                    ref settings.medievalTechOnly);
    listingStandard.CheckboxLabeled("Disable Hostile Military Actions",
                                    ref settings.disableHostileMilitaryActions);
    listingStandard.CheckboxLabeled("Disable Random Events",
                                    ref settings.disableRandomEvents);
    listingStandard.CheckboxLabeled(
        "Disable Forced Pausing During Events",
        ref settings.disableForcedPausingDuringEvents);
    listingStandard.Label(
        "Min/Max Days Until Military Action (ex. Settlements being attacked)");
    ...
    if (listingStandard.ButtonText("Reset Settings")) {
      ...
      settings.minMaxDaysTillMilitaryAction =
          blank.minMaxDaysTillMilitaryAction;
      settings.disableRandomEvents = blank.disableRandomEvents;
      settings.disableForcedPausingDuringEvents =
          blank.disableForcedPausingDuringEvents;
    }
  ...
  public override string SettingsCategory() { return "Empire"; }

  public override void WriteSettings() {
    LoadedModManager.GetMod<FactionColoniesMod>()
        .GetSettings<FactionColonies>()
        .timeBetweenTaxes = daysBetweenTaxes * 60000;
    LoadedModManager.GetMod<FactionColoniesMod>()
        .GetSettings<FactionColonies>()
        .minDaysTillMilitaryAction =
        LoadedModManager.GetMod<FactionColoniesMod>()
