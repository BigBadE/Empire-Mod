      ...
      if (__instance.def.generateNewLeaderFromMapMembersOnly) {
        for (int i = 0; i < Find.Maps.Count; i++) {
          ...
          for (int j = 0; j < map.mapPawns.AllPawnsCount; j++) {
            if (map.mapPawns.AllPawns[j] != pawn &&
                !map.mapPawns.AllPawns[j].Destroyed &&
                map.mapPawns.AllPawns[j].FactionOrExtraMiniOrHomeFaction ==
                    __instance) {
        ...
        foreach (ThingDef def in Find.World.GetComponent<FactionFC>()
        ...
        if (list.TryRandomElement(out kind)) {
          PawnGenerationRequest request = new PawnGenerationRequest(
              kind, __instance, PawnGenerationContext.NonPlayer, -1,
              __instance.def.leaderForceGenerateNewPawn, false, false, false,
              true, false, 1f, false, true, true, true, false, false, false,
              false, 0f, null, 1f, null, null, null, null, null, null, null,
          ...
          if (!Find.WorldPawns.Contains(__instance.leader)) {
            Find.WorldPawns.PassToWorld(__instance.leader,
...
class PawnGroupKindWorker_NormalGeneratePawns {
  static bool Prefix(ref PawnGroupKindWorker_Normal __instance,
                     PawnGroupMakerParms parms, PawnGroupMaker groupMaker,
                     List<Pawn> outPawns, bool errorOnZeroResults = true) {
    if (parms.faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      if (!__instance.CanGenerateFrom(parms, groupMaker)) {
        if (errorOnZeroResults) {
          Log.Error(string.Concat(new object[]{
                        "Cannot generate pawns for ", parms.faction, " with ",
                        parms.points,
      ...
      bool allowFood =
          parms.raidStrategy == null || parms.raidStrategy.pawnsCanBringFood ||
          (parms.faction != null && !parms.faction.HostileTo(Faction.OfPlayer));
      Predicate<Pawn> validatorPostGear =
          (parms.raidStrategy != null)
              ? ((Pawn p) => parms.raidStrategy.Worker.CanUsePawn(p, outPawns))
              : (Predicate<Pawn>) null;
      bool flag = false;
      foreach (PawnGenOption pawnGenOption in PawnGroupMakerUtility
                   .ChoosePawnGenOptionsByPoints(parms.points,
                                                 groupMaker.options, parms)) {
        ...
        pawnkind.apparelAllowHeadgearChance =
            pawnGenOption.kind.apparelAllowHeadgearChance;
        pawnkind.techHediffsMoney = pawnGenOption.kind.techHediffsMoney;
        pawnkind.label = pawnGenOption.kind.label;

        Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
            pawnkind, parms.faction, PawnGenerationContext.NonPlayer,
            parms.tile, false, false, false, false, true, true, 1f, false, true,
            allowFood, true, parms.inhabitants, false, false, false, 0f, null,
            1f, null, validatorPostGear, null, null, null, null, null, null,
            null, null, null, null));
...
class PawnGroupKindWorker_TraderGenerateGuards {
  static bool Prefix(ref PawnGroupKindWorker_Trader __instance,
                     PawnGroupMakerParms parms, PawnGroupMaker groupMaker,
                     Pawn trader, List<Thing> wares, List<Pawn> outPawns) {
    ...
    if (parms.faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      foreach (PawnGenOption pawnGenOption in PawnGroupMakerUtility
        ...
        request.RedressValidator =
            ((Pawn x) => x.royalty == null ||
                         !x.royalty.AllTitlesForReading.Any<RoyalTitle>());
...
class PawnGroupKindWorker_TraderGenerateTrader {
  static bool Prefix(ref PawnGroupKindWorker_Trader __instance,
                     ref Pawn __result, PawnGroupMakerParms parms,
                     PawnGroupMaker groupMaker, TraderKindDef traderKind) {
    ...
    if (parms.faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
          list.RandomElement(), parms.faction, PawnGenerationContext.NonPlayer,
          parms.tile, false, false, false, false, true, false, 1f, false, true,
          true, true, parms.inhabitants, false, false, false, 0f, null, 1f,
...
class QuestGen_PawnsQuestGeneratePawn {
  static bool
  Prefix(ref Pawn __result, Quest quest, PawnKindDef kindDef, Faction faction,
         bool allowAddictions = true, IEnumerable<TraitDef> forcedTraits = null,
         float biocodeWeaponChance = 0f, bool mustBeCapableOfViolence = true,
         Pawn extraPawnForExtraRelationChance = null,
         float relationWithExtraPawnChanceFactor = 0f,
         float biocodeApparelChance = 0f, bool ensureNonNumericName = false,
    ...
    if (faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
          list.RandomElement(), faction, PawnGenerationContext.NonPlayer, -1,
          forceGenerateNewPawn, false, false, false, true,
          mustBeCapableOfViolence, 1f, false, true, true, allowAddictions,
          false, false, false, false, biocodeWeaponChance,
          extraPawnForExtraRelationChance, relationWithExtraPawnChanceFactor,
          null, null, forcedTraits, null, null, null, null, null, null, null,
          null, null){BiocodeApparelChance = biocodeApparelChance});
      if (ensureNonNumericName && (pawn.Name == null || pawn.Name.Numerical)) {
        pawn.Name = PawnBioAndNameGenerator.GeneratePawnName(
            pawn, NameStyle.Full, null);
      }
...
class QuestGen_PawnsQuestGeneratePawn2 {
  static bool Prefix(ref Pawn __result, QuestGen_Pawns.GetPawnParms parms,
    ...
    if (faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
          list.RandomElement(), faction, PawnGenerationContext.NonPlayer, -1,
          true, false, false, false, true, false, 1f, false, true, true, true,
          false, false, false, false, 0f, null, 1f, null, null, null, null,
          null, null, null, null, null, null, null));
      Find.WorldPawns.PassToWorld(pawn, PawnDiscardDecideMode.Decide);
      if (pawn.royalty != null &&
          pawn.royalty.AllTitlesForReading.Any<RoyalTitle>()) {
...
class QuestNode_GetPawnGeneratePawn {
  static bool Prefix(ref QuestNode_GetPawn __instance, ref Pawn __result,
                     Slate slate, Faction faction = null) {
...
class RaidStrategyWorkerSpawnThreats {
  static bool Prefix(ref RaidStrategyWorker __instance, ref List<Pawn> __result,
                     IncidentParms parms) {
    ...
    if (parms.faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      if (parms.pawnKind != null) {
        ...
        for (int i = 0; i < parms.pawnCount; i++) {
          ...
          pawnkind.apparelAllowHeadgearChance =
              parms.pawnKind.apparelAllowHeadgearChance;
          pawnkind.techHediffsMoney = parms.pawnKind.techHediffsMoney;
          pawnkind.label = parms.pawnKind.label;
          Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
              pawnkind, faction, context, tile, forceGenerateNewPawn, newborn,
              allowDead, allowDowned, canGeneratePawnRelations,
              mustBeCapableOfViolence, colonistRelationChanceFactor,
              forceAddFreeWarmLayerIfNeeded, allowGay,
              __instance.def.pawnsCanBringFood, true, false, false, false,
              false, biocodeWeaponsChance, null, 1f, null, null, null, null,
              null, null, null, null, null, null, null,
              null){BiocodeApparelChance = 1f});
          if (pawn != null) {
            list.Add(pawn);
          }
        }
...
class ThingSetMaker_RefugeePodGenerate {
  static bool Prefix(ref ThingSetMaker_RefugeePod __instance,
                     ThingSetMakerParams parms, List<Thing> outThings) {
    ...
    if (faction == FactionColonies.getPlayerColonyFaction()) {
      ...
      Pawn pawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
          ...
          false, 20f, false, true, true, true, false, false, false, false, 0f,
          null, 1f, null, null, null, null, null, null, null, null, null, null,
          null, null));
      outThings.Add(pawn);
