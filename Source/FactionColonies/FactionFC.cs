...
public class FactionFC : WorldComponent
{
    ...
    public List<FCPolicy> factionTraits = new List<FCPolicy>() {
        new FCPolicy(FCPolicyDefOf.empty), new FCPolicy(FCPolicyDefOf.empty), new FCPolicy(FCPolicyDefOf.empty), new FCPolicy(FCPolicyDefOf.empty), new FCPolicy(FCPolicyDefOf.empty)
    };
            ...
            } else
            {
                ...
                if (fireSupport.fireSupportType == "fireSupport")
                {
                    if ((fireSupport.timeRunning % 15) == 0 && fireSupport.timeRunning >= fireSupport.startupTime) //15
                    {
                        ...
                        select x).RandomElementByWeight((IntVec3 x) => new SimpleCurve { { new CurvePoint(0f, 1f), true }, { new CurvePoint(fireSupport.accuracy, 0.1f), true }
