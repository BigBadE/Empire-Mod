...
public class PaymentUtil {
  ...
  public static(List<BillFC>, List<BillFC>)
      returnBillTypes(List<BillFC> bills) {
  ...
  public static void autoresolveBills(List<BillFC> bills) {
    ...
    (List<BillFC> negativeBills, List<BillFC> positiveBills) =
        returnBillTypes(bills);
  ...
  // if make it to the end of the positive bills, goto function to check if
  // there's enough silver. If so, pay, if not, return the function
  Reset:
    foreach (BillFC negativeBill in negativeBills) {
    ResetInner:
      foreach (BillFC positiveBill in positiveBills) {
        float result =
            positiveBill.taxes.silverAmount + negativeBill.taxes.silverAmount;
        if (result == 0) {
          // Log.Message("Equal");
  ...
  ResetOuter:
    ...
    Messages.Message(TranslatorFormattedStringExtensions.Translate(
                         "NumberTaxesHasBeenSolved", resolvedBills),
  ...
  public static void placeThing(Thing thing) {
    ...
    if (taxMap == null) {
      if (Find.WorldObjects.SettlementAt(
              Find.World.GetComponent<FactionFC>().capitalLocation) == null) {
  ...
  Paid:
    while (amount > 0) {
      foreach (Map map in Find.Maps) {
        if (map.IsPlayerHome) {
        List:
          foreach (Thing item in map.listerThings
                       .AllThings) // loop through each item in cell
          {
            if (item.def == ThingDefOf.Silver &&
                item.IsInAnyStorage() == true) { // if silver, add to count
              if (amount - item.stackCount <
                  0) // if removing silver would pay too much
              {
                int overdraw = -1 * (amount - item.stackCount);
                amount -= (item.stackCount - overdraw);
                item.SplitOff(item.stackCount - overdraw)
                    .Destroy(DestroyMode.Vanish);
                goto Paid;
              } else if (amount - item.stackCount >
                         0) // if removing silver would leave some
              {
                amount -= item.stackCount;
                item.Destroy(DestroyMode.Vanish);
                goto List;
              } else if (amount - item.stackCount ==
                         0) // if removing silver will make amount = 0
              {
                amount -= item.stackCount;
                item.Destroy(DestroyMode.Vanish);
                goto Paid;
  ...
  public static bool checkForTaxSpot(Map map, out IntVec3 dropSpot) {
    ...
    {
      foreach (Thing item in map.thingGrid.ThingsAt(
                   cell)) // loop through each item in cell
      {
        if (item.def.defName == "TaxSpot") { // if silver, add to count
          dropSpot = cell;
      ...
      if (map.IsPlayerHome) {
        foreach (Thing thing in map.listerThings.AllThings) {
          if (thing.def == ThingDefOf.Silver &&
              thing.IsInAnyStorage() == true) {
            // Log.Message(thing.LabelCap + " inStorage: " +
  ...
  public static ThingSetMakerParams returnThingSetMakerParams(int baseValue,
                                                              int rangeMod) {
    ThingSetMakerParams parms = new ThingSetMakerParams();
    parms.techLevel = Find.FactionManager.OfPlayer.def.techLevel;
    parms.totalMarketValueRange =
        new FloatRange(baseValue - rangeMod, baseValue + rangeMod);
    return parms;
  }
  ...
  public static List<Thing> generateRaidLoot(int lootLevel,
    ...
    param.totalMarketValueRange =
        new FloatRange((500 + (lootLevel * 200)) * trait_LootMulitplier,
    ...
    // set disallow
    param.filter.SetAllow(DefDatabase<ThingDef>.GetNamedSilentFail("Teachmat"),
                          false);
  ...
  public static void resetThingFilter(in FactionFC faction, int resourceID) {
    ...
    case 3: // animals
      ...
      List<PawnKindDef> allAnimalDefs =
    ...
    case 5: // Mining
      ...
      // Android shit?
      filter.SetAllow(DefDatabase<ThingDef>.GetNamedSilentFail("Teachmat"),
                      false);
      // Remove RimBees Beeswax
      filter.SetAllow(
          DefDatabase<StuffCategoryDef>.GetNamedSilentFail("RB_Waxy"), false);
      // Remove Alpha Animals skysteel
      filter.SetAllow(DefDatabase<ThingDef>.GetNamedSilentFail("AA_SkySteel"),
                      false);

      ThingDef rawMagicyte =
          DefDatabase<ThingDef>.GetNamedSilentFail("RawMagicyte");
      if (rawMagicyte != null)
    ...
    case 8: // Medicine/Bionics
      ...
      filter.SetAllow(
          DefDatabase<ThingCategoryDef>.GetNamedSilentFail("BodyPartsNatural"),
          true);
      switch (faction.techLevel) {
      case TechLevel.Archotech:
      case TechLevel.Ultra:
        filter.SetAllow(
            DefDatabase<ThingCategoryDef>.GetNamedSilentFail("BodyPartsUltra"),
            true);
        filter.SetAllow(
            DefDatabase<ThingCategoryDef>.GetNamedSilentFail("BodyPartsBionic"),
            true);
        filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                            "BodyPartsProsthetic"),
                        true);
        filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                            "BodyPartsNatural"),
        ...
        if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                "BionicProstheses") != null)
          filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                              "BionicProstheses"),
                          true);
        if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                "AdvancedProstheses") != null)
          filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                              "AdvancedProstheses"),
                          true);
        if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                "SyntheticOrgans") != null)
          filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                              "SyntheticOrgans"),
                          true);
        if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail("Neurotrainers") !=
            null)
          filter.SetAllow(
              DefDatabase<ThingCategoryDef>.GetNamedSilentFail("Neurotrainers"),
              true);
        break;
  ...
  public static void resetThingFilter(in SettlementFC settlement,
                                      int resourceID) {
    ...
    case 1: // weapons
      filter.SetAllow(ThingCategoryDefOf.Weapons, true);
      filter.SetAllow(
          DefDatabase<ThingCategoryDef>.GetNamedSilentFail("MortarShells"),
          true);
      if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail("Ammo") != null)
        filter.SetAllow(
            DefDatabase<ThingCategoryDef>.GetNamedSilentFail("Ammo"), true);
      break;
    case 2: // apparel
      filter.SetAllow(ThingCategoryDefOf.Apparel, true);
      ...
      if (FactionColonies.returnIsResearched(
              DefDatabase<ResearchProjectDef>.GetNamedSilentFail(
                  "Devilstrand"))) {
        filter.SetAllow(
            DefDatabase<ThingDef>.GetNamedSilentFail("DevilstrandCloth"), true);
      }
      break;
    case 3: // animals
      List<PawnKindDef> things = new List<PawnKindDef>();
  ...
  public static List<ThingDef> debugGenerateTithe(int resourceID) {
    ...
    case 5: // Mining
      ...
      param.filter.SetAllow(StuffCategoryDefOf.Stony, true);
      param.filter.SetAllow(ThingDefOf.Silver, false);
      // Android shit?
      param.filter.SetAllow(
          DefDatabase<ThingDef>.GetNamedSilentFail("Teachmat"), false);
      // Remove RimBees Beeswax
      param.filter.SetAllow(
          DefDatabase<StuffCategoryDef>.GetNamedSilentFail("RB_Waxy"), false);
      // Remove Alpha Animals skysteel
      param.filter.SetAllow(
          DefDatabase<ThingDef>.GetNamedSilentFail("AA_SkySteel"), false);
      ...
      ThingDef rawMagicyte =
          DefDatabase<ThingDef>.GetNamedSilentFail("RawMagicyte");
      if (rawMagicyte != null)
        param.filter.SetAllow(rawMagicyte, true);
    ...
    case 8: // Medicine/Bionics
      ...
      case TechLevel.Ultra:
        ...
        param.filter.SetAllow(
            DefDatabase<ThingCategoryDef>.GetNamedSilentFail("BodyPartsBionic"),
            true);
        param.filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                                  "BodyPartsProsthetic"),
                              true);
        param.filter.SetAllow(DefDatabase<ThingCategoryDef>.GetNamedSilentFail(
                                  "BodyPartsNatural"),
  ...
  public static List<Thing> generateTithe(double valueBase, double valueDiff,
                                          int multiplier, int resourceID,
                                          double traitValueMod,
    ...
    param.totalMarketValueRange = new FloatRange(
        (float)((valueBase) - ((valueDiff + traitValueMod))),
  ...
  public static List<Thing> generateThing(double valueBase,
                                          string resourceOfThing) {
  regen:
    List<Thing> things = new List<Thing>();
    ThingSetMaker thingSetMaker = new ThingSetMaker_MarketValue();
    ThingSetMakerParams param = new ThingSetMakerParams();
    param.totalMarketValueRange =
        new FloatRange((float)(valueBase - 300), (float)(valueBase + 300));
    param.filter = new ThingFilter();
    param.techLevel = FactionColonies.getPlayerColonyFaction().def.techLevel;
    ...
    case "weapons": // weapons
      ...
      param.totalMarketValueRange = new FloatRange(
          (float)(valueBase - valueBase * .5), (float)(valueBase * 2));
      param.countRange = new IntRange(1, 1);
      break;
    ...
    case "armor": // armor
      param.qualityGenerator = QualityGenerator.Gift;
      param.filter.SetAllow(
          DefDatabase<ThingCategoryDef>.GetNamed("ApparelArmor"), true);
      param.filter.SetAllow(ThingCategoryDefOf.Apparel, true);
      param.countRange = new IntRange(1, 1);
      param.totalMarketValueRange = new FloatRange(
          (float)(valueBase - valueBase * .5), (float)(valueBase * 2));
    ...
    case "animals": // animals
      ...
      param.techLevel = TechLevel.Undefined;
      param.totalMarketValueRange = new FloatRange(
          (float)(valueBase - valueBase * .5), (float)(valueBase * 1.5));
      // param.countRange = new IntRange(1,4);
      break;
    ...
    default: // log error
      Log.Message(
          "This is an error. Report this to the dev. generateThing - nonexistent case");
    ...
    // thingSetMaker.root
    things = thingSetMaker.Generate(param);
    if (PaymentUtil.returnValueOfTithe(things) <
        param.totalMarketValueRange.Value.min) {
      goto regen;
    }
  ...
  public static double returnValueOfTithe(List<Thing> things) {
    double totalValue = 0;
    foreach (Thing thing in things) {
      // Log.Message(thing.def + " #" + thing.stackCount + " $" +
      // thing.stackCount * thing.MarketValue);
...
public class ThingSetMaker_Animals : ThingSetMaker {
  protected override void Generate(ThingSetMakerParams parms,
    ...
    int value = 0;
    foreach (PawnKindDef def in allAnimalDefs) {
      if (parms.filter.Allows(def.race) && def.race.race.Animal &&
          def.RaceProps.IsFlesh && def.race.BaseMarketValue != 0 &&
          def.race.tradeTags != null &&
          !def.race.tradeTags.Contains("AnimalMonster") &&
          !def.race.tradeTags.Contains("AnimalGenetic") &&
          !def.race.tradeTags.Contains("AnimalAlpha")) {
    ...
    int attempts = 0;
    // Log.Message("Min: " + parms.totalMarketValueRange.Value.min + " = " +
    // parms.totalMarketValueRange.Value.max + " max");

    while (parms.totalMarketValueRange.Value.min > value &&
           value < parms.totalMarketValueRange.Value.max) {
    Regen:
      ...
      PawnGenerationRequest request = new PawnGenerationRequest(
          things.RandomElement<PawnKindDef>(), Find.FactionManager.OfPlayer,
          PawnGenerationContext.NonPlayer, -1, false, false, false, false, true,
          false, 1f, false, true, true, false, false, false, false, true, 0,
          null, 1, null, null, null, null);
      Pawn pawn = PawnGenerator.GeneratePawn(request);
      // Log.Message("Pawn generate: " + pawn.LabelCap + " value: " +
      // pawn.MarketValue);
      ...
      // Log.Message(pawn.Name + "   " + pawn.MarketValue + "MarketValue: max
      // value" + parms.totalMarketValueRange.Value.max + ", min val: " +
      // parms.totalMarketValueRange.Value.min);
  ...
  protected override IEnumerable<ThingDef>
  AllGeneratableThingsDebugSub(ThingSetMakerParams parms) {
...
public class ThingSetMaker_Animal : ThingSetMaker {
  ...
  regen:
    PawnGenerationRequest request = new PawnGenerationRequest(
        things.RandomElement<PawnKindDef>(), Find.FactionManager.OfPlayer,
        PawnGenerationContext.NonPlayer, -1, false, false, false, false, true,
        false, 1f, false, true, true, false, false, false, false, true, 0, null,
