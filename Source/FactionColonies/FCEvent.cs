...
public static class FCEventMaker {
  public static void calculateSuccess(FCOptionDef option, FCEvent parentEvent) {
    ...
    if (roll <= baseChance) { // if success
      if (option.parentEvent.settlementsCarryOver == true) {
        tempEvent = FCEventMaker.MakeRandomEvent(
            option.successEvent, parentEvent.settlementTraitLocations);
      } else {
      ...
      if (option.parentEvent.settlementsCarryOver == true) {
        tempEvent = FCEventMaker.MakeRandomEvent(
            option.failEvent, parentEvent.settlementTraitLocations);
      } else {
  ...
  public static bool isValidRandomEvent(FCEventDef cEvent) {
    ...
    if (cEvent.isRandomEvent == true &&
        Find.World.PlayerWealthForStoryteller >= cEvent.requiredWealth) {
      // If meets happiness requirement
      if (cEvent.minimumHappiness <= tmp.averageHappiness &&
          tmp.averageHappiness <= cEvent.maximumHappiness) {
        // If meets loyalty requirement
        if (cEvent.minimumLoyalty <= tmp.averageLoyalty &&
            tmp.averageLoyalty <= cEvent.maximumLoyalty) {
          // If meets unrest requirement
          if (cEvent.minimumUnrest <= tmp.averageUnrest &&
              tmp.averageUnrest <= cEvent.maximumUnrest) {
            ...
            if (cEvent.minimumProsperity <= tmp.averageProsperity &&
              ...
              if (cEvent.rangeSettlementsAffected.min == 0 &&
                ...
                // if doesn't require resource or if required resource has more
                // than 1 production
                if (cEvent.requiredResource ==
                        null ? Find.World.GetComponent<FactionFC>()
                                .returnResource(cEvent.requiredResource)
                                .assignedWorkers > 0
                    : true || (cEvent.requiredResource == "research" &&
                               traitUtilsFC.returnResearchAmount() > 0)) {
                  // if event is not incompatible with any currently-running
                  // events
                  foreach (FCEvent evt in Find.World.GetComponent<FactionFC>()
                               .events) {
                    if (evt.def != null) {
                      if (cEvent == evt.def) { // if there's already the same
                                               // event running
                        return false;
                      }
                      foreach (FCEventDef inEvt in evt.def.incompatibleEvents) {
                        if (cEvent == inEvt) { // if not compatible
                          return false;
                        }
                      }
                    }
  ...
  public static FCEventDef returnRandomEvent() {
    ...
    foreach (
        FCEventDef eventDef in DefDatabase<FCEventDef>.AllDefsListForReading) {
  ...
  public static FCEvent MakeEvent(FCEventDef def) {
    ...
    FCEvent tempEvent = new FCEvent(true);
    tempEvent.def = def;
    tempEvent.timeTillTrigger =
        def.timeTillTrigger + Find.TickManager.TicksGame;
    return tempEvent;
  }

  public static FCEvent
  MakeRandomEvent(FCEventDef def, List<SettlementFC> SettlementTraitLocations) {
    FCEvent tempEvent = new FCEvent(true);
    tempEvent.def = def;
    tempEvent.timeTillTrigger =
        def.timeTillTrigger + Find.TickManager.TicksGame;
    ...
    if (tempEvent.def.rangeSettlementsAffected.max != 0) {
      ...
      // if random number of settlements more than total settlements, reset
      // number settlements.
      if (numSettlements >
          Find.World.GetComponent<FactionFC>().settlements.Count()) {
        numSettlements =
      ...
      if (SettlementTraitLocations == null ||
          SettlementTraitLocations.Count == 0) {
        foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
                     .settlements.InRandomOrder()) {
          if (tempEvent.def.requiredResource !=
              "") { // if there is a required resource
            if (settlement.returnResource(tempEvent.def.requiredResource)
                    .assignedWorkers >
                0) { // if have someone working on that resource
              tmp.Add(settlement);
            } else {
              // don't add
            }
    ...
    if (tempEvent.def.options.Count > 0 &&
        tempEvent.def.activateAtStart == true) {
  ...
  public static void ProcessEvents(in List<FCEvent> events) {
    ...
    for (int i = 0; i < events.Count(); i++) {
      ...
      if (events[i].timeTillTrigger <=
          Find.TickManager.TicksGame) // if due time past game time, do stuff
      {                               // Make custom event functions here
        ...
        if (temp.def.defName == "settleNewColony") { // Settle new colony event
          // Log.Message(events[i].def.defName + " event triggered" +
          // Find.TickManager.TicksGame)
          faction.addExperienceToFactionLevel(10f);
          if (Find.World.info.name == temp.planetName) {
            Settlement settlement =
                FactionColonies.createPlayerColonySettlement(
                    temp.location, true, temp.planetName);
          } else {
            Settlement settlement =
                FactionColonies.createPlayerColonySettlement(
                    temp.location, false, temp.planetName);
            faction.createSettlementQueue.Add(
                new SettlementSoS2Info(temp.planetName, temp.location));
          }
          faction.settlementCaravansList.Remove(temp.location.ToString());
        } else if (temp.def.defName == "taxColony") {
          Messages.Message(
              "TaxesFrom".Translate() + " " +
        ...
        } else if (temp.def.defName == "constructBuilding") { // Create building
          faction
              .settlements [faction.returnSettlementFCIDByLocation(
                  temp.source, temp.planetName)]
          ...
          Messages.Message(
              temp.building.label + " " + "HasBeenConstructedAt".Translate() +
                  ...
                  "!",
              MessageTypeDefOf.PositiveEvent);
        } else if (temp.def.defName == "upgradeSettlement") {
          if (faction.returnSettlementByLocation(temp.location,
                                                 temp.planetName) !=
              null) { // if settlement is not null
            SettlementFC settlement = faction.returnSettlementByLocation(
                temp.location, temp.planetName);
        ...
        } else if (temp.def.defName == "captureEnemySettlement" ||
                   temp.def.defName == "raidEnemySettlement" ||
                   temp.def.defName == "enslaveEnemySettlement") {
          // Process military event
          faction.returnSettlementByLocation(temp.location, temp.planetName)
              .processMilitaryEvent();

        } else if (temp.def.defName == "cooldownMilitary") {
          // Process military event
        ...
        if (temp.def.defName == "settlementBeingAttacked") {
          ...
          int winner = simulateBattleFC.FightBattle(
              temp.militaryForceAttacking, temp.militaryForceDefending);
          if (winner == 1) {
            faction.addExperienceToFactionLevel(5f);
            // if winner is player
            Find.LetterStack.ReceiveLetter(
                "DefenseSuccessful".Translate(),
                TranslatorFormattedStringExtensions.Translate(
                    "DefenseSuccessfulFull", temp.settlementFCDefending.name,
                    temp.militaryForceAttackingFaction.Name),
                LetterDefOf.PositiveEvent,
                new LookTargets(Find.WorldObjects.SettlementAt(
                    temp.settlementFCDefending.mapLocation)));
          } else {
            // get multipliers
            double happinessLostMultiplier =
                (traitUtilsFC.cycleTraits(
                     new double(), "happinessLostMultiplier",
                     temp.settlementFCDefending.traits, "multiply") *
                 traitUtilsFC.cycleTraits(
                     new double(), "happinessLostMultiplier",
            ...
            double loyaltyLostMultiplier =
                (traitUtilsFC.cycleTraits(new double(), "loyaltyLostMultiplier",
                                          temp.settlementFCDefending.traits,
                                          "multiply") *
                 traitUtilsFC.cycleTraits(
                     new double(), "loyaltyLostMultiplier",
                     Find.World.GetComponent<FactionFC>().traits, "multiply"));
            ...
            temp.settlementFCDefending.prosperity -=
                (20 * trait_Prosperity_Multiplier);
            temp.settlementFCDefending.happiness -=
                (25 * happinessLostMultiplier);
            temp.settlementFCDefending.loyalty -=
                (15 * loyaltyLostMultiplier * trait_Muliplier);

            string str = TranslatorFormattedStringExtensions.Translate(
                "DefenseFailureFull", temp.settlementFCDefending.name,
                temp.militaryForceAttackingFaction.Name);

            for (int k = 0; k < 4; k++) {
              int num = new IntRange(0, 10).RandomInRange;
              if (num >= 7 &&
                  temp.settlementFCDefending.buildings[k].defName != "Empty" &&
                  temp.settlementFCDefending.buildings[k].defName !=
                      "Construction" &&
                  trait_CanDestroyBuildings) {
                str +=
                    "\n" + TranslatorFormattedStringExtensions.Translate(
                               "BulidingDestroyedInRaid",
                               temp.settlementFCDefending.buildings[k].label);
                temp.settlementFCDefending.deconstructBuilding(k);
              }
            }

            // level remover checker
            if (temp.settlementFCDefending.settlementLevel > 1 &&
                trait_CanDestroyBuildings) {
              int num = new IntRange(0, 10).RandomInRange;
              if (num >= 7) {
            ...
            Find.LetterStack.ReceiveLetter(
                "DefenseFailure".Translate(), str, LetterDefOf.Death,
                new LookTargets(Find.WorldObjects.SettlementAt(
                    temp.settlementFCDefending.mapLocation)));
          }

          if (temp.militaryForceDefending.homeSettlement !=
              temp.settlementFCDefending) {
            // if not the home settlement defending
            if (temp.militaryForceDefending.forceRemaining >= 7) {
              Find.LetterStack.ReceiveLetter(
                  "OverwhelmingVictory".Translate(),
                  "OverwhelmingVictoryDesc".Translate(),
                  LetterDefOf.PositiveEvent);
            ...
            // log
            // Log.Message("Military force is being returned from settlement " +
            // temp.militaryForceDefending.homeSettlement.name);
          } else {
            // if home settlement
        ...
        } else // if undefined event
        {
          ...
          // Log.Message(temp.def.label + " " + temp.def.randomThingValue + "
          // value:thing " + temp.def.randomThingType);
          if (temp.def.randomThingValue > 0 && temp.def.randomThingType != "") {
            List<Thing> list = PaymentUtil.generateThing(
                temp.def.randomThingValue, temp.def.randomThingType);

            string str;
            str = TranslatorFormattedStringExtensions.Translate(
                "GoodsReceivedFollowing", temp.def.label);
            foreach (Thing thing in list) {
              str = str + "\n" + thing.LabelCap;
            }
            Find.LetterStack.ReceiveLetter("GoodsReceived".Translate(), str,
                                           LetterDefOf.PositiveEvent);
        ...
        // location;
        if (temp.settlementTraitLocations.Count() >
            0) // if has specific locations
        {
        ...
        // if have options
        if (temp.def != null && temp.def.options.Count > 0 &&
            temp.def.activateAtStart == false) {
        ...
        if (temp.def.eventFollows == true) {
          ...
          if (temp.def.splitEventFollows == true) // if a split event
          {
            ...
            int roll = Rand.Range(1, 100);
            if (temp.def.settlementsCarryOver == true) { // if settlements carry
              if (roll <= baseChance) {                  // first event
                tempEvent = FCEventMaker.MakeRandomEvent(
                    temp.def.followingEvent, temp.settlementTraitLocations);
              } else { // if second event
                tempEvent = FCEventMaker.MakeRandomEvent(
                    temp.def.followingEvent2, temp.settlementTraitLocations);
              }
            } else {
              if (roll <= baseChance) { // first event
                tempEvent =
                    FCEventMaker.MakeRandomEvent(temp.def.followingEvent, null);
              } else { // if second event
                tempEvent = FCEventMaker.MakeRandomEvent(
                    temp.def.followingEvent2, null);
              }
            }
          } else {
            if (temp.def.settlementsCarryOver == true) { // if settlements carry
  ...
  public static void createTaxEvent(BillFC bill) {
    ...
    if (bill.settlement != null &&
        faction.settlements.Contains(bill.settlement)) {
      ...
      tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
          "TaxesFromSettlementAreBeingDelivered", bill.settlement.name);
      tmp.planetName = bill.settlement.planetName;
    } else {
      tmp.source = -1;
      tmp.planetName = Find.World.info.name;
      tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
          "TaxesFromSettlementAreBeingDelivered", "Null");
    }
    ...
    if (tmp.goods.Count >
        0) // if any silver or tithe in bill create event. else, well, don't
    {
...
public class FCEvent : IExposable, ILoadReferenceable {
  ...
  public void ExposeData() {
    ...
    Scribe_Collections.Look<SettlementFC>(ref settlementTraitLocations,
    ...
    Scribe_Values.Look<IntRange>(ref rangeSettlementsAffected,
                                 "rangeSettlementsAffected");
    ...
    Scribe_Collections.Look<FCEventDef>(ref incompatibleEvents,
                                        "incompatibleEvents", LookMode.Def);
    ...
    Scribe_Collections.Look<string>(ref applicableBiomes, "applicableBiomes",
                                    LookMode.Value);
    ...
    Scribe_Values.Look<bool>(ref passEventToClassMethodToRun,
                             "passEventToClassMethodToRun");

    // Military stuff
    Scribe_Deep.Look<militaryForce>(ref militaryForceAttacking,
                                    "militaryForceAttacking");
    Scribe_References.Look<Faction>(ref militaryForceAttackingFaction,
                                    "militaryForceAttackingFaction");
    Scribe_Deep.Look<militaryForce>(ref militaryForceDefending,
                                    "militaryForceDefending");
    Scribe_References.Look<Faction>(ref militaryForceDefendingFaction,
                                    "militaryForceDefendingFaction");
    Scribe_References.Look<SettlementFC>(ref settlementFCDefending,
                                         "SettlementFCDefending");
  ...
  public string GetUniqueLoadID() { return "FCEvent_" + this.loadID; }

  public void runAction() {
    if (!this.classToRun.NullOrEmpty() &&
        !this.classMethodToRun.NullOrEmpty()) {
      Type typ = null;
