...
public class FCRoadBuilder : IExposable {
  ...
  public void ExposeData() {
    ...
    Scribe_Collections.Look<FCRoadBuilderQueue>(
        ref roadQueues, "roadQueues", LookMode.Deep,
        new object[]{Find.World.info.name, this.roadDef});
  }
  ...
  public void RoadTick() {
    ...
    foreach (FCRoadBuilderQueue queue in roadQueues.Where(
                 x => x.planetName == Find.World.info.name)) {
  ...
  public bool isNewRoadBetter(RoadDef old, RoadDef newroad) {
    ...
    if (newroad == RoadDefOf.AncientAsphaltRoad &&
        old != RoadDefOf.AncientAsphaltHighway) {
      return true;
    }
    if (newroad == RoadDefOf.DirtRoad &&
        old == DefDatabase<RoadDef>.GetNamed("DirtPath")) {
      return true;
    }
    return false;
  }
  public void createNextSegment(FCRoadBuilderQueue queue) {
    ...
    foreach (WorldPath path in queue.ToBuild) {
      ...
      num += 1;
      List<int> nodeList = (List<int>)(
          typeof(WorldPath)
              .GetField("nodes", System.Reflection.BindingFlags.NonPublic |
                                     System.Reflection.BindingFlags.Instance)
              .GetValue(path));
      typeof(WorldPath)
          .GetField("curNodeIndex", System.Reflection.BindingFlags.NonPublic |
                                        System.Reflection.BindingFlags.Instance)
          .SetValue(path, nodeList.Count - 1,
      ...
      for (int i = 0; i < nodes - 1; i++) {
        ...
        if (tilesConstructed.Contains(prevtile)) {
          // Log.Message(num + " tried to make a road from an already
          // constructed tile");
          goto Next;
        }
        ...
        if (def != null && isNewRoadBetter(def, roadDef)) {
          grid.tiles[prevtile].potentialRoads.RemoveAll(
              (Tile.RoadLink rl) => rl.neighbor == tile);
          grid.tiles[tile].potentialRoads.RemoveAll(
              (Tile.RoadLink rl) => rl.neighbor == prevtile);

          grid.tiles[prevtile].potentialRoads.Add(
              new Tile.RoadLink{neighbor = tile, road = roadDef});
          grid.tiles[tile].potentialRoads.Add(
              new Tile.RoadLink{neighbor = prevtile, road = roadDef});
  ...
  public void calculateRoadPath(FCRoadBuilderQueue queue) {
    ...
    foreach (SettlementFC settlement in faction.settlements) {
      if (settlement.planetName != null &&
          settlement.planetName == queue.planetName) {
    ...
    foreach (Settlement settlement in Find.World.worldObjects.Settlements) {
      ...
      Log.Message(
          "Empire - RoadBuilderQueue - Add allied settlement locations");

      if (faction.hasTrait(FCPolicyDefOf.roadBuilders)) {
        if (settlement.Faction != null &&
            settlement.Faction != Find.FactionManager.OfPlayer &&
            settlement.Faction.PlayerRelationKind != null &&
            settlement.Faction.PlayerRelationKind == FactionRelationKind.Ally &&
            settlement.Tile != null) {
          settlementLocations.Add(settlement.Tile);
        }
      }
    }
    Log.Message("Empire - RoadBuilderQueue - Pair with every other settlement");
    // from each settlement location, pair up with every other settlement
    // location
    for (int i = settlementLocations.Count() - 1; i > 0; i--) {
      for (int k = 0; k < (settlementLocations.Count() - 1); k++) {
        ...
        WorldPath path = Find.WorldPathFinder.FindPath(
            settlementLocations[i], settlementLocations[k], null, null);
    ...
    // Log.Message(queue.ToBuild.Count().ToString() + " number of road paths
    // calculated");
  }
  ...
  public bool testPath(WorldPath path) {
    ...
    typeof(WorldPath)
        .GetField("curNodeIndex", System.Reflection.BindingFlags.NonPublic |
                                      System.Reflection.BindingFlags.Instance)
        .SetValue(testpath, nodeList.Count - 1,
                  System.Reflection.BindingFlags.NonPublic |
                      System.Reflection.BindingFlags.Instance,
                  null, null);
  ...
  public void updateRoadTech() {
    ...
    if (Find.ResearchManager.GetProgress(
            DefDatabase<ResearchProjectDef>.GetNamed("FCRoadBuildingHighway",
                                                     false)) ==
        DefDatabase<ResearchProjectDef>.GetNamed("FCRoadBuildingHighway", false).baseCost) {
      def = RoadDefOf.AncientAsphaltHighway;
      // Log.Message("Empire roadDef updated to " + def.ToString());

    } else if (
        Find.ResearchManager.GetProgress(
            DefDatabase<ResearchProjectDef>.GetNamed("FCRoadBuildingRoad",
                                                     false)) ==
        DefDatabase<ResearchProjectDef>.GetNamed("FCRoadBuildingRoad", false).baseCost) {
