...
public class SettlementFC : IExposable, ILoadReferenceable {
  ...
  public string GetUniqueLoadID() { return "SettlementFC_" + this.loadID; }
  public SettlementFC(string name, int location) {
    ...
    this.workers = 0;
    this.workersMax =
        this.settlementLevel * 3 + returnMaxWorkersFromPrisoners();
    this.workersUltraMax = workersMax + 5 + returnOverMaxWorkersFromPrisoners();

    // Log.Message(Find.WorldGrid.tiles[location].biome.ToString());   <=
    // Returns biome
    // biome info
    ...
    for (int i = 0;
         i < Find.World.GetComponent<FactionFC>().returnNumberResource(); i++) {
  ...
  public int numberBuildings {
    get { return 3 + (int) Math.Floor(settlementLevel / 2f); }
  }
  ...
  public void tickSpecialActions(int tick) {
    if (trait_Egalitarian_TaxBreak_Enabled == true &&
  ...
  public void initBaseProduction() {
    for (int i = 0; i < getNumberResource(); i++) {
      ...
      resource.baseProduction = biomeDef.BaseProductionAdditive[i] +
                                hillinessDef.BaseProductionAdditive[i];
      resource.baseProduction = biomeDef.BaseProductionMultiplicative[i] +
                                hillinessDef.BaseProductionMultiplicative[i];
  ...
  public void updateStats() {
    ...
    settlementMilitaryLevel =
        (settlementLevel - 1) +
        Convert.ToInt32(
            traitUtilsFC.cycleTraits(new double(), "militaryBaseLevel", traits,
                                     "add") +
            traitUtilsFC.cycleTraits(
                new double(), "militaryBaseLevel",
                Find.World.GetComponent<FactionFC>().traits, "add"));

    // Worker Stats
    workersMax = (settlementLevel * (3 + trait_Isolationist_Extra_Workers)) +
                 (traitUtilsFC.cycleTraits(new double(), "workerBaseMax",
                                           traits, "add") +
                  traitUtilsFC.cycleTraits(
                      new double(), "workerBaseMax",
                      Find.World.GetComponent<FactionFC>().traits, "add")) +
                 returnMaxWorkersFromPrisoners();
    workersUltraMax =
        (workersMax + 5 +
         (traitUtilsFC.cycleTraits(new double(), "workerBaseOverMax", traits,
                                   "add") +
          traitUtilsFC.cycleTraits(new double(), "workerBaseOverMax",
                                   Find.World.GetComponent<FactionFC>().traits,
                                   "add")) +
  ...
  public void updateHappiness() {
    ...
    if (factionfc.hasPolicy(FCPolicyDefOf.egalitarian) &&
        this.trait_Egalitarian_TaxBreak_Enabled == true)
      policyIncrease = 2;

    happiness +=
        happinessGainMultiplier *
        (policyIncrease + FactionColonies.happinessBaseGain +
         traitUtilsFC.cycleTraits(new double(), "happinessGainedBase", traits,
                                  "add") +
         traitUtilsFC.cycleTraits(
             new double(), "happinessGainedBase",
             Find.World.GetComponent<FactionFC>().traits,
             "add")); // Go through traits and add happiness where needed
    happiness -=
        happinessLostMultiplier *
        (FactionColonies.happinessBaseLost +
         traitUtilsFC.cycleTraits(new double(), "happinessLostBase", traits,
                                  "add") +
         traitUtilsFC.cycleTraits(
             new double(), "happinessLostBase",
             Find.World.GetComponent<FactionFC>().traits,
             "add")); // Go through traits and remove happiness where needed
  ...
  public void updateLoyalty() {
    ...
    double loyaltyLostMultiplier =
        (traitUtilsFC.cycleTraits(new double(), "loyaltyLostMultiplier", traits,
                                  "multiply") *
         traitUtilsFC.cycleTraits(new double(), "loyaltyLostMultiplier",
                                  Find.World.GetComponent<FactionFC>().traits,
                                  "multiply"));

    loyalty += loyaltyGainMultiplier *
               (FactionColonies.loyaltyBaseGain +
                traitUtilsFC.cycleTraits(new double(), "loyaltyGainedBase",
                                         traits, "add") +
                traitUtilsFC.cycleTraits(
                    new double(), "loyaltyGainedBase",
                    Find.World.GetComponent<FactionFC>().traits,
                    "add")); // Go through traits and add loyalty where needed
    loyalty -=
        loyaltyLostMultiplier *
        (FactionColonies.loyaltyBaseLost +
         traitUtilsFC.cycleTraits(new double(), "loyaltyLostBase", traits,
                                  "add") +
         traitUtilsFC.cycleTraits(
             new double(), "loyaltyLostBase",
             Find.World.GetComponent<FactionFC>().traits,
             "add")); // Go through traits and remove loyalty where needed
  ...
  public void updateProsperity() {
    ...
    prosperity +=
        (policyIncrease + FactionColonies.prosperityBaseRecovery +
         traitUtilsFC.cycleTraits(new double(), "prosperityBaseRecovery",
                                  traits, "add") +
         traitUtilsFC.cycleTraits(
             new double(), "prosperityBaseRecovery",
             Find.World.GetComponent<FactionFC>().traits,
             "add")); // Go through traits and add prosperity where needed
  ...
  public void updateUnrest() {
    ...
    double unrestLostMultiplier =
        (traitUtilsFC.cycleTraits(new double(), "unrestLostMultiplier", traits,
                                  "multiply") *
         traitUtilsFC.cycleTraits(new double(), "unrestLostMultiplier",
                                  Find.World.GetComponent<FactionFC>().traits,
                                  "multiply"));

    unrest += unrestGainMultiplier *
              (FactionColonies.unrestBaseGain +
               traitUtilsFC.cycleTraits(new double(), "unrestGainedBase",
                                        traits, "add") +
               traitUtilsFC.cycleTraits(
                   new double(), "unrestGainedBase",
                   Find.World.GetComponent<FactionFC>().traits,
                   "add")); // Go through traits and add unrest where needed
    unrest -= unrestLostMultiplier *
              (FactionColonies.unrestBaseLost +
               traitUtilsFC.cycleTraits(new double(), "unrestLostBase", traits,
                                        "add") +
               traitUtilsFC.cycleTraits(
                   new double(), "unrestLostBase",
                   Find.World.GetComponent<FactionFC>().traits,
                   "add")); // Go through traits and remove unrest where needed
  ...
  public void updateProduction() // updates production of settlemetns
  {
    ...
    for (int i = 0; i < getNumberResource(); i++) {
      ...
      returnResourceByInt(i).baseProduction =
          biomeDef.BaseProductionAdditive[i] +
          hillinessDef.BaseProductionAdditive[i] +
          traitUtilsFC.cycleTraits(
              new double(), "productionBase" + returnResourceNameByInt(i),
              traits, "add") +
          traitUtilsFC.cycleTraits(
              new double(), "productionBase" + returnResourceNameByInt(i),
              Find.World.GetComponent<FactionFC>().traits, "add");
      returnResourceByInt(i).baseProductionMultiplier =
          traitResourceMultiplier * biomeDef.BaseProductionMultiplicative[i] *
          hillinessDef.BaseProductionMultiplicative[i] *
          (((100 + trait_Egalitarian_TaxBoost + trait_Isolationist_TaxBoost) +
            traitUtilsFC.cycleTraits(new double(), "taxBasePercentage", traits,
                                     "add") +
            traitUtilsFC.cycleTraits(
      ...
      if (returnResourceByInt(i).baseProductionAdditives.Count() >
          1) // over one to skip null value (used to save in Expose
      {
        for (int k = 1;
             k < returnResourceByInt(i).baseProductionAdditives.Count(); k++) {
          tempAdditive +=
              returnResourceByInt(i).baseProductionAdditives[k].value;
        }
      }

      // multiply multiplicative variables
      double tempMultiplier = 1;
      if (returnResourceByInt(i).baseProductionMultipliers.Count() >
          1) // over one to skip null value (used to save in Expose
      {
        for (int k = 1;
             k < returnResourceByInt(i).baseProductionMultipliers.Count();
             k++) {
          tempMultiplier *=
              returnResourceByInt(i).baseProductionMultipliers[k].value;
        }
      }

      // calculate end multiplier + endproduction and update to resource
      returnResourceByInt(i).endProductionMultiplier =
          (returnResourceByInt(i).baseProductionMultiplier *
           (prosperity / 100) * tempMultiplier *
           traitUtilsFC.cycleTraits(new double(),
                                    "productionMultiplier" +
                                        returnResourceNameByInt(i),
                                    traits, "multiply") *
           traitUtilsFC.cycleTraits(
               new double(),
               "productionMultiplier" + returnResourceNameByInt(i),
               Find.World.GetComponent<FactionFC>().traits, "multiply"));
      returnResourceByInt(i).endProduction =
          returnResourceByInt(i).endProductionMultiplier *
          ((returnResourceByInt(i).baseProduction + tempAdditive) *
           returnResourceByInt(i).assignedWorkers);
    }
  }
    ...
    for (int i = 0; i < getNumberResource(); i++) {
      if (returnResourceByInt(i) != null) {
        if (returnResourceByInt(i).isTithe ==
            false) { // if resource is not paid by tithe
          income += (returnResourceByInt(i).endProduction *
                     LoadedModManager.GetMod<FactionColoniesMod>()
                         .GetSettings<FactionColonies>()
                         .silverPerResource);
        } else { // if resource is paid by tithe  (Calculate value of tithe
                 // items
        }
      }
    }
  ...
  public bool increaseWorkers(int ResourceID, int numWorkers) {
    ...
    } else if (workers + numWorkers <= workersUltraMax &&
  ...
  public double getBaseWorkerCost() {
    return (LoadedModManager.GetMod<FactionColoniesMod>()
                .GetSettings<FactionColonies>()
                .workerCost +
            (traitUtilsFC.cycleTraits(new double(), "workerBaseCost", traits,
                                      "add") +
             traitUtilsFC.cycleTraits(
                 new double(), "workerBaseCost",
                 Find.World.GetComponent<FactionFC>().traits, "add")));
  ...
  public double getTotalUpkeep() // returns total upkeep of all settlements
  {
    ...
    workerTotalUpkeep = (workers * getBaseWorkerCost()) +
                        ((workers * getBaseWorkerCost()) * (overWork / 20));
    ...
    foreach (BuildingFCDef building in buildings) {
      ...
      if (building.upkeep != 0 && !isMilitary ||
  ...
  public void
  updateWorkerCost() // runs inside updateProfit to attach during updating
  {
  ...
  public void ExposeData() {
    ...
    Scribe_Values.Look<double>(ref productionEfficiency,
    ...
    Scribe_Values.Look<int>(ref settlementMilitaryLevel,
                            "settlementMilitaryLevel");
    ...
    // Taxes
    Scribe_Collections.Look<BuildingFCDef>(ref buildings, "buildings",
                                           LookMode.Def);
    ...
    Scribe_References.Look<MercenarySquadFC>(ref militarySquad,
                                             "militarySquad");
    Scribe_Values.Look<int>(ref artilleryTimer, "artilleryTimer");
    Scribe_Values.Look<string>(ref militaryLocationPlanet,
    ...
    Scribe_Collections.Look<FCPrisoner>(ref prisonerList, "prisonerList",
                                        LookMode.Deep);

    // Traits
    Scribe_Values.Look<int>(ref trait_Egalitarian_TaxBreak_Tick,
                            "trait_Egalitarian_TaxBreak_Tick");
    Scribe_Values.Look<bool>(ref trait_Egalitarian_TaxBreak_Enabled,
                             "trait_Egalitarian_TaxBreak_Enabled");
  }
  ...
  public bool isMilitaryBusy() {
    if (militaryBusy) {
      Messages.Message("militaryAlreadyAssigned".Translate(),
    ...
    if (militarySquad != null) {
      if (militarySquad.outfit != null) {
        if (militarySquad.EquippedMercenaries.Count > 0) {
          ...
          Messages.Message(
              "You can't deploy a squad with no equipped personnel!",
              MessageTypeDefOf.RejectInput);
          return false;
        }
      } else {
        Messages.Message(
            "There is no squad loadout assigned to that settlement!",
            MessageTypeDefOf.RejectInput);
        return false;
      }
    } else {
      Messages.Message(
          "There is no military squad assigned to that settlement!",
          MessageTypeDefOf.RejectInput);
      return false;
    }
  }
  ...
  public bool isMilitaryBusySilent() { return militaryBusy; }
  ...
  public bool isTargetOccupied(int location) {
    ...
    if (Find.World.GetComponent<FactionFC>().militaryTargets.Contains(
            location)) {
      Messages.Message("targetAlreadyBeingAttacked".Translate(),
                       MessageTypeDefOf.RejectInput);
      return true;
    } else {
      return false;
  ...
  public float Happiness {
    get { return (float) Math.Round(this.happiness, 1); }
  }
  public float Unrest {
    get { return (float) Math.Round(this.unrest, 1); }
  }
  public float Loyalty {
    get { return (float) Math.Round(this.loyalty, 1); }
  }
  public float Prosperity {
    get { return (float) Math.Round(this.prosperity, 1); }
  }

  public bool sendMilitary(int location, string planet, string job,
                           int timeToFinish, Faction enemy) {
    FactionFC factionfc = Find.World.GetComponent<FactionFC>();
    if (isMilitaryBusy() == false && isTargetOccupied(location) == false) {
      ...
      if (militaryJob == "raidEnemySettlement") {
        ...
        tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
            "settlementMilitaryForcesRaiding", name,
            returnMilitaryTarget().Label); // +
        factionfc.addEvent(tmp);
        Find.LetterStack.ReceiveLetter(
            "Military Action",
            TranslatorFormattedStringExtensions.Translate(
                "FCMilitarySentRaid", this.name,
                Find.WorldObjects.SettlementAt(location)),
            LetterDefOf.NeutralEvent);
      }

      if (militaryJob == "enslaveEnemySettlement") {
        FCEvent tmp =
            FCEventMaker.MakeEvent(FCEventDefOf.enslaveEnemySettlement);
        tmp.hasCustomDescription = true;
        tmp.timeTillTrigger = Find.TickManager.TicksGame + timeToFinish;
        tmp.location = mapLocation;
        tmp.planetName = Find.World.info.name;
        tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
            "settlementMilitaryForcesEnslave", name,
            returnMilitaryTarget().Label); // +
        factionfc.addEvent(tmp);
        Find.LetterStack.ReceiveLetter(
            "Military Action",
            TranslatorFormattedStringExtensions.Translate(
                "FCMilitarySentEnslave", this.name,
                Find.WorldObjects.SettlementAt(location)),
            LetterDefOf.NeutralEvent);
      }
      ...
      if (militaryJob == "captureEnemySettlement") {
        FCEvent tmp =
            FCEventMaker.MakeEvent(FCEventDefOf.captureEnemySettlement);
        tmp.hasCustomDescription = true;
        tmp.timeTillTrigger = Find.TickManager.TicksGame + timeToFinish;
        tmp.location = mapLocation;
        tmp.planetName = Find.World.info.name;
        tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
            "settlementMilitaryForcesCapturing", name,
            returnMilitaryTarget().Label); // +
        factionfc.addEvent(tmp);
        Find.LetterStack.ReceiveLetter(
            "Military Action",
            TranslatorFormattedStringExtensions.Translate(
                "FCMilitarySentCapture", this.name,
                Find.WorldObjects.SettlementAt(location)),
            LetterDefOf.NeutralEvent);
      }
      ...
      if (militaryJob == "defendFriendlySettlement") {
        ...
        //
        // Find.LetterStack.ReceiveLetter("militarySent".Translate(),
        // TranslatorFormattedStringExtensions.Translate("militarySentToDefend",
        // name, factionfc.returnSettlementByLocation(location,
        // this.planetName).name), LetterDefOf.NeutralEvent);
      }

      if (militaryJob == "Deploy") {
        // Find.LetterStack.ReceiveLetter("Military Deployed", "The Military
        // forces of " + name + " have been deployed to " +
        // Find.Maps[militaryLocation].Parent.LabelCap,
        // LetterDefOf.NeutralEvent);
        militaryBusyTimer = -1;
      }
  ...
  public void militaryTick() {
    if (militaryBusy == false) { // if not busy
      return;
    } else { // if busy
    }
  }
  ...
  public void processMilitaryEvent() {
    ...
    if (militaryJob == "raidEnemySettlement") {
      int winner = simulateBattleFC.FightBattle(
          militaryForce.createMilitaryForceFromSettlement(this, true),
          militaryForce.createMilitaryForceFromFaction(militaryEnemy, false));
      if (winner == 0) {
        // if won
        faction.addExperienceToFactionLevel(5f);

        TechLevel tech = Find.WorldObjects.SettlementAt(militaryLocation)
        ...
        if (Find.WorldObjects.SettlementAt(militaryLocation)
        ...
        if (num <= 4 && getSlaves) {
          ...
          text = text + TranslatorFormattedStringExtensions.Translate(
                            "PrisonerCaptureInfo", prisoner.Name.ToString(),
                            this.name);
        ...
        // if lost
        Find.LetterStack.ReceiveLetter(
            "RaidFailure".Translate(),
            TranslatorFormattedStringExtensions.Translate(
                "RaidEnemySettlementFailure",
                Find.WorldObjects.SettlementAt(militaryLocation).LabelCap),
      ...
      if (winner == 0) {
        ...
        for (int i = 0; i <= num; i++) {
          ...
          text =
              text +
              TranslatorFormattedStringExtensions.Translate(
                  "PrisonerCaptureInfo", prisoner.Name.ToString(), this.name) +
              "\n";
          this.addPrisoner(prisoner);
        }
      ...
      if (winner == 0) {
        ...
        TechLevel tech = Find.WorldObjects.SettlementAt(militaryLocation)
                             .Faction.def.techLevel;
        Faction tempFactionLink =
            Find.WorldObjects.SettlementAt(militaryLocation).Faction;
        Find.WorldObjects.SettlementAt(militaryLocation).Destroy();
        if (Find.World.info.name == militaryLocationPlanet) {
          Settlement settlement = FactionColonies.createPlayerColonySettlement(
              militaryLocation, true, militaryLocationPlanet);
          settlement.Name = tmpName;
        } else {
          Settlement settlement = FactionColonies.createPlayerColonySettlement(
              militaryLocation, false, militaryLocationPlanet);
          Find.World.GetComponent<FactionFC>().createSettlementQueue.Add(
              new SettlementSoS2Info(militaryLocationPlanet, militaryLocation));
        }
        SettlementFC settlementfc =
            Find.World.GetComponent<FactionFC>().returnSettlementByLocation(
                militaryLocation, Find.World.info.name);
        ...
        Find.LetterStack.ReceiveLetter(
            "CaptureSettlement".Translate(),
            TranslatorFormattedStringExtensions.Translate(
                "CaptureEnemySettlementSuccess", this.name,
                Find.WorldObjects.SettlementAt(militaryLocation).Name,
                settlementfc.settlementLevel),
            LetterDefOf.PositiveEvent,
            new LookTargets(Find.WorldObjects.SettlementAt(militaryLocation)));
      } else if (winner == 1) {
        ...
        Find.LetterStack.ReceiveLetter(
            "CaptureSettlement".Translate(),
            TranslatorFormattedStringExtensions.Translate(
                "CaptureEnemySettlementFailure", this.name,
                Find.WorldObjects.SettlementAt(militaryLocation).Name),
            LetterDefOf.NegativeEvent,
            new LookTargets(Find.WorldObjects.SettlementAt(militaryLocation)));
      }
  ...
  public void returnMilitary(bool alert) {
    ...
    if (alert) {
      Find.LetterStack.ReceiveLetter(
          "Military Cooldown",
          TranslatorFormattedStringExtensions.Translate("FCMilitaryCooldown",
                                                        this.name),
          LetterDefOf.PositiveEvent);
    }
  }
  ...
  public void cooldownMilitary() {
    ...
    if (faction.hasTrait(FCPolicyDefOf.raiders) &&
        (militaryJob == "raidEnemySettlement" ||
         militaryJob == "enslaveEnemySettlement")) {
    ...
    tmp.hasCustomDescription = true;
    tmp.timeTillTrigger =
        Find.TickManager.TicksGame + 180000 - trait_CooldownReduction;
    tmp.location = mapLocation;
    tmp.planetName = this.planetName;
    tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
        "MilitaryForcesReorganizing", name); // +
    Find.World.GetComponent<FactionFC>().addEvent(tmp);
  }
  ...
  public void deconstructBuilding(int buildingSlot) {
    foreach (FCTraitEffectDef trait in buildings[buildingSlot]
  ...
  public bool validConstructBuilding(BuildingFCDef building, int buildingSlot,
    ...
    bool valid = true;
    foreach (BuildingFCDef slot in buildings) // check if already a building of
                                              // that type constructed
    {
      if (slot == building) {
        valid = false;
        Messages.Message("BuildingAlreadyType".Translate() + "!",
                         MessageTypeDefOf.RejectInput);
        break;
      }
    }

    if (PaymentUtil.getSilver() <
        building.cost) // check if the player has enough money
    {
      valid = false;
      Messages.Message("NotEnoughSilverConstructBuilding".Translate() + "!",
                       MessageTypeDefOf.RejectInput);
    }

    foreach (FCEvent event1 in Find.World.GetComponent<FactionFC>()
                 .events) // check if construction would match any
                          // already-occuring events
    {
      if (event1.source == mapLocation && event1.building == building &&
          event1.def.defName == "constructBuilding") {
        valid = false;
        Messages.Message("BuildingBeingBuiltAlreadyType".Translate() + "!",
                         MessageTypeDefOf.RejectInput);
        break;
      }
      ...
      if (event1.source == mapLocation && event1.buildingSlot == buildingSlot &&
          event1.def.defName ==
              "constructBuilding") // check if there is already a building being
                                   // constructed in that slot
      {
        valid = false;
        Messages.Message("BuildingAlreadyConstructed".Translate() + "!",
                         MessageTypeDefOf.RejectInput);
        break;
      }
    }
    ...
    if (building.applicableBiomes.Count() != 0) {
      ...
      if (match == false) {
        valid = false;
        Messages.Message("BuildingInvalidEnvironment".Translate(),
                         MessageTypeDefOf.RejectInput);
      }
    }
  ...
  public double returnTitheEstimatedValue() {
    ...
    for (int i = 0; i < getNumberResource(); i++) {
      if (returnResourceByInt(i).isTithe == true) {
        titheVal += returnResourceByInt(i).endProduction *
                    LoadedModManager.GetMod<FactionColoniesMod>()
                        .GetSettings<FactionColonies>()
                        .silverPerResource;
      }
  ...
  public string returnResourceNameByInt(
      int name) // used to return the correct resource based on string name
  {
    if (name == 0) {
      return "Food";
  ...
  public int getNumberResource() { return 9; }
  ...
  public void taxProductionGoods() // update goods (TAX TAX TAX)   # Not used?
  {
    int silver = 0;
    for (int i = 0; i < getNumberResource(); i++) {
      if (returnResourceByInt(i)
              .isTithe) { // if resource is paying via tithe
                          // generate the tithe things
                          // run tithe cash evaluation here
                          // ThingSetMaker gen = new ThingSetMaker();
      } else { // if resource is paying via silver
        silver += (int)(returnResourceByInt(i).endProduction *
                        LoadedModManager.GetMod<FactionColoniesMod>()
                            .GetSettings<FactionColonies>()
                            .silverPerResource); // Add randomness?
      }
    }
  }

  // UNUSED FUNCTIONS
  public float getSilverIncome() { return silverIncome; }

  public void resetSilverIncome() { silverIncome = 0; }

  public void addSilverIncome(float amount) { silverIncome += amount; }
  ...
  public List<Thing> getTithe() { return tithe; }

  public void resetTithe() { tithe = new List<Thing>(); }
  ...
  public void goTo() {
    ...
    Find.WorldSelector.Select(Find.WorldObjects.SettlementAt(
        mapLocation)); // = Convert.ToInt32(settlementList[i][10]);
                       // //(Find.World.GetComponent<FactionFC>().settlements[i]);
  ...
  public float createResearchPool() {
    ...
    if (faction.hasTrait(FCPolicyDefOf.innovative))
      trait_Innovative_BonusResearch =
          (this.getTotalProfit() * .05) * trait_Technocratic_Modifier;
    return (float) Math.Max(
        Math.Round((production * FactionColonies.productionResearchBase) +
                   trait_Innovative_BonusResearch),
        0);
  }
    ...
    for (int i = 0; i < getNumberResource(); i++) {
      ...
      if (resource.isTithe == true && i != 6 && i != 7) {
        ...
        if (resource.filter.AllowedThingDefs.Count() == 0) {
          Find.LetterStack.ReceiveLetter(
              "No Tithe",
              "There are no enabled items in the tithe" +
                  returnResourceNameByInt(i) + " of settlement " + name,
              LetterDefOf.NegativeEvent);
          continue;
        }
        ...
        // Create Temp Value
        double tmpValue =
            (production * LoadedModManager.GetMod<FactionColoniesMod>()
                              .GetSettings<FactionColonies>()
                              .silverPerResource);
        ...
        if (resource.checkMinimum()) {
          ...
          tmpList = PaymentUtil.generateTithe(
              resource.taxStock,
              (double) LoadedModManager.GetMod<FactionColoniesMod>()
                  .GetSettings<FactionColonies>()
                  .productionTitheMod,
              workers, i,
              traitUtilsFC.cycleTraits(
                  0.0, "taxBaseRandomModifier",
                  Find.World.GetComponent<FactionFC>().traits, "add") +
