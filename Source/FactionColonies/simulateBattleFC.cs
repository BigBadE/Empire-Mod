...
class simulateBattleFC {
  ...
  public static void FightRound(militaryForce MFA, militaryForce MFB) {
    ...
    // Log.Message("A Begin: " + MFA.forceRemaining + " : " + MFB.forceRemaining
    // + " B begin"); Log.Message("A Rolled: " + randA.ToString() + " : " +
    // randB.ToString() + " B rolled");
    if (randA == randB) {
      return;
    } else if (randA > randB) {
    ...
    // Log.Message("A Remain: " + MFA.forceRemaining + " : " +
    // MFB.forceRemaining + " B remain");
  }
}
...
public class militaryForce : IExposable {
  ...
  public militaryForce(double militaryLevel, double militaryEfficiency,
                       SettlementFC homeSettlement, Faction homeFaction) {
  ...
  public static militaryForce
  createMilitaryForceFromSettlement(SettlementFC settlement,
    ...
    double militaryLevel = settlement.settlementMilitaryLevel +
                           trait_MilitaryLevelBonus + homeForceLevel;
    double efficiency =
    ...
    if (isAttacking && faction.hasPolicy(FCPolicyDefOf.militaristic))
      ;
    efficiency *= 1.2;
    militaryForce returnForce =
        new militaryForce(militaryLevel, efficiency, settlement,
  ...
  public static militaryForce
  createMilitaryForceFromEnemySettlement(Settlement settlement) {
    ...
    default:
      ...
      efficiency = 1;
      Log.Message(
          "Defaulted createMilitaryForceFromEnemyFaction switch case - Empire Mod");
      break;
    }

    militaryForce returnForce =
        new militaryForce(militaryLevel, efficiency, null, settlement.Faction);
    return returnForce;
  }

  public static militaryForce createMilitaryForce(double strength,
                                                  Faction faction) {
    militaryForce returnForce = new militaryForce(strength, 1, null, faction);
    return returnForce;
  }

  public static militaryForce createMilitaryForceFromFaction(Faction faction,
                                                             bool handicap) {
    double militaryLevel = 1;
    double efficiency = 1;
    if (faction != null && faction.def != null &&
        faction.def.techLevel != null) {
    ...
    if (handicap) {
      value = Math.Min(
          value,
          (2 +
           Math.Round((double)(Find.TickManager.TicksGame -
    ...
    militaryForce returnForce =
        new militaryForce(value, efficiency, null, faction);
    return returnForce;
  }
}

class MilitaryUtilFC {
  public static void attackPlayerSettlement(militaryForce attackingForce,
    ...
    tmp.customDescription = TranslatorFormattedStringExtensions.Translate(
        "settlementAboutToBeAttacked", settlement.name, enemyFaction.Name); // +
    tmp.militaryForceDefending =
        militaryForce.createMilitaryForceFromSettlement(settlement);
    tmp.militaryForceDefendingFaction =
        FactionColonies.getPlayerColonyFaction();
    ...
    foreach (SettlementFC settlementCompare in factionfc.settlements) {
      if (settlementCompare.autoDefend &&
          settlementCompare.militaryBusy == false &&
          settlementCompare.settlementMilitaryLevel >
              settlement.settlementMilitaryLevel &&
          (highest == null || settlementCompare.settlementMilitaryLevel >
                                  highest.settlementMilitaryLevel))
    ...
    Find.LetterStack.ReceiveLetter(
        "settlementInDanger".Translate(), tmp.customDescription,
        LetterDefOf.ThreatBig,
        new LookTargets(
            Find.WorldObjects.SettlementAt(settlement.mapLocation)));
  }

  public static void
  changeDefendingMilitaryForce(FCEvent evt,
                               SettlementFC settlementOfMilitaryForce) {
    FactionFC factionfc = Find.World.GetComponent<FactionFC>();
    ...
    if (settlementOfMilitaryForce ==
        evt.militaryForceDefending.homeSettlement) {
      Messages.Message("militaryAlreadyDefendingSettlement".Translate(),
    ...
    if (settlementOfMilitaryForce != homeSettlement)
      tmpMilitaryForce = militaryForce.createMilitaryForceFromSettlement(
          factionfc.returnSettlementByLocation(evt.location, evt.planetName),
          true);
    ...
    evt.militaryForceDefending =
        militaryForce.createMilitaryForceFromSettlement(
            settlementOfMilitaryForce, homeDefendingForce
            : tmpMilitaryForce);

    if (settlementOfMilitaryForce == homeSettlement) {
      // if home settlement is reseting to defense
      Messages.Message("defendingMilitaryReset".Translate(),
                       MessageTypeDefOf.NeutralEvent);

    } else {
      // if settlement is foreign
      settlementOfMilitaryForce.sendMilitary(
          evt.settlementFCDefending.mapLocation, evt.planetName,
          "defendFriendlySettlement", -1, evt.militaryForceAttackingFaction);
      Find.LetterStack.ReceiveLetter(
          "Military Action",
          TranslatorFormattedStringExtensions.Translate(
              "ForeignMilitarySwitch", settlementOfMilitaryForce.name,
              factionfc.returnSettlementByLocation(evt.location, evt.planetName)
                  .name,
...
class RelationsUtilFC {
  public static void attackFaction(Faction faction) {
    // Log.Message(Find.FactionManager.OfPlayer.RelationWith(faction).goodwill +
    // " player:colony ");
    Find.FactionManager.OfPlayer.TryAffectGoodwillWith(faction, -50);
    Find.FactionManager.OfPlayer.TrySetRelationKind(
        faction, FactionRelationKind.Hostile);
    resetPlayerColonyRelations();
    // Log.Message(Find.FactionManager.OfPlayer.RelationWith(faction).goodwill +
    // " player:colony ");
    // FactionColonies.getPlayerColonyFaction().TryAffectGoodwillWith(faction,
    // -50)
  }
  ...
  public static void resetPlayerColonyRelations() {
    ...
    foreach (Faction faction in Find.FactionManager.AllFactionsInViewOrder) {
      if (faction != Find.FactionManager.OfPlayer && faction != PCFaction) {
        ...
        PCFaction.TryAffectGoodwillWith(
            faction,
            (Find.FactionManager.OfPlayer.RelationWith(faction).goodwill -
             PCFaction.RelationWith(faction).goodwill));
        PCFaction.TrySetRelationKind(
            faction, Find.FactionManager.OfPlayer.RelationKindWith(faction));
        // Log.Message(Find.FactionManager.OfPlayer.RelationWith(faction).goodwill
        // + " player:colony " + PCFaction.RelationWith(faction).goodwill);
      }
    }
  }
}
