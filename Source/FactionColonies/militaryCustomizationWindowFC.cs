...
public class MilitaryFireSupport : IExposable, ILoadReferenceable {
  ...
  public MilitaryFireSupport(string fireSupportType, Map map, IntVec3 location,
                             int ticksTillEnd, int startupTime, float accuracy,
                             List<ThingDef> projectiles = null) {
  ...
  public void setLoadID() {
    this.loadID =
        Find.World.GetComponent<FactionFC>().GetNextMilitaryFireSupportID();
  }
  ...
  public float returnTotalCost() {
    ...
    foreach (ThingDef def in projectiles) {
      cost += def.BaseMarketValue * 1.5f *
              (1 + returnAccuracyCostPercentage() / 100);
    }
  ...
  public void delete () {
    Find.World.GetComponent<FactionFC>()
        .militaryCustomizationUtil.fireSupportDefs.Remove(this);
  }
  ...
  public List<ThingDef> returnFireSupportOptions() {
    ...
    param.filter.SetAllow(
        DefDatabase<ThingCategoryDef>.GetNamedSilentFail("MortarShells"), true);
    if (DefDatabase<ThingCategoryDef>.GetNamedSilentFail("AmmoShells") != null)
      param.filter.SetAllow(
          DefDatabase<ThingCategoryDef>.GetNamedSilentFail("AmmoShells"), true);
    List<ThingDef> list =
        thingSetMaker.AllGeneratableThingsDebug(param).ToList();
    return list;
  }
  ...
  public void ExposeData() {
    ...
    Scribe_Collections.Look<ThingDef>(ref projectiles, "projectiles",
                                      LookMode.Def);
  }
}
...
public class MilitaryCustomizationUtil : IExposable {
  ...
  public List<MilitaryFireSupport> fireSupport =
      new List<MilitaryFireSupport>();
  public List<MilitaryFireSupport> fireSupportDefs =
      new List<MilitaryFireSupport>();
  ...
  public void checkMilitaryUtilForErrors() {
    ...
    foreach (MilSquadFC squad in squads) {
      ...
      for (int count = 0; count < 30; count++) {
        if (squad.units[count] == null ||
            (units.Contains(squad.units[count]) == false &&
    ...
    foreach (MercenarySquadFC squad in mercenarySquads) {
      if (squad.outfit == null || squads.Contains(squad.outfit) == false) {
        ...
        if (squad.outfit != null &&
            squad.outfit.equipmentTotalCost >
                FactionColonies.calculateMilitaryLevelPoints(
                    settlementMilLevel)) {
          if (squad.settlement != null)
            Messages.Message(
                "The max allowed equipment cost for the squad assigned to " +
  ...
  public MercenarySquadFC returnSquadFromUnit(Pawn unit) {
    ...
    Log.Message(
        "Empire - MercenarySquadFC - returnSquadFromUnit - Did not find squad.");
    return null;
  }
  ...
  public void resetSquads() { squads = new List<MilSquadFC>(); }
  ...
  public void attemptToAssignSquad(SettlementFC settlement, MilSquadFC squad) {
    if (FactionColonies.calculateMilitaryLevelPoints(
            settlement.settlementMilitaryLevel) >= squad.equipmentTotalCost) {
      ...
      Messages.Message(squad.name + "'s loadout has been assigned to " +
                           settlement.name,
                       MessageTypeDefOf.TaskCompletion);
    } else {
      Messages.Message("That squad exceeds the settlement's max allotted cost",
                       MessageTypeDefOf.RejectInput);
    }
  }

  public MercenarySquadFC createMercenarySquad(SettlementFC settlement,
                                               bool isExtra = false) {
  ...
  public void changeTick() { tickChanged = Find.TickManager.TicksGame; }
  ...
  public void ExposeData() {
    ...
    Scribe_Collections.Look<MercenarySquadFC>(ref mercenarySquads,
                                              "mercenarySquads", LookMode.Deep);
    Scribe_Collections.Look<MilitaryFireSupport>(ref fireSupport, "fireSupport",
                                                 LookMode.Deep);
    Scribe_Collections.Look<MilitaryFireSupport>(
        ref fireSupportDefs, "fireSupportDefs", LookMode.Deep);
    Scribe_Collections.Look<Mercenary>(ref deadPawns, "deadPawns",
                                       LookMode.Deep);
...
public class MilUnitFC : IExposable, ILoadReferenceable {
  ...
  public string GetUniqueLoadID() { return "MilUnitFC_" + this.loadID; }
  public void ExposeData() {
    Scribe_Values.Look<int>(ref loadID, "loadID");
    Scribe_Values.Look<string>(ref name, "name");
    Scribe_Deep.Look<Pawn>(ref defaultPawn, "defaultPawn");
    Scribe_Values.Look<bool>(ref isBlank, "blank");
    Scribe_Values.Look<double>(ref equipmentTotalCost, "equipmentTotalCost",
                               -1);
  ...
  public void generateDefaultPawn() {
    ...
    Reset:
      ...
      foreach (ThingWithComps weapon in defaultPawn.equipment
    ...
    defaultPawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
        kindDef, FactionColonies.getPlayerColonyFaction(),
        PawnGenerationContext.NonPlayer, -1, false, false, false, false, true,
        true, 0, false, false, false, false, false, false, false, false, 0,
  ...
  public void equipWeapon(ThingWithComps weapon) {
    ...
    if (isCivilian == false) {
      ...
      Messages.Message("You cannot put a weapon on a civilian!",
                       MessageTypeDefOf.RejectInput);
    }
  }
    ...
    foreach (ApparelLayerDef layer in Equipment.def.apparel.layers) {
      foreach (BodyPartGroupDef part in Equipment.def.apparel.bodyPartGroups) {
        foreach (Apparel apparel in defaultPawn.apparel.WornApparel) {
          if ((apparel.def.apparel.layers.Contains(layer) == true &&
               apparel.def.apparel.bodyPartGroups.Contains(part)) ||
              (Equipment.def.apparel.layers.Contains(
                   ApparelLayerDefOf.Overhead) &&
               apparel.def.apparel.layers.Contains(
                   ApparelLayerDefOf.Overhead))) {
  ...
  public void removeUnit() {
    Find.World.GetComponent<FactionFC>().militaryCustomizationUtil.units.Remove(
        this);
  }
  ...
  public void updateEquipmentTotalCost() {
    if (isBlank == true) {
      ...
      totalCost += Math.Floor(defaultPawn.def.BaseMarketValue *
      ...
      foreach (Thing thing in defaultPawn.equipment
                   .AllEquipmentListForReading) {
        totalCost += thing.MarketValue;
      }

      if (animal != null) {
        totalCost += Math.Floor(animal.race.BaseMarketValue *
...
public class MercenarySquadFC : IExposable, ILoadReferenceable {
  ...
  public void ExposeData() {
    ...
    Scribe_Values.Look<string>(ref name, "name");
    Scribe_Collections.Look<Mercenary>(ref mercenaries, "mercenaries",
                                       LookMode.Deep);
    ...
    Scribe_Collections.Look<ThingWithComps>(
        ref UsedWeaponList, "UsedWeaponList", LookMode.Reference);
    Scribe_Collections.Look<Apparel>(ref UsedApparelList, "UsedApparelList",
                                     LookMode.Reference);
  ...
  public string GetUniqueLoadID() { return "MercenarySquadFC_" + this.loadID; }
  ...
  public List<Mercenary> EquippedMercenaries {
    get {
      ...
      foreach (Mercenary merc in mercenaries) {
        if ((merc.pawn.apparel.WornApparel.Count() > 0 ||
             merc.pawn.equipment.AllEquipmentListForReading.Count() > 0 ||
             merc.animal != null) &&
            merc.deployable == true) {
          pawns.Add(merc);
        }
      }
      return pawns;
  ...
  public SettlementFC getSettlement {
    get {
      if (settlement != null) {
        ...
        foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
                     .settlements) {
          if (settlement.militarySquad != null &&
              settlement.militarySquad == this) {
  ...
  public void initiateSquad() {
    ...
    if (loadID == -1) {
      this.loadID =
          Find.World.GetComponent<FactionFC>().GetNextMercenarySquadID();
    }
  ...
  public void resetNeeds() {
    foreach (Pawn merc in AllEquippedMercenaryPawns) {
      ...
      if (!merc.AnimalOrWildMan()) {
        ...
        merc.needs.mood.thoughts.memories.TryGainMemory(
            DefDatabase<ThoughtDef>.GetNamed("FC_Mercenary"));
      }
    }
  }
  ...
  public void createNewAnimal(ref Mercenary merc, PawnKindDef race) {
    Pawn newPawn;
    newPawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
        race, FactionColonies.getPlayerColonyFaction(),
        PawnGenerationContext.NonPlayer, -1, false, false, false, false, false,
        true, 0, false, false, false, false, false, false, false, false, 0,
        null, 0, null, null, null, null, null, null, null, null, null, null,
        null, null));
    ...
    // Log.Message(newPawn.Name + "   State: Dead - " + newPawn.health.Dead + "
    // Apparel Count: " + newPawn.apparel.WornApparel.Count());
    merc.pawn = newPawn;
  }
  ...
  public void createNewPawn(ref Mercenary merc, PawnKindDef race) {
    ...
    Pawn newPawn;
    newPawn = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
        raceChoice, FactionColonies.getPlayerColonyFaction(),
        PawnGenerationContext.NonPlayer, -1, false, false, false, false, false,
        true, 0, false, false, false, false, false, false, false, false, 0,
        null, 0, null, null, null, null, null, null, null, null, null, null,
        null, null));
  ...
  public void updateSquadStats(int level) {
    foreach (Mercenary merc in mercenaries) {
      merc.pawn.skills.GetSkill(SkillDefOf.Shooting).Level =
          Math.Min(level * 2, 20);
      merc.pawn.skills.GetSkill(SkillDefOf.Melee).Level =
          Math.Min(level * 2, 20);
      merc.pawn.skills.GetSkill(SkillDefOf.Medicine).Level =
          Math.Min(level * 1, 20);
    }
  }
  ...
  public void PassPawnToDeadMercenaries(Mercenary merc) {
    ...
    MilitaryCustomizationUtil util =
        Find.World.GetComponent<FactionFC>().militaryCustomizationUtil;
  ...
  public void OutfitSquad(MilSquadFC outfit) {
    ...
    foreach (MilUnitFC loadout in outfit.units) {
      if (mercenaries[count].pawn.kindDef != loadout.pawnKind ||
          mercenaries[count].pawn.Dead == true) {
        Mercenary pawn = new Mercenary(true);
      ...
      if (loadout != null) {
        ...
        if (mercenaries[count].loadout !=
            faction.militaryCustomizationUtil.blankUnit)
      ...
      if (mercenaries[count].pawn.equipment.AllEquipmentListForReading !=
          null) {
        UsedWeaponList.AddRange(
            mercenaries[count].pawn.equipment.AllEquipmentListForReading);
  ...
  public void EquipPawn(Mercenary merc, MilUnitFC loadout) {
    ...
    foreach (ThingWithComps weapon in loadout.defaultPawn.equipment
                 .AllEquipmentListForReading) {
      if (weapon.def.MadeFromStuff) {
        merc.pawn.equipment.AddEquipment(
            ThingMaker.MakeThing(weapon.def, weapon.Stuff) as ThingWithComps);
      } else {
        merc.pawn.equipment.AddEquipment(ThingMaker.MakeThing(weapon.def)
                                             as ThingWithComps);
      }
      ...
      if (FactionColonies.checkForMod("CETeam.CombatExtended")) {
        ...
        foreach (ThingComp comp in merc.pawn.AllComps) {
          if (comp.GetType().ToString() == "CombatExtended.CompInventory") {
            ...
            // Method not static, so create instance of object and define the
            // parameters to the method.
            var obj = Activator.CreateInstance(typ);
            object[] paramArgu =
                new object[]{merc.pawn.equipment.Primary, comp, 1};

            Traverse.Create(obj)
                .Method("TryGenerateAmmoFor", paramArgu)
                .GetValue();
            Traverse.Create(obj)
                .Method("LoadWeaponWithRandAmmo",
                        new object[]{merc.pawn.equipment.Primary})
                .GetValue();
          }
        }
      }
      ...
      foreach (ThingWithComps weapon in UsedWeaponList) {
        if (weapon.ParentHolder is Pawn_EquipmentTracker) {
          if ((((Pawn_EquipmentTracker) weapon.ParentHolder).pawn.Faction ==
              ...
              ((Pawn_EquipmentTracker) weapon.ParentHolder).pawn.Dead ==
                  false) {
  ...
  public void debugMercenarySquad() {
    foreach (Mercenary merc in mercenaries) {
      ...
      Log.Message(
          merc.pawn.equipment.AllEquipmentListForReading.Count().ToString());
      // Log.Message(pawn.Name + "   State: Dead - " + pawn.health.Dead + "
...
public class MilSquadFC : IExposable, ILoadReferenceable {
  ...
  public void newSquad() {
    ...
    for (int sq = 0; sq < 30; sq++) {
      units.Add(Find.World.GetComponent<FactionFC>()
                    .militaryCustomizationUtil.blankUnit);
    }
    updateEquipmentTotalCost();
  }
  ...
  public void deleteSquad() {
    Find.World.GetComponent<FactionFC>()
        .militaryCustomizationUtil.squads.Remove(this);
  }

  public string GetUniqueLoadID() { return "MilSquadFC_" + this.loadID; }
  ...
  public void setTraderCaravan(bool state) {
    ...
    if (state) {
      ...
      if (hasTrader == false) {
        Messages.Message(
            "There must be a trader in the squad to be a trader caravan!",
            MessageTypeDefOf.RejectInput);
...
class militaryCustomizationWindowFC : Window {
  ...
  public override void PreOpen() {
    ...
    settlementMaxScroll =
        (Find.World.GetComponent<FactionFC>().settlements.Count() *
             (settlementYSpacing + settlementHeight) -
  ...
  public void DrawHeaderTabs(Rect inRect) {
    ...
    Rect milSetSquad = new Rect(milDesigination.x + milDesigination.width,
                                milDesigination.y, 187, milDesigination.height);
    Rect milCreateSquad =
        new Rect(milSetSquad.x + milSetSquad.width, milDesigination.y, 187,
                 milDesigination.height);
    Rect milCreateUnit =
        new Rect(milCreateSquad.x + milCreateSquad.width, milDesigination.y,
                 187, milDesigination.height);
    Rect milCreateFiresupport =
        new Rect(milCreateUnit.x + milCreateUnit.width, milDesigination.y, 187,
                 milDesigination.height);
    ...
    if (Widgets.ButtonImage(helpButton, texLoad.questionmark)) {
      ...
      string description =
          "Need Help with this menu? Go to this youtube video: https://youtu.be/lvWb1rMMsq8";
  ...
  public void DrawTabAssign(Rect inRect) {
    ...
    Rect SettlementName =
        new Rect(SettlementBox.x + 5, SettlementBox.y + 5, 250, 25);
    Rect MilitaryLevel =
        new Rect(SettlementName.x, SettlementName.y + 30, 250, 25);
    Rect AssignedSquad =
        new Rect(MilitaryLevel.x, MilitaryLevel.y + 30, 250, 25);
    Rect isBusy = new Rect(AssignedSquad.x, AssignedSquad.y + 30, 250, 25);

    Rect buttonSetSquad = new Rect(SettlementBox.x + SettlementBox.width - 265,
                                   SettlementBox.y + 5, 100, 25);
    Rect buttonViewSquad =
        new Rect(buttonSetSquad.x, buttonSetSquad.y + 3 + buttonSetSquad.height,
                 buttonSetSquad.width, buttonSetSquad.height);
    Rect buttonDeploySquad = new Rect(
        buttonViewSquad.x, buttonViewSquad.y + 3 + buttonViewSquad.height,
        buttonSetSquad.width, buttonSetSquad.height);
    Rect buttonResetPawns = new Rect(
        buttonDeploySquad.x, buttonDeploySquad.y + 3 + buttonDeploySquad.height,
        buttonSetSquad.width, buttonSetSquad.height);
    Rect buttonOrderFireSupport =
        new Rect(buttonSetSquad.x + 125 + 5, SettlementBox.y + 5, 125, 25);
    ...
    foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
                 .settlements) {
      if (true) // count * (settlementYSpacing+settlementHeight) + scroll >= 0
                // && count * (settlementYSpacing + settlementHeight) + scroll <=
                // settlementWindowHeight)
      {
        Text.Font = GameFont.Small;

        Widgets.DrawMenuSection(new Rect(
            SettlementBox.x,
            SettlementBox.y +
                (SettlementBox.height + settlementYSpacing) * count + scroll,
        ...
        if (Widgets.ButtonTextSubtle(
                new Rect(SettlementName.x,
                         SettlementName.y +
                             (SettlementBox.height + settlementYSpacing) *
                                 count +
                             scroll,
                         SettlementName.width, SettlementName.height),
                settlement.name)) {
        ...
        Widgets.Label(
            new Rect(MilitaryLevel.x,
                     MilitaryLevel.y +
                         (SettlementBox.height + settlementYSpacing) * count +
                         scroll,
                     MilitaryLevel.width, MilitaryLevel.height * 2),
            "Mil Level: " + settlement.settlementMilitaryLevel +
                " - Max Squad Cost: " +
                FactionColonies.calculateMilitaryLevelPoints(
        ...
        if (settlement.militarySquad != null) {
          if (settlement.militarySquad.outfit != null) {
            Widgets.Label(
                ...
                "Assigned Squad: " +
                    settlement.militarySquad.outfit
                        .name); // settlement.militarySquad.name);
          } else {
            Widgets.Label(
                new Rect(AssignedSquad.x,
                         AssignedSquad.y +
        ...
        Widgets.Label(
            new Rect(isBusy.x,
                     isBusy.y +
                         (SettlementBox.height + settlementYSpacing) * count +
                         scroll,
                     isBusy.width, isBusy.height),
            "Available: " + (!settlement.isMilitaryBusySilent()));
        ...
        if (Widgets.ButtonText(
          ...
          foreach (MilSquadFC squad in util.squads) {
            Squads.Add(new FloatMenuOption(
                squad.name +
                    " - Total Equipment Cost: " + squad.equipmentTotalCost,
                delegate {
                  // Unit is selected
                ...
                new Rect(buttonViewSquad.x,
                         buttonViewSquad.y +
                             (SettlementBox.height + settlementYSpacing) *
                                 count +
                             scroll,
                         buttonViewSquad.width, buttonViewSquad.height),
                "View Squad")) {
          Messages.Message("This is currently not implemented.",
                           MessageTypeDefOf.RejectInput);
        }
        ...
        if (Widgets.ButtonText(
                new Rect(buttonDeploySquad.x,
                         buttonDeploySquad.y +
                             (SettlementBox.height + settlementYSpacing) *
                                 count +
                             scroll,
                         buttonDeploySquad.width, buttonDeploySquad.height),
                "Deploy Squad")) {
          if (!(settlement.isMilitaryBusy()) &&
              settlement.isMilitarySquadValid()) {
          ...
          } else if (settlement.isMilitaryBusy() &&
                     settlement.isMilitarySquadValid() &&
                     faction.hasPolicy(FCPolicyDefOf.militaristic)) {
            if ((faction.traitMilitaristicTickLastUsedExtraSquad +
                 GenDate.TicksPerDay * 5) <= Find.TickManager.TicksGame) {
              int cost = (int) Math.Round(
                  settlement.militarySquad.outfit.updateEquipmentTotalCost() *
                  .2);
              List<FloatMenuOption> options = new List<FloatMenuOption>();

              options.Add(new FloatMenuOption(
                  "Deploy Secondary Squad - $" + cost + " silver", delegate {
                    if (PaymentUtil.getSilver() >= cost) {
                      List<FloatMenuOption> deployment =
                      ...
                      // check if medieval only
                      bool medievalOnly =
                          LoadedModManager.GetMod<FactionColoniesMod>()
                              .GetSettings<FactionColonies>()
                              .medievalTechOnly;
                      if (!medievalOnly && Find.ResearchManager.GetProgress(DefDatabase<ResearchProjectDef>.GetNamed("TransportPod", false)) == DefDatabase<ResearchProjectDef>.GetNamed("TransportPod", false).baseCost)
                                    {
                        deployment.Add(
                            new FloatMenuOption("Drop-Pod", delegate {
                              FactionColonies.CallinAlliedForces(settlement,
                                                                 true, cost);
                              Find.WindowStack.currentlyDrawnWindow.Close();
                            }));
                      ...
                      Messages.Message(
                          "NotEnoughSilverToDeploySquad".Translate(),
                          MessageTypeDefOf.RejectInput);
                    }
                  }));
              ...
              Messages.Message(
                  TranslatorFormattedStringExtensions.Translate(
                      ...
                      Math.Round(
                          GenDate.TicksToDays(
                              (faction.traitMilitaristicTickLastUsedExtraSquad +
                               GenDate.TicksPerDay * 5) -
                ...
                new Rect(buttonResetPawns.x,
                         buttonResetPawns.y +
                             (SettlementBox.height + settlementYSpacing) *
                                 count +
                             scroll,
                         buttonResetPawns.width, buttonResetPawns.height),
                "Reset Pawns")) {
          FloatMenuOption confirm =
              new FloatMenuOption("Are you sure? Click to confirm", delegate {
                if (settlement.militarySquad != null) {
                  ...
                  Messages.Message(
                      "There is no pawns to reset. Assign a squad first.",
                      MessageTypeDefOf.RejectInput);
                }
              });
        ...
        if (Widgets.ButtonText(
                new Rect(buttonOrderFireSupport.x,
                         buttonOrderFireSupport.y +
                             (SettlementBox.height + settlementYSpacing) *
                                 count +
                             scroll,
                         buttonOrderFireSupport.width,
                         buttonOrderFireSupport.height),
                "Order Fire Support")) {
          List<FloatMenuOption> list = new List<FloatMenuOption>();
          ...
          foreach (MilitaryFireSupport support in util.fireSupportDefs) {
            ...
            FloatMenuOption option =
                new FloatMenuOption(support.name + " - $" + cost, delegate {
                  if (support.returnTotalCost() <=
                      FactionColonies.calculateMilitaryLevelPoints(
                          settlement.settlementMilitaryLevel)) {
                    if (settlement.buildings.Contains(
                            BuildingFCDefOf.artilleryOutpost)) {
                      if (settlement.artilleryTimer <=
                          Find.TickManager.TicksGame) {
                        if (PaymentUtil.getSilver() >= cost) {
                          FactionColonies.FireSupport(settlement, support);
                          Find.WindowStack.TryRemove(
                              typeof(militaryCustomizationWindowFC));
                        ...
                        Messages.Message(
                            "That firesupport option is on cooldown for another " +
                                GenDate.ToStringTicksToDays(
                                    settlement.artilleryTimer -
                                    Find.TickManager.TicksGame),
                            MessageTypeDefOf.RejectInput);
                      }
                    } else {
          ...
          if (list.Count() == 0) {
            list.Add(new FloatMenuOption(
                "No fire supports currently made. Make one", delegate{}));
          }
  ...
  public void DrawTabSquad(Rect inRect) {
    ...
    Rect DeleteButton =
        new Rect(ResetButton.x, ResetButton.y + ResetButton.height + 5,
                 ResetButton.width, ResetButton.height);
    Rect PointRefButton =
        new Rect(DeleteButton.x, DeleteButton.y + DeleteButton.height + 5,
                 DeleteButton.width, DeleteButton.height);

    // If squad is not selected
    if (Widgets.CustomButtonText(ref SelectionBar, selectedText, Color.gray,
    ...
    if (selectedSquad != null) {
      ...
      if (settlementPointReference != null) {
        Widgets.Label(
            EquipmentTotalCost,
            "Total Squad Equipment Cost: " +
                selectedSquad.equipmentTotalCost.ToString() + " / " +
                FactionColonies.calculateMilitaryLevelPoints(
                    settlementPointReference.settlementMilitaryLevel) +
                " (Max Cost)");
      } else {
        Widgets.Label(EquipmentTotalCost,
                      "Total Squad Equipment Cost: " +
                          selectedSquad.equipmentTotalCost.ToString() + " / " +
                          "No Reference");
      }
      ...
      Widgets.CheckboxLabeled(isTrader, "is Trader Caravan",
                              ref selectedSquad.isTraderCaravan);
      ...
      if (Widgets.ButtonText(PointRefButton, "Set Point Ref")) {
        ...
        foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>()
          ...
          settlementList.Add(new FloatMenuOption(
              settlement.name +
                  " - Military Level : " + settlement.settlementMilitaryLevel,
              delegate {
                // set points
      ...
      // for (int k = 0; k < 30; k++)
      //{
      //	Widgets.ButtonImage(new Rect(UnitStandBase.x + (k * 15),
      //UnitStandBase.y + ((k % 5) * 70), 50, 20), texLoad.unitCircle);
      //}
      ...
      for (int k = 0; k < 30; k++) {
        if (Widgets.ButtonImage(new Rect(UnitStandBase.x + ((k % 6) * 80),
          ...
          foreach (MilUnitFC unit in util.units) {
            Units.Add(new FloatMenuOption(
                unit.name + " - Cost: " + unit.equipmentTotalCost, delegate {
                  // Unit is selected
        ...
        if (selectedSquad.units[k].isBlank == false) {
          if (selectedSquad.units.ElementAt(k).animal != null) {
            Widgets.ButtonImage(
                new Rect(UnitStandBase.x + 15 + ((k % 6) * 80),
                         UnitStandBase.y - 45 + (k - k % 6) / 5 * 70, 60, 60),
                selectedSquad.units.ElementAt(k).animal.race.uiIcon);
          }
          Widgets.ThingIcon(
              new Rect(UnitStandBase.x - 5 + ((k % 6) * 80),
                       UnitStandBase.y - 45 + (k - k % 6) / 5 * 70, 60, 60),
              selectedSquad.units.ElementAt(k).defaultPawn);
          if (selectedSquad.units.ElementAt(k)
                  .defaultPawn.equipment.AllEquipmentListForReading.Count > 0) {
            Widgets.ThingIcon(
                new Rect(UnitStandBase.x - 5 + ((k % 6) * 80),
                         UnitStandBase.y - 15 + (k - k % 6) / 5 * 70, 40, 40),
                selectedSquad.units.ElementAt(k)
                    .defaultPawn.equipment.AllEquipmentListForReading[0]);
          }
          Widgets.Label(new Rect(UnitStandBase.x - 15 + ((k % 6) * 80),
                                 UnitStandBase.y - 65 + (k - k % 6) / 5 * 70,
                                 80, 60),
                        selectedSquad.units.ElementAt(k).name);
        }
      }
  ...
  public void DrawTabUnit(Rect inRect) {
    ...
    Rect isTrader = new Rect(isCivilian.x, isCivilian.y + isCivilian.height + 5,
                             isCivilian.width, isCivilian.height);
    ...
    Rect RollNewPawn =
        new Rect(DeleteButton.x, DeleteButton.y + DeleteButton.height + 5,
                 DeleteButton.width, DeleteButton.height);
    Rect ChangeRace =
        new Rect(325, ResetButton.y, RollNewPawn.width, RollNewPawn.height);
    // Rect ApparelTorso
    ...
    // if unit is not selected
    Widgets.Label(new Rect(new Vector2(ApparelHead.x, ApparelHead.y - 15),
                           ApparelHead.size),
                  "Head");
    Widgets.DrawMenuSection(ApparelHead);
    Widgets.Label(
        new Rect(new Vector2(ApparelTorsoSkin.x, ApparelTorsoSkin.y - 15),
                 ApparelTorsoSkin.size),
        "Shirt");
    Widgets.DrawMenuSection(ApparelTorsoSkin);
    Widgets.Label(
        new Rect(new Vector2(ApparelTorsoMiddle.x, ApparelTorsoMiddle.y - 15),
                 ApparelTorsoMiddle.size),
        "Chest");
    Widgets.DrawMenuSection(ApparelTorsoMiddle);
    Widgets.Label(
        new Rect(new Vector2(ApparelTorsoShell.x, ApparelTorsoShell.y - 15),
                 ApparelTorsoShell.size),
        "Over");
    Widgets.DrawMenuSection(ApparelTorsoShell);
    Widgets.Label(new Rect(new Vector2(ApparelBelt.x, ApparelBelt.y - 15),
                           ApparelBelt.size),
                  "Belt");
    Widgets.DrawMenuSection(ApparelBelt);
    Widgets.Label(new Rect(new Vector2(ApparelLegs.x, ApparelLegs.y - 15),
                           ApparelLegs.size),
                  "Pants");
    Widgets.DrawMenuSection(ApparelLegs);
    Widgets.Label(
        new Rect(new Vector2(EquipmentWeapon.x, EquipmentWeapon.y - 15),
                 EquipmentWeapon.size),
        "Weapon");
    Widgets.DrawMenuSection(EquipmentWeapon);
    Widgets.Label(
        new Rect(new Vector2(AnimalCompanion.x, AnimalCompanion.y - 15),
                 AnimalCompanion.size),
        "Animal");
    ...
    if (selectedUnit != null) {
      ...
      if (Widgets.ButtonText(ChangeRace, "Change Race")) {
        ...
        foreach (
            PawnKindDef def in DefDatabase<PawnKindDef>.AllDefsListForReading) {
          if (def.race.race.intelligence == Intelligence.Humanlike &
                  races.Contains(def.race.label) == false &&
            ...
            if (def.race.label == "Human" && def.LabelCap != "Colonist") {
              ...
              options.Add(new FloatMenuOption(
                  def.race.label.CapitalizeFirst() + " - Cost: " +
                      Math.Floor(def.race.BaseMarketValue *
                                 FactionColonies.militaryRaceCostMultiplier),
                  delegate {
      ...
      Widgets.CheckboxLabeled(isCivilian, "is Civilian",
                              ref selectedUnit.isCivilian);
      ...
      if (Widgets.ButtonInvisible(AnimalCompanion)) {
        ...
        foreach (PawnKindDef animal in DefDatabase<PawnKindDef>.AllDefs) {
          if (animal.RaceProps.IsFlesh == true &&
              animal.race.race.Animal == true &&
      ...
      if (Widgets.ButtonInvisible(EquipmentWeapon)) {
        ...
        foreach (ThingDef thing in DefDatabase<ThingDef>.AllDefs) {
          if (thing.IsWeapon == true && thing.BaseMarketValue != 0 &&
              ...
              list.Add(new FloatMenuOption(
                  ...
                  delegate {
                    if (thing.MadeFromStuff == true) {
                      // If made from stuff
                      List<FloatMenuOption> stuffList =
                          new List<FloatMenuOption>();
                      foreach (
                          ThingDef stuff in DefDatabase<ThingDef>.AllDefs) {
                        if (stuff.IsStuff == true &&
                            thing.stuffCategories.SharesElementWith(
                                stuff.stuffProps.categories) == true) {
                          stuffList.Add(new FloatMenuOption(
                              stuff.LabelCap + " - Total Value: " +
                                  (StatWorker_MarketValue
                                       .CalculatedBaseMarketValue(thing,
                                                                  stuff)),
                              delegate {
                                selectedUnit.equipWeapon(ThingMaker.MakeThing(
                                    thing, stuff) as ThingWithComps);
                              }));
                        }
                      }
                      ...
                      selectedUnit.equipWeapon(ThingMaker.MakeThing(thing)
                                                   as ThingWithComps);
                    }
                  },
                  thing));
      ...
      if (Widgets.ButtonInvisible(ApparelHead)) {
              ...
              headgearList.Add(new FloatMenuOption(
                  ...
                  delegate {
                    if (thing.MadeFromStuff == true) {
                          ...
                          stuffList.Add(new FloatMenuOption(
                              ...
                              delegate {
                                selectedUnit.wearEquipment(
                                    ThingMaker.MakeThing(thing, stuff)
                                        as Apparel,
                                    true);
                              }));
                        }
                      ...
                      selectedUnit.wearEquipment(
                          ThingMaker.MakeThing(thing) as Apparel, true);
                    }
                  },
                  thing));
            }
        ...
        headgearList.Insert(0, new FloatMenuOption("Unequip", delegate {
                              // Remove old
                              foreach (Apparel apparel in selectedUnit
                                           .defaultPawn.apparel.WornApparel) {
                                if (apparel.def.apparel.layers.Contains(
                                        ApparelLayerDefOf.Overhead) == true) {
                                  selectedUnit.defaultPawn.apparel.Remove(
                                      apparel);
                                  break;
                                }
                              }
      ...
      if (Widgets.ButtonInvisible(ApparelTorsoShell)) {
        ...
        foreach (ThingDef thing in DefDatabase<ThingDef>.AllDefs) {
          if (thing.IsApparel == true) {
            if (thing.apparel.layers.Contains(ApparelLayerDefOf.Shell) ==
                    true &&
                thing.apparel.bodyPartGroups.Contains(
                    BodyPartGroupDefOf.Torso) == true &&
        ...
        list.Insert(0, new FloatMenuOption("Unequip", delegate {
                      // Remove old
                      foreach (Apparel apparel in selectedUnit.defaultPawn
                                   .apparel.WornApparel) {
                        if (apparel.def.apparel.layers.Contains(
                                ApparelLayerDefOf.Shell) == true &&
      ...
      if (Widgets.ButtonInvisible(ApparelLegs)) {
        ...
        foreach (ThingDef thing in DefDatabase<ThingDef>.AllDefs) {
          if (thing.IsApparel == true) {
            if (thing.apparel.bodyPartGroups.Contains(
                    BodyPartGroupDefOf.Legs) == true &&
        ...
        list.Insert(0, new FloatMenuOption("Unequip", delegate {
                      ...
                      foreach (Apparel apparel in selectedUnit.defaultPawn
                                   .apparel.WornApparel) {
                        if (apparel.def.apparel.bodyPartGroups.Contains(
                                BodyPartGroupDefOf.Legs) == true &&
                            apparel.def.apparel.layers.Contains(
                                ApparelLayerDefOf.OnSkin)) // CHANGE THIS
                        {
                          selectedUnit.defaultPawn.apparel.Remove(apparel);
                          break;
                        }
      ...
      if (Widgets.ButtonInvisible(ApparelBelt)) {
        ...
        foreach (ThingDef thing in DefDatabase<ThingDef>.AllDefs) {
          if (thing.IsApparel == true) {
            if (thing.apparel.layers.Contains(ApparelLayerDefOf.Belt) == true &&
                FactionColonies.canCraftItem(thing)) {
              list.Add(new FloatMenuOption(
                  thing.LabelCap + " - Cost: " + thing.BaseMarketValue,
                  delegate {
      ...
      totalCost +=
          (float) Math.Floor(selectedUnit.defaultPawn.def.BaseMarketValue *
                             FactionColonies.militaryRaceCostMultiplier);

      foreach (Thing thing in selectedUnit.defaultPawn.apparel.WornApparel
                   .Concat(selectedUnit.defaultPawn.equipment
                               .AllEquipmentListForReading)) {
        Rect tmp = new Rect(ApparelWornItems.x, ApparelWornItems.y + i * 25,
                            ApparelWornItems.width, 25);
        i++;

        totalCost += thing.MarketValue;

        if (Widgets.CustomButtonText(
                ref tmp, thing.LabelCap + " Cost: " + thing.MarketValue,
      ...
      if (selectedUnit.animal != null) {
        ...
        totalCost +=
            (float) Math.Floor(selectedUnit.animal.race.BaseMarketValue *
                               FactionColonies.militaryAnimalCostMultiplier);
      }
      ...
      foreach (Thing thing in selectedUnit.defaultPawn.apparel.WornApparel) {
        ...
        if (thing.def.apparel.layers.Contains(ApparelLayerDefOf.Shell) &&
            thing.def.apparel.bodyPartGroups.Contains(
                BodyPartGroupDefOf.Torso)) {
        ...
        if (thing.def.apparel.bodyPartGroups.Contains(
      ...
      foreach (Thing thing in selectedUnit.defaultPawn.equipment
                   .AllEquipmentListForReading) {
  ...
  public void DrawTabFireSupport(Rect inRect) {
    ...
    Rect SelectionBar = new Rect(5, 45, 200, 30);
    Rect nameTextField = new Rect(5, 90, 250, 30);
    Rect floatRangeAccuracyLabel =
        new Rect(nameTextField.x, nameTextField.y + nameTextField.height + 5,
                 nameTextField.width, (float)(nameTextField.height * 1.5));
    Rect floatRangeAccuracy =
        new Rect(floatRangeAccuracyLabel.x,
                 floatRangeAccuracyLabel.y + floatRangeAccuracyLabel.height + 5,
                 floatRangeAccuracyLabel.width, floatRangeAccuracyLabel.height);

    Rect UnitStandBase = new Rect(140, 200, 50, 30);
    Rect TotalCost = new Rect(325, 50, 450, 20);
    Rect numberProjectiles =
        new Rect(TotalCost.x, TotalCost.y + TotalCost.height + 5,
                 TotalCost.width, TotalCost.height);
    Rect duration = new Rect(numberProjectiles.x,
                             numberProjectiles.y + numberProjectiles.height + 5,
    ...
    if (Widgets.CustomButtonText(ref SelectionBar, selectedText, Color.gray,
      ...
      supports.Add(new FloatMenuOption("Create New Fire Support", delegate {
        ...
        newFireSupport.name =
            "New Fire Support " + (util.fireSupportDefs.Count() + 1);
    ...
    if (selectedSupport != null) {
      ...
      if (settlementPointReference != null) {
        Widgets.Label(
            TotalCost,
            "Total Fire Support Silver Cost: " +
                selectedSupport.returnTotalCost() + " / " +
                FactionColonies.calculateMilitaryLevelPoints(
                    settlementPointReference.settlementMilitaryLevel) +
                " (Max Cost)");
      } else {
        Widgets.Label(TotalCost, "Total Fire Support Silver Cost: " +
                                     selectedSupport.returnTotalCost() + " / " +
                                     "No Reference");
      }
      Widgets.Label(numberProjectiles,
                    "Number of Projectiles: " +
                        selectedSupport.projectiles.Count().ToString());
      Widgets.Label(
          duration,
          "Duration of fire support: " +
              Math.Round((double) selectedSupport.projectiles.Count() * .25, 2)
                  .ToString() +
              " seconds");
      Widgets.Label(
          floatRangeAccuracyLabel,
      ...
      selectedSupport.accuracy = Widgets.HorizontalSlider(
          floatRangeAccuracy, selectedSupport.accuracy,
          Math.Max(3, (15 - Find.World.GetComponent<FactionFC>()
                                .returnHighestMilitaryLevel())),
          30, roundTo : 1);
      Text.Font = GameFont.Tiny;
      Text.Anchor = TextAnchor.UpperCenter;
      ...
      selectedSupport.name =
          Widgets.TextField(nameTextField, selectedSupport.name);
  ...
  public Rect deriveRectRow(Rect rect, float x, float y = 0, float width = 0,
                            float height = 0) {
    float inputWidth;
    float inputHeight;
    if (width == 0) {
    ...
    Rect newRect =
        new Rect(rect.x + rect.width + x, rect.y + y, inputWidth, inputHeight);
  ...
  public void DrawFireSupportBox(float x, float y, float rowHeight) {
    ...
    for (int i = 0; i <= selectedSupport.projectiles.Count(); i++) {
      // Declare Rects
      Rect text = new Rect(x + 2, y + 2 + i * rowHeight + scroll, rowHeight - 4,
                           rowHeight - 4);
      ...
      Rect upArrow =
          deriveRectRow(options, 12, 0, rowHeight - 4, rowHeight - 4);
      ...
      Rect box = new Rect(x, y + i * rowHeight + scroll,
                          delete.x + delete.width + 4 - x, rowHeight);

      Widgets.DrawHighlight(box);
      Widgets.DrawMenuSection(box);

      if (i == selectedSupport.projectiles.Count()) { // If on last row
        Text.Anchor = TextAnchor.MiddleCenter;
        Widgets.Label(text, i.ToString());
        if (Widgets.ButtonTextSubtle(
                icon, "Add new projectile")) { // if creating new projectile
        ...
        Widgets.Label(
            ...
            "$ " +
                (Math.Round(
                     selectedSupport.projectiles[i].BaseMarketValue * 1.5, 2))
                    .ToString()); // ADD in future mod setting for firesupport
                                  // cost

        Widgets.DefLabelWithIcon(icon, selectedSupport.projectiles[i]);
        if (Widgets.ButtonTextSubtle(options,
                                     "Options")) { // If clicked options button
          int k = i;
          List<FloatMenuOption> listOptions = new List<FloatMenuOption>();
          listOptions.Add(
              new FloatMenuOption("Insert Projectile Above Slot", delegate {
                List<FloatMenuOption> thingOptions =
                    new List<FloatMenuOption>();
                foreach (ThingDef def in selectedSupport
                             .returnFireSupportOptions()) {
                  thingOptions.Add(new FloatMenuOption(
                      def.LabelCap + " - " +
                          Math.Round(def.BaseMarketValue * 1.5, 2).ToString(),
                      delegate {
        ...
        if (Widgets.ButtonTextSubtle(upArrow, "")) { // if click up arrow button
          if (i != 0) {
        ...
        if (Widgets.ButtonTextSubtle(delete, "")) { // if click delete  button
