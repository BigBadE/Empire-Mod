...
public class FCOptionWindow : Window {
  ...
  public override Vector2 InitialSize {
    get { return new Vector2(580f, 580f); }
  }
  ...
  public FCOptionWindow(FCEventDef evt, FCEvent parentEvent) {
    this.forcePause =
        !FactionColonies.Settings().disableForcedPausingDuringEvents;
    this.draggable = true;
    this.doCloseX = false;
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    for (int i = 0; i < options.Count(); i++) {
      if (Widgets.ButtonTextSubtle(new Rect(xoffset,
                                            yoffset + (i * (height + yspacing)),
                                            length, height),
                                   "")) {
        if (PaymentUtil.getSilver() >= options[i].silverCost) {
          ...
          Messages.Message(
              "You do not have enough silver on the map to pay for that option",
              MessageTypeDefOf.RejectInput);
        }
      }

      // label
      Widgets.Label(new Rect(xoffset + 5,
                             yoffset + 5 + (i * (height + yspacing)),
                             length - 10, height - 10),
                    options[i].label);
    }
