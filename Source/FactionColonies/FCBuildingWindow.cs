...
class FCBuildingWindow : Window {
  ...
  public override Vector2 InitialSize {
    get { return new Vector2(436f, 536f); }
  }
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    foreach (BuildingFCDef building in buildingList) {
      newBuildingWindow =
          new Rect(BaseBuildingWindow.x,
                   BaseBuildingWindow.y + (i * (rowHeight)) + scroll,
                   BaseBuildingWindow.width, BaseBuildingWindow.height);
      newBuildingIcon = new Rect(
          BaseBuildingIcon.x, BaseBuildingIcon.y + (i * (rowHeight)) + scroll,
          BaseBuildingIcon.width, BaseBuildingIcon.height);
      newBuildingLabel = new Rect(
          BaseBuildingLabel.x, BaseBuildingLabel.y + (i * (rowHeight)) + scroll,
          BaseBuildingLabel.width, BaseBuildingLabel.height);
      newBuildingDesc = new Rect(
          BaseBuildingDesc.x, BaseBuildingDesc.y + (i * (rowHeight)) + scroll,
          BaseBuildingDesc.width, BaseBuildingDesc.height);
      ...
      if (Widgets.ButtonInvisible(newBuildingWindow)) {
        ...
        if (building == buildingDef) {
          ...
          list.Add(new FloatMenuOption("Build".Translate(), delegate {
            if (settlement.validConstructBuilding(building, buildingSlot,
                                                  settlement)) {
              ...
              settlement.deconstructBuilding(buildingSlot);
              Messages.Message(
                  building.label + " " + "WillBeConstructedIn".Translate() +
                      " " +
                      GenDate.ToStringTicksToDays(tmpEvt.timeTillTrigger -
    ...
    Widgets.Label(
        new Rect(TopName.x + 5, TopName.y, TopName.width, TopName.height),
        buildingDef.LabelCap);

    Widgets.DrawMenuSection(new Rect(TopDescription.x - 5, TopDescription.y - 5,
                                     TopDescription.width,
                                     TopDescription.height));
    Text.Font = GameFont.Small;
  ...
  public FCBuildingWindow(SettlementFC settlement, int buildingSlot) {
    ...
    foreach (
        BuildingFCDef building in DefDatabase<BuildingFCDef>.AllDefsListForReading) {
