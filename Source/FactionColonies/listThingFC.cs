...
public class listThingFC : Window {
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    List<String> headerList = new List<String>(){
        "Name".Translate(), "StackCount".Translate(), "MarketValue".Translate(),
        "FCTotalValue".Translate()};
    for (int i = 0; i < 4; i++) //-2 to exclude location and ID
    {
      if (i == 0) {
        Widgets.Label(new Rect(xoffset + 2 + i * xspacing, yoffset - yspacing,
                               xspacing + headerSpacing, yspacing),
                      headerList[i]);
      } else {
        Widgets.Label(new Rect(xoffset + headerSpacing + 2 + i * xspacing,
                               yoffset - yspacing, xspacing, yspacing),
                      headerList[i]);
      }
    }
      ...
      if (i * yspacing + scroll >= 0 && i * yspacing + scroll <= height) {
        if (i % 2 == 0) {
          Widgets.DrawHighlight(new Rect(
              xoffset, yoffset + i * yspacing + scroll, length, yspacing));
        }
        for (int k = 0; k < 4; k++) // Browse through thing information
        {
          if (k == 0) // name of thing
          {
            ...
            Widgets.Label(new Rect(xoffset + 2 + k * xspacing,
                                   yoffset + i * yspacing + scroll,
                                   xspacing + headerSpacing, yspacing),
                          list[i].def.label); // timedue is date made
          } else if (k == 1)                  // number of thing
          {
            Widgets.Label(new Rect(xoffset + headerSpacing + 2 + k * xspacing,
                                   yoffset + i * yspacing + scroll, xspacing,
                                   yspacing),
                          list[i].stackCount.ToString());
          } else if (k == 2) // Base market value
          {
            Widgets.Label(new Rect(xoffset + headerSpacing + 2 + k * xspacing,
                                   yoffset + i * yspacing + scroll, xspacing,
                                   yspacing),
                          list[i].MarketValue.ToString());
          } else if (k == 3) // Value of thing
          {
            Widgets.Label(
                new Rect(xoffset + headerSpacing + 2 + k * xspacing,
                         yoffset + i * yspacing + scroll, xspacing, yspacing),
                "CashSymbol".Translate() +
                    (list[i].stackCount * list[i].MarketValue).ToString());
          } else // Catch all
          {
            Widgets.Label(new Rect(xoffset + headerSpacing + 2 + k * xspacing,
                                   yoffset + i * yspacing + scroll, xspacing,
                                   yspacing),
                          "REPORT THIS - listThingFC");
          }
        }
      }
    }
