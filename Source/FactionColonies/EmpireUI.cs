...
public class EmpireUIMercenaryCommandMenu : Window {
  ...
  public override Vector2 InitialSize {
    get { return new Vector2(200f, 200f); }
  }

  protected override void SetInitialSizeAndPosition() {
    this.windowRect = new Rect((float) UI.screenWidth - this.InitialSize.x, 0f,
  ...
  public override void DoWindowContents(Rect rect) {
    ...
    if (selectedSquad == null) {
      ...
      squadText =
          selectedSquad.getSettlement.name + "'s " + selectedSquad.outfit.name;
    }

    // Select a squad
    if (Widgets.ButtonText(selectSquad, squadText)) {
      List<FloatMenuOption> list = new List<FloatMenuOption>();
      foreach (MercenarySquadFC squad in faction.militaryCustomizationUtil
                   .DeployedSquads) {
        if (squad.getSettlement != null) {
          list.Add(new FloatMenuOption(squad.getSettlement.name + "'s " +
                                           squad.outfit.name,
                                       delegate { selectedSquad = squad; }));
        }
      }
      if (list.Count() == 0) {
    ...
    if (Widgets.ButtonTextSubtle(commandAttack, "Attack")) {
      if (selectedSquad != null) {
        ...
        Messages.Message(selectedSquad.outfit.name +
                             " are now charging the enemy.",
    ...
    if (Widgets.ButtonTextSubtle(commandMove, "Move")) {
      if (selectedSquad != null) {
        ...
        tool = new DebugTool("Select Move Position", delegate() {
          ...
          selectedSquad.orderLocation = Position;
          Messages.Message(selectedSquad.outfit.name +
                               " are moving to the position and standing by.",
                           MessageTypeDefOf.NeutralEvent);
