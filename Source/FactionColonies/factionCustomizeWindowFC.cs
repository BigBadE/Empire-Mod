...
public class factionCustomizeWindowFC : Window {
  ...
  Rect buttonMilitaristic =
      new Rect((float)(600 + 60 * Math.Cos(0 * Math.PI)),
               (float)(100 + 60 * Math.Sin(0 * Math.PI)), 30, 30);
  Rect buttonAuthoritarian =
      new Rect((float)(600 + 60 * Math.Cos(0.25 * Math.PI)),
               (float)(100 + 60 * Math.Sin(0.25 * Math.PI)), 30, 30);
  Rect buttonIsolationist =
      new Rect((float)(600 + 60 * Math.Cos(0.5 * Math.PI)),
               (float)(100 + 60 * Math.Sin(0.5 * Math.PI)), 30, 30);
  Rect buttonFeudal =
      new Rect((float)(600 + 60 * Math.Cos(0.75 * Math.PI)),
               (float)(100 + 60 * Math.Sin(0.75 * Math.PI)), 30, 30);
  Rect buttonPacifist =
      new Rect((float)(600 + 60 * Math.Cos(1 * Math.PI)),
               (float)(100 + 60 * Math.Sin(1 * Math.PI)), 30, 30);
  Rect buttonEgalitarian =
      new Rect((float)(600 + 60 * Math.Cos(1.25 * Math.PI)),
               (float)(100 + 60 * Math.Sin(1.25 * Math.PI)), 30, 30);
  Rect buttonExpansionist =
      new Rect((float)(600 + 60 * Math.Cos(1.5 * Math.PI)),
               (float)(100 + 60 * Math.Sin(1.5 * Math.PI)), 30, 30);
  Rect buttonTechnocrat =
      new Rect((float)(600 + 60 * Math.Cos(1.75 * Math.PI)),
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    if (Widgets.ButtonTextSubtle(buttonAllowedRaces,
                                 "AllowedRaces".Translate())) {
      List<string> races = new List<string>();
      List<FloatMenuOption> list = new List<FloatMenuOption>();
      list.Add(new FloatMenuOption("Enable All",
                                   delegate { faction.resetRaceFilter(); }));
      foreach (
        ...
        if (def.race.race.intelligence == Intelligence.Humanlike &
          ...
          if (def.race.label == "Human" && def.LabelCap != "Colonist") {
            ...
            list.Add(new FloatMenuOption(
                def.race.label.CapitalizeFirst() +
                    " - Allowed: " + faction.raceFilter.Allows(def.race),
                delegate {
                  if (faction.raceFilter.AllowedThingDefs.Count() == 1 &&
                      faction.raceFilter.Allows(def.race) == true) {
                    Messages.Message("CannotHaveLessThanOneRace".Translate(),
                                     MessageTypeDefOf.RejectInput);
                  } else if (faction.raceFilter.AllowedThingDefs.Count() > 1) {

                    faction.raceFilter.SetAllow(
                        def.race, !faction.raceFilter.Allows(def.race));
                  } else {
                    Log.Message(
                        "Empire Error - Zero races available for faction - Report this");
    ...
    if (Widgets.ButtonText(buttonConfirm, "ConfirmChanges".Translate())) {
      ...
      faction.updateFactionIcon(ref fact,
                                "FactionIcons/" + tempFactionIconPath);

      if (!traitsChosen) {
        // check each trait bool. If true and does not exist already, add to
        // factionfc
        if (boolMilitaristic)
      ...
      Find.LetterStack.ReceiveLetter(
          "Note on Faction Icon",
          "Note: The faction icon on the world map will only update after a full restart of your game. Or pure luck.",
    ...
    if (buttonMilitaristic.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonMilitaristic,
                               returnPolicyText(FCPolicyDefOf.militaristic));
    }
    if (Widgets.ButtonImage(buttonMilitaristic, icon)) {
      if (numberTraitsSelected <= 1 || boolMilitaristic == true) {
        ...
        if (boolPacifist == false) {
          ...
          Messages.Message("FCConflictingTraits".Translate(),
                           MessageTypeDefOf.RejectInput);
        }
      } else {
        Messages.Message("FCUnselectTrait".Translate(),
                         MessageTypeDefOf.RejectInput);
      }
    }
    ...
    if (buttonAuthoritarian.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonAuthoritarian,
                               returnPolicyText(FCPolicyDefOf.authoritarian));
    }
    ...
    if (buttonIsolationist.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonIsolationist,
                               returnPolicyText(FCPolicyDefOf.isolationist));
    }
    ...
    if (buttonFeudal.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonFeudal,
                               returnPolicyText(FCPolicyDefOf.feudal));
    }
    ...
    if (buttonPacifist.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonPacifist,
                               returnPolicyText(FCPolicyDefOf.pacifist));
    }
    ...
    if (buttonEgalitarian.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonEgalitarian,
                               returnPolicyText(FCPolicyDefOf.egalitarian));
    }
    ...
    if (buttonExpansionist.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonExpansionist,
                               returnPolicyText(FCPolicyDefOf.expansionist));
    }
    ...
    if (buttonTechnocrat.Contains(Event.current.mousePosition)) {
      TooltipHandler.TipRegion(buttonTechnocrat,
                               returnPolicyText(FCPolicyDefOf.technocratic));
    }
