...
public class FCPrisoner : ILoadReferenceable, IExposable {
  ...
  public FCPrisoner(Pawn pawn, SettlementFC settlement) {
    ...
    this.health = (float) Math.Round(
        this.prisoner.health.summaryHealth.SummaryHealthPercent * 100);
  ...
  public void ExposeData() {
    ...
    Scribe_Deep.Look<Pawn_HealthTracker>(ref healthTracker, "healthTracker",
                                         new object[]{this.prisoner});
  }

  public string GetUniqueLoadID() { return "FCPrisoner_" + this.loadID; }
  ...
  public bool checkDead() {
    if (this.health <= 0) {
      ...
      Find.LetterStack.ReceiveLetter(
          "PrisonerHasDiedLetter".Translate(),
          TranslatorFormattedStringExtensions.Translate(
              "PrisonerHasDied", this.prisoner.Name.ToString(),
