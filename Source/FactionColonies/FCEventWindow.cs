...
class FCEventWindow : Window {
  ...
  public FCEventWindow() {
    ...
    eventLocationBase =
        new Rect(eventDescBase.x + eventDescBase.width, 0, 100, eventHeight);
    // rect time time remaining
    eventTimeRemaining = new Rect(eventLocationBase.x + eventLocationBase.width,
                                  0, 140, eventHeight);
  }
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    foreach (FCEvent evt in events) {
      ...
      //
      if (Widgets.ButtonText(location,
                             "Location".Translate().CapitalizeFirst())) {
        if (evt.hasDestination == true) {
          Find.WindowStack.Add(
              new settlementWindowFC(faction.returnSettlementByLocation(
                  evt.location, evt.planetName)));
        } else {
      ...
      //
      Widgets.Label(time,
                    GenDate.ToStringTicksToDays(evt.timeTillTrigger -
