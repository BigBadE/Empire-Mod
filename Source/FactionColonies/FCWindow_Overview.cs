...
public class FCWindow_Overview : Window {
  ...
  public FCWindow_Overview() {
    ...
    maxScroll =
        (int)((faction.settlements.Count() * 25) - settlementsBox.height);
  }
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    Widgets.FillableBar(progressBarLevelUp,
                        faction.factionXPCurrent / faction.factionXPGoal,
                        foreground, background, true);
    Widgets.DrawShadowAround(progressBarLevelUp);
    Text.Font = GameFont.Tiny;
    Text.Anchor = TextAnchor.MiddleCenter;
    Widgets.Label(progressBarLevelUp, Math.Round(faction.factionXPCurrent) +
                                          "/" + faction.factionXPGoal);
    Text.Anchor = TextAnchor.MiddleLeft;
    Widgets.DrawBox(progressBarLevelUp);

    Text.Font = GameFont.Small;
    Widgets.Label(factionLevel, TranslatorFormattedStringExtensions.Translate(
                                    "FCLevel", faction.factionLevel));
    ...
    if (faction.policies.Count() == 2) {
      ...
      if (policy_1.Contains(Event.current.mousePosition)) {
        TooltipHandler.TipRegion(policy_1,
                                 returnPolicyText(faction.policies[0].def));
      }
      ...
      if (policy_2.Contains(Event.current.mousePosition)) {
        TooltipHandler.TipRegion(policy_2,
                                 returnPolicyText(faction.policies[1].def));
      }
    } else {
      Widgets.Label(new Rect(policy_1.x, policy_1.y, 200, 50),
    ...
    if (Widgets.ButtonTextSubtle(
            trait_1, returnTraitAvailibility(
                         1, faction.factionTraits[0]))) // CHANGE THIS
    {
          ...
          list.Add(new FloatMenuOption(trait.label, delegate
                    {
                        ...
                        confirm.Add(new FloatMenuOption(TranslatorFormattedStringExtensions.Translate("FCConfirmTrait", trait.label), delegate {
                            faction.factionTraits[0] = new FCPolicy(trait);
                        }));
                        Find.WindowStack.Add(new FloatMenu(confirm));

                    }, mouseoverGuiAction: delegate {
                        TooltipHandler.TipRegion(new Rect(Event.current.mousePosition,new Vector2(100,200)), returnPolicyText(trait));
                    }));
        }
        Find.WindowStack.Add(new FloatMenu(list));
      }
    }
    if (Mouse.IsOver(trait_1) &&
        faction.factionTraits[0].def != FCPolicyDefOf.empty) {
      TooltipHandler.TipRegion(
          new Rect(Event.current.mousePosition, new Vector2(100, 200)),
    ...
    // TraitSlot
    if (Widgets.ButtonTextSubtle(
            trait_2, returnTraitAvailibility(
                         2, faction.factionTraits[1]))) // CHANGE THIS
    {
          ...
          list.Add(new FloatMenuOption(trait.label, delegate
                    {
                        List<FloatMenuOption> confirm = new List<FloatMenuOption>();
                        confirm.Add(new FloatMenuOption(TranslatorFormattedStringExtensions.Translate("FCConfirmTrait", trait.label), delegate {
                            faction.factionTraits[1] = new FCPolicy(trait);
                        }));
                        Find.WindowStack.Add(new FloatMenu(confirm));

                    }, mouseoverGuiAction: delegate {
                        TooltipHandler.TipRegion(new Rect(Event.current.mousePosition, new Vector2(200, 200)), returnPolicyText(trait));
                    }));
        }
        Find.WindowStack.Add(new FloatMenu(list));
      }
    }
    if (Mouse.IsOver(trait_2) &&
        faction.factionTraits[1].def != FCPolicyDefOf.empty) {
      TooltipHandler.TipRegion(
          new Rect(Event.current.mousePosition, new Vector2(200, 200)),
    ...
    // TraitSlot
    if (Widgets.ButtonTextSubtle(
            trait_3, returnTraitAvailibility(
                         3, faction.factionTraits[2]))) // CHANGE THIS
    {
          ...
          list.Add(new FloatMenuOption(trait.label, delegate
                    {
                        List<FloatMenuOption> confirm = new List<FloatMenuOption>();
                        confirm.Add(new FloatMenuOption(TranslatorFormattedStringExtensions.Translate("FCConfirmTrait", trait.label), delegate {
                            faction.factionTraits[2] = new FCPolicy(trait);
                        }));
                        Find.WindowStack.Add(new FloatMenu(confirm));

                    }, mouseoverGuiAction: delegate {
                        TooltipHandler.TipRegion(new Rect(Event.current.mousePosition, new Vector2(300, 200)), returnPolicyText(trait));
                    }));
        }
        Find.WindowStack.Add(new FloatMenu(list));
      }
    }
    if (Mouse.IsOver(trait_3) &&
        faction.factionTraits[2].def != FCPolicyDefOf.empty) {
      TooltipHandler.TipRegion(
          new Rect(Event.current.mousePosition, new Vector2(300, 200)),
    ...
    // TraitSlot
    if (Widgets.ButtonTextSubtle(
            trait_4, returnTraitAvailibility(
                         4, faction.factionTraits[3]))) // CHANGE THIS
    {
          ...
          list.Add(new FloatMenuOption(trait.label, delegate
                    {
                        List<FloatMenuOption> confirm = new List<FloatMenuOption>();
                        confirm.Add(new FloatMenuOption(TranslatorFormattedStringExtensions.Translate("FCConfirmTrait", trait.label), delegate {
                            faction.factionTraits[3] = new FCPolicy(trait);
                        }));
                        Find.WindowStack.Add(new FloatMenu(confirm));

                    }, mouseoverGuiAction: delegate {
                        TooltipHandler.TipRegion(new Rect(Event.current.mousePosition, new Vector2(400, 200)), returnPolicyText(trait));
                    }));
        }
        Find.WindowStack.Add(new FloatMenu(list));
      }
    }
    if (Mouse.IsOver(trait_4) &&
        faction.factionTraits[3].def != FCPolicyDefOf.empty) {
      TooltipHandler.TipRegion(
          new Rect(Event.current.mousePosition, new Vector2(400, 200)),
    ...
    // TraitSlot
    if (Widgets.ButtonTextSubtle(
            trait_5, returnTraitAvailibility(
                         5, faction.factionTraits[4]))) // CHANGE THIS
    {
          ...
          list.Add(new FloatMenuOption(trait.label, delegate
                    {
                        List<FloatMenuOption> confirm = new List<FloatMenuOption>();
                        confirm.Add(new FloatMenuOption(TranslatorFormattedStringExtensions.Translate("FCConfirmTrait", trait.label), delegate {
                            faction.factionTraits[4] = new FCPolicy(trait);
                        }));
                        Find.WindowStack.Add(new FloatMenu(confirm));

                    }, mouseoverGuiAction: delegate {
                        TooltipHandler.TipRegion(new Rect(Event.current.mousePosition, new Vector2(500, 200)), returnPolicyText(trait));
                    }));
        }
        Find.WindowStack.Add(new FloatMenu(list));
      }
    }
    if (Mouse.IsOver(trait_5) &&
        faction.factionTraits[4].def != FCPolicyDefOf.empty) {
      TooltipHandler.TipRegion(
          new Rect(Event.current.mousePosition, new Vector2(500, 200)),
    ...
    for (int i = 0; i < faction.settlements.Count(); i++) {
      ...
      // settlement name
      if (Widgets.ButtonTextSubtle(AdjustRect(settlement_1, i),
                                   settlement.name)) {
        Find.WindowStack.Add(new settlementWindowFC(settlement));
      }
      Text.Font = GameFont.Tiny;
      Text.Anchor = TextAnchor.MiddleCenter;
      Widgets.Label(AdjustRect(settlement_2, i),
                    settlement.settlementLevel.ToString());
      Widgets.Label(AdjustRect(settlement_3, i),
                    settlement.settlementMilitaryLevel.ToString());
      Widgets.Label(AdjustRect(settlement_4, i),
                    settlement.getTotalProfit().ToString());
      Widgets.Label(AdjustRect(settlement_5, i),
                    (settlement.workersUltraMax - settlement.getTotalWorkers())
                        .ToString());
      Widgets.Label(AdjustRect(settlement_6, i),
                    settlement.Happiness.ToString());
