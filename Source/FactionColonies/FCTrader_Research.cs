...
public class FCTrader_Research : ITrader {
    ...
    foreach (Map map in Find.Maps) {
      if (map.IsPlayerHome) {
        foreach (Thing thing in map.listerThings.AllThings) {
          if (thing.IsInAnyStorage() == true &&
              thing.def.category == ThingCategory.Item &&
              TradeUtility.PlayerSellableNow(thing, this) &&
              !FactionColonies.canCraftItem(thing.def, true)) {
            yield return thing;
          }
        }
      }
  ...
  public void GiveSoldThingToPlayer(Thing toGive, int countToGive,
                                    Pawn playerNegotiator) {
    // Nothing to give dude
  }

  public void GiveSoldThingToTrader(Thing toGive, int countToGive,
                                    Pawn playerNegotiator) {
  ...
  public IEnumerable<Thing> Goods {
    get { // Possibly set to empty list?
      return Enumerable.Empty<Thing>();
    }
  }

  public TraderKindDef TraderKind {
    get { return DefDatabase<TraderKindDef>.GetNamed("FCResearchTrader"); }
  }
  ...
  public Faction Faction {
    get { return FactionColonies.getPlayerColonyFaction(); }
  }

  public TradeCurrency TradeCurrency {
    get { return TradeCurrency.Silver; }
  }
}
    ...
    if (TradeSession.trader.TraderKind.defName == "FCResearchTrader") {
      ...
      if (__result == true && faction.tradedAmount != 0) {
        Find.LetterStack.ReceiveLetter(
            "FCFactionResearch".Translate(),
            TranslatorFormattedStringExtensions.Translate(
                "PointsAddedToResearchPool", faction.tradedAmount),
