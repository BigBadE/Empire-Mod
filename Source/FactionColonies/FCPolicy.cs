  ...
  public FCPolicy(FCPolicyDef def) {
    ...
    if (def == FCPolicyDefOf.roadBuilders) {
      ResearchProjectDef researchdef =
          DefDatabase<ResearchProjectDef>.GetNamed("FCRoadBuildingDirt", false);
      if (researchdef == null)
        Log.Message("Empire Error - Road research returned Null");
      if (!(Find.ResearchManager.GetProgress(researchdef) ==
...
public class FCPolicyDef : Def, IExposable {
  ...
  public void ExposeData() {
    Scribe_Values.Look<int>(ref factionLevelRequirement,
                            "factionLevelRequirement");
    Scribe_Values.Look<TechLevel>(ref techLevelRequirement,
                                  "techLevelRequirement");
    Scribe_Values.Look<string>(ref desc, "desc");
    Scribe_Values.Look<FCPolicyCategory>(ref category, "category");
    Scribe_Collections.Look<string>(ref positiveEffects, "positiveEffects",
                                    LookMode.Value);
    Scribe_Collections.Look<string>(ref negativeEffects, "negativeEffects",
                                    LookMode.Value);
  }
