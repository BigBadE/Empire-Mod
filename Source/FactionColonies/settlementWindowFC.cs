...
public class settlementWindowFC : Window {
  ...
  private List<string> stats = new List<string>(){
      "militaryLevel", "happiness", "loyalty", "unrest", "prosperity"};
  private List<string> buttons = new List<string>(){
      "DeleteSettlement".Translate(), "UpgradeTown".Translate(),
      "FCSpecialActions".Translate(), "PrisonersMenu".Translate(),
      "Military".Translate()};
  // private List<string> productionButtons = new List<string>() { "Collect
  // Tithe", "View Tithe" };
  ...
  public void DrawProductionHeaderLower(int x, int y, int spacing) {
    ...
    Widgets.Label(new Rect(x, y, 410, 30),
                  "AssignedWorkers".Translate() + ": " +
                      settlement.getTotalWorkers().ToString() + " / " +
                      settlement.workersMax.ToString() + " / " +
                      settlement.workersUltraMax.ToString());
    ...
    Widgets.Label(new Rect(x + 80, y + 30, 100, 40),
                  "ProductionEfficiency".Translate());
    ...
    Widgets.Label(new Rect(x + 365, y + 30, 45, 40),
                  "EstimatedProfit".Translate());

    Widgets.DrawHighlight(new Rect(x + 420, y + 30, 45, 40));
    Widgets.Label(new Rect(x + 420, y + 30, 45, 40),
                  "TaxPercentage".Translate());
      ...
      if ((i * scrollSpacing) + scroll < 0) {
        ...
        case 8:
          if (Widgets.ButtonImage(
                  new Rect(x - 15, scroll + y + 65 + (i * (45 + spacing)) + 8,
                           20, 20),
                  texLoad.iconCustomize)) { // if click faction customize button
            int k = i;
            ...
            options.Add(new FloatMenuOption("Enable All", delegate {
              PaymentUtil.resetThingFilter(settlement, k);
              resource.returnLowestCost();
            }));
            options.Add(new FloatMenuOption("Disable All", delegate {
              ...
              if (!FactionColonies.canCraftItem(thing)) {
                ...
                option = new FloatMenuOption(
                    thing.LabelCap + " - Cost - " + thing.BaseMarketValue +
                        " | Allowed: " + resource.filter.Allows(thing),
                    delegate {
                      resource.filter.SetAllow(thing,
                                               !resource.filter.Allows(thing));
                      resource.returnLowestCost();
                    },
                    thing);
                options.Add(option);
        ...
        Widgets.Checkbox(
            new Vector2(x + 8, scroll + y + 65 + (i * (45 + spacing)) + 8),
            ref settlement.returnResourceByInt(i).isTithe, 24, disabled);

        if (settlement.returnResourceByInt(i).isTithe !=
            settlement.returnResourceByInt(i).isTitheBool) {
          settlement.returnResourceByInt(i).isTitheBool =
              settlement.returnResourceByInt(i).isTithe;
        ...
        // Icon
        if (Widgets.ButtonImage(new Rect(x + 45,
                                         scroll + y + 75 + (i * (45 + spacing)),
                                         30, 30),
                                settlement.returnResourceByInt(i).getIcon())) {
          Find.WindowStack.Add(new descWindowFC(
              "SettlementProductionOf".Translate() + ": " +
                  settlement.returnResourceByInt(i).label,
              char.ToUpper(settlement.returnResourceByInt(i).label[0]) +
                  settlement.returnResourceByInt(i).label.Substring(1)));
        };

        // Production Efficiency
        Widgets.DrawBox(
            new Rect(x + 80, scroll + y + 70 + (i * (45 + spacing)), 100, 20));
        Widgets.FillableBar(
            new Rect(x + 80, scroll + y + 70 + (i * (45 + spacing)), 100, 20),
            (float) Math.Min(
                settlement.returnResourceByInt(i).baseProductionMultiplier,
                1.0));
        Widgets.Label(
            new Rect(x + 80, scroll + y + 90 + (i * (45 + spacing)), 100, 20),
            "Workers".Translate() + ": " +
                settlement.returnResourceByInt(i).assignedWorkers.ToString());
        if (Widgets.ButtonText(new Rect(x + 80,
                                        scroll + y + 90 + (i * (45 + spacing)),
                                        20, 20),
                               "<")) { // if clicked to lower amount of workers
          settlement.increaseWorkers(i, -1);
          WindowUpdateFC();
        }
        if (Widgets.ButtonText(new Rect(x + 160,
                                        scroll + y + 90 + (i * (45 + spacing)),
                                        20, 20),
                               ">")) { // if clicked to lower amount of workers
          settlement.increaseWorkers(i, 1);
        ...
        // Base Production
        Widgets.Label(
            new Rect(x + 195, scroll + y + 70 + (i * (45 + spacing)), 45, 40),
            FactionColonies.FloorStat(
                settlement.returnResourceByInt(i).baseProduction));

        // Final Modifier
        Widgets.Label(
            new Rect(x + 250, scroll + y + 70 + (i * (45 + spacing)), 50, 40),
            FactionColonies.FloorStat(
                settlement.returnResourceByInt(i).endProductionMultiplier));

        // Final Base
        Widgets.Label(
            new Rect(x + 310, scroll + y + 70 + (i * (45 + spacing)), 45, 40),
            (FactionColonies.FloorStat(
                settlement.returnResourceByInt(i).endProduction)));

        // Est Income
        Widgets.Label(
            new Rect(x + 365, scroll + y + 70 + (i * (45 + spacing)), 45, 40),
            (FactionColonies.FloorStat(
                settlement.returnResourceByInt(i).endProduction *
        ...
        // Tithe Percentage
        Widgets.Label(
            new Rect(x + 420, scroll + y + 70 + (i * (45 + spacing)), 45, 40),
            FactionColonies.FloorStat(
                settlement.returnResourceByInt(i).taxPercentage) +
                "%");
      }
    }
  ...
  public void DrawHeader() {
    ...
    Widgets.Label(new Rect(5, 15, 30, 30),
                  settlement.settlementLevel.ToString());
    ...
    // Draw town title
    Text.Font = GameFont.Tiny;
    Widgets.Label(new Rect(50, 25, 150, 20),
                  settlement.title); // returnSettlement().title);

    // Draw town location flabor text
    Text.Font = GameFont.Tiny;
    Widgets.Label(
        new Rect(55, 40, 470, 20),
        "Located".Translate() + " " +
            HillinessUtility.GetLabel(
                Find.WorldGrid.tiles[settlement.mapLocation].hilliness) +
            " " + "LandOf".Translate() + " " +
            Find.WorldGrid.tiles[settlement.mapLocation]
                .biome.LabelCap.ToLower()); // returnSettlement().title);

    // Draw header Settings button
    if (Widgets.ButtonImage(
            new Rect(495, 5, 20, 20),
            texLoad.iconCustomize)) { // if click faction customize button
  ...
  public void DrawSettlementStats(int x, int y) {
    ...
    for (int i = 0; i < stats.Count(); i++) {
      Widgets.DrawMenuSection(
          new Rect(x, y + ((statSize + 15) * i), 125, statSize + 10));
      // Widgets.DrawHighlight(new Rect(x, y + ((statSize + 15) * i), 125,
      // statSize + 10));
      if (stats[i] == "militaryLevel") {
        if (Widgets.ButtonImage(new Rect(x + 5 - 2,
                                         y + 5 + ((statSize + 15) * i) - 2,
                                         statSize + 4, statSize + 4),
                                texLoad.iconMilitary)) {
          Find.WindowStack.Add(
              new descWindowFC("SettlementMilitaryLevelDesc".Translate(),
                               "SettlementMilitaryLevel".Translate()));
        };
        Widgets.Label(
            new Rect(x + 50, y + ((statSize + 15) * i), 80, statSize + 10),
      ...
      if (stats[i] == "happiness") {
        if (Widgets.ButtonImage(new Rect(x + 5 - 2,
          ...
          Find.WindowStack.Add(
              new descWindowFC("SettlementHappinessDesc".Translate(),
      ...
      if (stats[i] == "loyalty") {
        if (Widgets.ButtonImage(new Rect(x + 5 - 2,
                                ...
                                texLoad.iconLoyalty)) {
          Find.WindowStack.Add(
              new descWindowFC("SettlementLoyaltyDesc".Translate(),
      ...
      if (stats[i] == "unrest") {
        if (Widgets.ButtonImage(new Rect(x + 5 - 2,
                                ...
                                texLoad.iconUnrest)) {
          Find.WindowStack.Add(
              new descWindowFC("SettlementUnrestDesc".Translate(),
      ...
      if (stats[i] == "prosperity") {
        if (Widgets.ButtonImage(new Rect(x + 5 - 2,
          ...
          Find.WindowStack.Add(
              new descWindowFC("SettlementProsperityDesc".Translate(),
  ...
  public void DrawButtons(int x, int y, int length, int size, int set) {
    ...
    if (set == 1) {
      for (int i = 0; i < buttons.Count(); i++) {
        if (Widgets.ButtonText(new Rect(x, y + ((size + 10) * i), length, size),
                               buttons[i])) { // If click a button button
          if (buttons[i] ==
              "UpgradeTown".Translate()) { // if click upgrade town button
            Find.WindowStack.Add(new settlementUpgradeWindowFC(settlement));
            // Log.Message(buttons[i]);
          }
          if (buttons[i] ==
              "AreYouSureRemove".Translate()) { // if click to delete colony
            Find.WindowStack.TryRemove(this);
            FactionColonies.removePlayerSettlement(settlement);
          }
          if (buttons[i] ==
              "DeleteSettlement".Translate()) { // if click town log button
            // Log.Message(buttons[i]);
          ...
          if (buttons[i] == "FCSpecialActions".Translate()) {
            ...
            if (factionfc.hasPolicy(FCPolicyDefOf.authoritarian))
              list.Add(
                  new FloatMenuOption("FCBuyLoyalty".Translate(), delegate {
                    Find.WindowStack.Add(
                        new FCWindow_Pay_Silver(this.settlement));
                  }));

            if (factionfc.hasPolicy(FCPolicyDefOf.egalitarian))
              list.Add(
                  new FloatMenuOption("FCGiveTaxBreak".Translate(), delegate {
                    if (this.settlement.trait_Egalitarian_TaxBreak_Enabled ==
                        false) {
                      Find.WindowStack.Add(
                          new FCWindow_Confirm_TaxBreak(this.settlement));
                    } else
                      Messages.Message(
                          TranslatorFormattedStringExtensions.Translate(
                              "FCAlreadyGivingTaxBreak",
                              Math.Round(
                                  (double)(
                                      ((this.settlement
            ...
            if (settlement.isUnderAttack == true) {
              ...
              list.Add(new FloatMenuOption(
                  "ChangeDefendingForce".Translate(), delegate {
                    ...
                    foreach (SettlementFC settlement in Find.World
                                 .GetComponent<FactionFC>()
                                 .settlements) {
              ...
              list.Add(new FloatMenuOption(
                  "SettlementNotBeingAttacked".Translate(), null));
  ...
  public void DrawUpgrades(int x, int y, int length, int size) {
    ...
    Widgets.Label(new Rect(x + 5, y + 5, 450, 30),
                  "BuildingUpgrades".Translate());
    ...
    int spacingFromSide =
        15; // (494 - (spacing + boxSide) * elementsPerRow) / 2;

    int row;
    int column;

    Rect box =
        new Rect(0 + spacingFromSide, 0 + spacingFromText, boxSide, boxSide);
    Rect buildingIcon =
        new Rect(4 + box.x, 4 + box.y, boxSide - 8, boxSide - 8);
    ...
    foreach (BuildingFCDef building in settlement.buildings) {
      ...
      nBox = new Rect(new Vector2(box.x + x + ((box.width + spacing) * column),
                                  box.y + y + ((box.height + 10) * row)),
                      box.size);
      nBuilding = new Rect(
          new Vector2(buildingIcon.x + x + ((box.width + spacing) * column),
                      buildingIcon.y + y + ((box.height + 10) * row)),
          buildingIcon.size);
      ...
      if (i < settlement.numberBuildings) {
        ...
        if (Widgets.ButtonImage(nBuilding, texLoad.buildingLocked)) {
          Messages.Message("That Building is locked",
                           MessageTypeDefOf.RejectInput);
        }
      }
      // Optional Label
  ...
  public void DrawDescription(int x, int y, int length, int size) {
    ...
    Widgets.Label(new Rect(x + 5, y + 5, length - 10, size - 10),
  ...
  public void DrawEconomicStats(int x, int y, int length, int size) {
    ...
    Widgets.Label(new Rect(x, 0, length, size), "Total".Translate() + " " +
                                                    "CashSymbol".Translate() +
                                                    " " + "Income".Translate());
    Widgets.Label(new Rect(x, size + 3, length, size),
                  settlement.totalIncome.ToString());
    Widgets.Label(new Rect(x, size * 2 + 6, length, size), "Upkeep");
    Widgets.Label(new Rect(x, size * 3 + 9, length, size),
                  settlement.totalUpkeep.ToString());
    Widgets.Label(new Rect(x + length + 10, 0, length, size),
                  "Total".Translate() + " " + "CashSymbol".Translate() + " " +
                      "Profit".Translate());
    Widgets.Label(new Rect(x + length + 10, size + 3, length, size),
                  settlement.totalProfit.ToString());
    Widgets.Label(new Rect(x + length + 10, size * 2 + 6, length, size),
                  "CostPerWorker".Translate());
    Widgets.Label(new Rect(x + length + 10, size * 3 + 9, length, size),
                  settlement.workerCost.ToString());
  }
