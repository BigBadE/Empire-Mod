...
public class TaxesFC : ILoadReferenceable, IExposable {
  ...
  public void ExposeData() {
    ...
    Scribe_Values.Look<float>(ref electricityAllotted, "electricityAllotted",
                              0);
  ...
  public string GetUniqueLoadID() { return "Taxes_" + this.loadID; }
...
public class BillFC : ILoadReferenceable, IExposable {
  ...
  public string GetUniqueLoadID() { return "Bill_" + this.loadID; }
  ...
  public bool resolve() {
    ...
    if (PaymentUtil.getSilver() >= -1 * this.taxes.silverAmount ||
        this.taxes.silverAmount >= 0) { // if have enough silver on the current
                                        // map to pay  & map belongs to player
      ...
      if (this.taxes.researchCompleted != 0) {
        ...
        Messages.Message(
            TranslatorFormattedStringExtensions.Translate(
                "PointsAddedToResearchPool", this.taxes.researchCompleted),
      ...
      Messages.Message("NotEnoughSilverForBill".Translate() + " " +
                           this.settlement.name + ". " +
                           "ConfiscatedTithes".Translate() + "." + " " +
                           "UnpaidTitheEffect".Translate(),
                       MessageTypeDefOf.NegativeEvent);
      this.settlement.unrest +=
          10 *
          traitUtilsFC.cycleTraits(new double(), "unrestGainedMultiplier",
                                   this.settlement.traits, "multiply") *
          traitUtilsFC.cycleTraits(new double(), "unrestGainedMultiplier",
                                   factionfc.traits, "multiply");
      this.settlement.happiness -=
          10 *
