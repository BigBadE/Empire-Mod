...
public class createColonyWindowFC : Window {
  public override Vector2 InitialSize {
    get { return new Vector2(300f, 600f); }
  }

  public int currentTileSelected = -1;
  public BiomeResourceDef
      currentBiomeSelected; // DefDatabase<BiomeResourceDef>.GetNamed(this.biome)
  ...
  public createColonyWindowFC() {
    ...
    this.doCloseX = true;
    this.windowRect = new Rect(UI.screenWidth - this.InitialSize.x,
                               (UI.screenHeight - this.InitialSize.y) / 2f -
                                   (UI.screenHeight / 8f),
                               this.InitialSize.x, this.InitialSize.y);
  }
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    if (Find.WorldSelector.selectedTile != -1 &&
        Find.WorldSelector.selectedTile != currentTileSelected) {
      currentTileSelected = Find.WorldSelector.selectedTile;
      // Log.Message("Current: " + currentTileSelected + ", Selected: " +
      // Find.WorldSelector.selectedTile);
      ...
      if (currentBiomeSelected.canSettle == true &&
          currentHillinessSelected.canSettle == true &&
    ...
    int silverToCreateSettlement = (int)(
        traitUtilsFC.cycleTraits(new double(), "createSettlementMultiplier",
                                 Find.World.GetComponent<FactionFC>().traits,
                                 "multiply") *
        (FactionColonies.silverToCreateSettlement +
         (500 * (Find.World.GetComponent<FactionFC>().settlements.Count() +
                 Find.World.GetComponent<FactionFC>()
                     .settlementCaravansList.Count())) +
         (traitUtilsFC.cycleTraits(new double(), "createSettlementBaseCost",
                                   Find.World.GetComponent<FactionFC>().traits,
                                   "add"))));
    ...
    if (faction.hasPolicy(FCPolicyDefOf.expansionist)) {
      if (faction.settlements.Count() == 0 &&
          faction.settlementCaravansList.Count() == 0) {
        traitExpansionistReducedFee = false;
        silverToCreateSettlement = 0;
      } else {
        if (faction.traitExpansionistTickLastUsedSettlementFeeReduction == -1 ||
            (faction.traitExpansionistBoolCanUseSettlementFeeReduction)) {
    ...
    DrawLabelBox(10, 50, 100, 100, "TravelTime".Translate(),
                 GenDate.ToStringTicksToDays(timeToTravel));
    DrawLabelBox(153, 50, 100, 100, "InitialCost".Translate(),
                 silverToCreateSettlement + " " + "Silver".Translate());
    ...
    if (currentTileSelected != -1) {
      for (int i = 0;
           i < Find.World.GetComponent<FactionFC>().returnNumberResource();
           i++) {
        int height = 15;
        if (Widgets.ButtonImage(
                new Rect(20, 335 + i * (5 + height), height, height),
                faction.returnResourceByInt(i).getIcon())) {
          Find.WindowStack.Add(new descWindowFC(
              "SettlementProductionOf".Translate() + ": " +
                  faction.returnResourceByInt(i).label,
              char.ToUpper(faction.returnResourceByInt(i).label[0]) +
                  faction.returnResourceByInt(i).label.Substring(1)));
        }
        Widgets.Label(new Rect(40, 335 + i * (5 + height), 60, height + 2),
                      (currentBiomeSelected.BaseProductionAdditive[i] +
                       currentHillinessSelected.BaseProductionAdditive[i])
                          .ToString());
        Widgets.Label(new Rect(110, 335 + i * (5 + height), 60, height + 2),
                      (currentBiomeSelected.BaseProductionMultiplicative[i] *
                       currentHillinessSelected.BaseProductionMultiplicative[i])
                          .ToString());
        Widgets.Label(
            new Rect(180, 335 + i * (5 + height), 60, height + 2),
            ((currentBiomeSelected.BaseProductionAdditive[i] +
              currentHillinessSelected.BaseProductionAdditive[i]) *
             (currentBiomeSelected.BaseProductionMultiplicative[i] *
              currentHillinessSelected.BaseProductionMultiplicative[i]))
    ...
    if (Widgets.ButtonText(new Rect((InitialSize.x - 32 - buttonLength) / 2f,
                                    535, buttonLength, 32),
                           "Settle".Translate() + ": (" +
                               silverToCreateSettlement +
                               ")")) // add inital cost
    {                                // if click button to settle
      if (PaymentUtil.getSilver() >=
          silverToCreateSettlement) // if have enough monies to make new
                                    // settlement
      {
        StringBuilder reason = new StringBuilder();
        if (!TileFinder.IsValidTileForNewSettlement(currentTileSelected,
                                                    reason) ||
            currentTileSelected == -1 ||
            Find.World.GetComponent<FactionFC>().checkSettlementCaravansList(
                currentTileSelected.ToString())) {
          // Alert Error to User
          ...
          if (traitExpansionistReducedFee) {
            faction.traitExpansionistTickLastUsedSettlementFeeReduction =
          ...
          Find.World.GetComponent<FactionFC>().settlementCaravansList.Add(
              tmp.location.ToString());
          Messages.Message(
              "CaravanSentToLocation".Translate() + " " +
                  GenDate.ToStringTicksToDays(
                      (tmp.timeTillTrigger - Find.TickManager.TicksGame)) +
                  "!",
              MessageTypeDefOf.PositiveEvent);
          // when event activate
          // FactionColonies.createPlayerColonySettlement(currentTileSelected);
        }
      } else { // if don't have enough monies to make settlement
        Messages.Message("NotEnoughSilverToSettle".Translate() + "!",
  ...
  public void DrawLabelBox(int x, int y, int length, int height, string text1,
                           string text2) {
    Text.Font = GameFont.Small;
    Text.Anchor = TextAnchor.MiddleCenter;
    // Draw highlight
