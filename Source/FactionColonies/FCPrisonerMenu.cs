...
class FCPrisonerMenu : Window {
  ...
  public FCPrisonerMenu(SettlementFC settlement) {
    ...
    optionPawnHealth =
        new Rect(optionPawnName.x, optionPawnName.y + 20, 300, 20);
    // rect for pawn unrest
    optionPawnUnrest =
        new Rect(optionPawnHealth.x, optionPawnHealth.y + 20, 300, 20);
    // rect for pawn workload
    optionPawnWorkload =
        new Rect(optionPawnUnrest.x, optionPawnUnrest.y + 20, 150, 20);
    // rect for info button
    optionButtonInfo = new Rect(optionBox.x + optionBox.width - 150 - 10,
                                optionPawnHealth.y + 20, 150, 20);
    // rect for action button
    optionButtonAction = new Rect(optionBox.x + optionBox.width - 150 - 10,
                                  optionPawnUnrest.y + 20, 150, 20);
  }
  ...
  public override void DoWindowContents(Rect inRect) {
    ...
    foreach (FCPrisoner prisoner in prisoners) {
      ...
      // Pawn Health
      Widgets.Label(PawnHealth, "Health".Translate().CapitalizeFirst() + " " +
                                    prisoner.health);
      // Pawn Unrest
      // Widgets.Label(PawnUnrest, "Unrest".Translate().CapitalizeFirst() + " "
      // + prisoner.unrest);
      ...
      if (Widgets.ButtonText(PawnWorkload,
                             "FCWorkload".Translate().CapitalizeFirst() + ": " +
                                 workload)) {
        List<FloatMenuOption> list = new List<FloatMenuOption>();
        list.Add(new FloatMenuOption(
            "FCHeavy".Translate().CapitalizeFirst() + " - " +
                "FCHeavyExplanation".Translate(),
            delegate { prisoner.workload = FCWorkLoad.Heavy; }));
        list.Add(new FloatMenuOption(
            "FCMedium".Translate().CapitalizeFirst() + " - " +
                "FCMediumExplanation".Translate(),
            delegate { prisoner.workload = FCWorkLoad.Medium; }));
        list.Add(new FloatMenuOption(
            "FCLight".Translate().CapitalizeFirst() + " - " +
                "FCLightExplanation".Translate(),
            delegate { prisoner.workload = FCWorkLoad.Light; }));
      ...
      if (Widgets.ButtonTextSubtle(ButtonAction, "Actions".Translate())) {
        ...
        list.Add(new FloatMenuOption(
            "SellPawn".Translate() + " $" + prisoner.prisoner.MarketValue +
                " " + "SellPawnInfo".Translate(),
            delegate
        ...
        list.Add(new FloatMenuOption("ReturnToPlayer".Translate(), delegate {
          if (prisoner.healthTracker != null) {
            ...
            prisoner.prisoner.health =
                new Pawn_HealthTracker(prisoner.prisoner);
            prisoner.healthTracker = new Pawn_HealthTracker(prisoner.prisoner);
          }
          ...
          FieldInfo hostFaction =
              typeof(Pawn_GuestTracker)
                  .GetField("hostFactionInt",
                            BindingFlags.NonPublic | BindingFlags.Instance);
          hostFaction.SetValue(prisoner.prisoner.guest,
          ...
          if (map != null)
            DropPodUtility.DropThingsNear(DropCellFinder.TradeDropSpot(map),
                                          Find.CurrentMap, things);
