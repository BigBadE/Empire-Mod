...
class FCBillWindow : Window {
  ...
  public override Vector2 InitialSize {
    get { return new Vector2(628f, 278f); }
  }
  ...
  public FCBillWindow() {
    ...
    billLocationBase =
        new Rect(billDescBase.x + billDescBase.width, 0, 80, billHeight);
    // rect time time remaining
    billTimeRemaining = new Rect(billLocationBase.x + billLocationBase.width, 0,
                                 90, billHeight);
    // rect for bill resolve button
    billResolveBase = new Rect(billTimeRemaining.x + billTimeRemaining.width, 0,
                               140, billHeight);
  }
  ...
  Reset:
    ...
    foreach (BillFC bill in bills) {
      ...
      highlight = new Rect(settlement.x, settlement.y,
                           resolve.x + resolve.width, billHeight);
      ...
      //
      Text.Anchor = TextAnchor.MiddleCenter;
      Widgets.Label(date, GenDate.ToStringTicksToDays(
                              bill.dueTick - Find.TickManager.TicksGame));
      ...
      Widgets.Checkbox(new Vector2(tithe.x + tithe.width / 2 - 12, tithe.y),
                       ref bul);

      if (Widgets.ButtonText(resolve, "ResolveBill".Translate())) {
        if (PaymentUtil.getSilver() >= -1 * (bill.taxes.silverAmount) ||
            bill.taxes.silverAmount >=
                0) { // if have enough silver on the current map to pay  & map
                     // belongs to player
          ...
          if (bill.taxes.researchCompleted != 0) {
            ...
            Messages.Message(
                TranslatorFormattedStringExtensions.Translate(
                    "PointsAddedToResearchPool", bill.taxes.researchCompleted),
          ...
          Messages.Message("NotEnoughSilverOnMapToPayBill".Translate() + "!",
    ...
    if (Widgets.ButtonTextSubtle(billResolveBase, "Auto-Resolve")) {
      ...
      FloatMenuOption option = new FloatMenuOption(
          "Auto-Resolving : " + faction.autoResolveBills, delegate {
            faction.autoResolveBills = !faction.autoResolveBills;
            switch (faction.autoResolveBills) {
            case true:
              Messages.Message("Bills are now autoresolving!",
                               MessageTypeDefOf.NeutralEvent);
              PaymentUtil.autoresolveBills(bills);
              break;
            case false:
              Messages.Message("Bills are now not autoresolving.",
                               MessageTypeDefOf.NeutralEvent);
              break;
            }
          });
    ...
    Widgets.Checkbox(new Vector2(billResolveBase.x + billResolveBase.width - 30,
                                 billResolveBase.y + 3),
                     ref faction.autoResolveBills, 24, true);
