...
public class SoS2HarmonyPatches {
  ...
  class testfix {
    static void Prefix(ref WorldLayer __instance) {
      ...
      Log.Message("world grid tile count" +
                  Find.WorldGrid.tiles.Count().ToString());
      Log.Message("tile 5 info" +
                  ((bool)(Find.WorldGrid.tiles[5] != null)).ToString());
      // foreach (Tile tile in Find.WorldGrid.tiles)
      // {
      // Log.Message("tile info - " + Find.WorldGrid.tiles.IndexOf(tile));
      //}
      Log.Message("world grid hilliness of tile 5" +
                  Find.WorldGrid.tiles[5].hilliness);
    }
  }
  ...
  public static void Prefix() {
    ...
    if (worldcomp.taxMap != null && worldcomp.taxMap.Parent != null &&
        worldcomp.taxMap.Parent.def.defName == "ShipOrbiting") {
  ...
  public static void updateFactionOnPlanet() {
    ...
    foreach (Faction factionOther in Find.FactionManager
      ...
      if (factionOther != faction1 &&
          faction1.RelationWith(factionOther, true) == null) {

        faction1.TryMakeInitialRelationsWith(factionOther);
      }
    }
    worldcomp.updateFactionIcon(ref faction1,
                                "FactionIcons/" + worldcomp.factionIconPath);
    worldcomp.factionUpdated = true;
    // foreach (SettlementFC settlement in worldcomp.settlements)
    //{
    //    Settlement obj =
    //    Find.WorldObjects.SettlementAt(settlement.mapLocation); if (obj !=
    //    null && obj.Faction != faction1)
    //    {
  ...
  public static void Postfix() {
    ...
    // FactionFC worldcomp = Find.World.GetComponent<FactionFC>();
    if (worldcomp != null && worldcomp.planetName != null &&
  ...
  public static void Patch(Harmony harmony) {
    ...
    Type typ = FactionColonies.returnUnknownTypeFromName(
        "SaveOurShip2.WorldSwitchUtility");
    Type typ2 = FactionColonies.returnUnknownTypeFromName(
        "SaveOurShip2.FixOutdoorTemp");

    // Get type inside of type
    Type[] types =
        typ2.GetNestedTypes(BindingFlags.Public | BindingFlags.Static);
    foreach (Type t in types) {
      // Log.Message( t.ToString());
      if (t.ToString() ==
          "SaveOurShip2.FixOutdoorTemp+SelectiveWorldGeneration") {
        typ2 = t;
    ...
    MethodInfo originalpre =
        typ.GetMethod("KillAllColonistsNotInCrypto",
                      BindingFlags.Static | BindingFlags.NonPublic);
    MethodInfo originalpost = typ.GetMethod(
        "DoWorldSwitch", BindingFlags.Static | BindingFlags.NonPublic);
    MethodInfo originalpost2 =
        typ2.GetMethod("Replace", BindingFlags.Static | BindingFlags.Public);
  ...
  public static void ResetFactionLeaders(bool planet = false) {
    ...
    if (planet) {
      ...
      var factions = Find.FactionManager.GetType()
                         .GetField("allFactions", BindingFlags.Instance |
                                                      BindingFlags.NonPublic)
                         .GetValue(Find.FactionManager);
      list = (List<Faction>) factions;
    }
    // Log.Message(list.Count().ToString());
    Log.Message("Resetting faction leaders");
    // List<Faction> list = (List<Faction>)mainclass
    // //mainclass.Field("allFactions", ).GetValue();
    foreach (Faction faction in list) {
      if (faction.leader == null && faction.def.leaderTitle != null &&
          faction.def.leaderTitle != "") {
        try {
          faction.TryGenerateNewLeader();
        } catch (NullReferenceException e) {
          Log.Message("Empire - Error trying to generate leader for " +
                      faction.Name);
        }
...
public class SettlementSoS2Info : IExposable {
  ...
  public SettlementSoS2Info(string planetName, int location,
